...
export interface ConnectionState {
  ...
  /** Awaiting-answer multiple-choice `ask_user_question` (the houhub-mcp blocking
 ...
 * record — the adapter's own wording and recovery buttons — or houhub's
...
interface TurnFailureState {
  ...
  /** houhub's verdict; `paired` once a typed account carries it. */
...
export function AcpConnectionsProvider({ children }: { children: ReactNode }) {
  ...
  const resolveConnectBlockState = useCallback(
    (agent: AcpAgentStatus | null): ConnectBlockState => {
      ...
      return {
        ...
        // `claude` on their PATH reads as a bug in houhub. Name what's actually
        // missing instead.
   ...
   * HouHub runs several agents at once by design, so that is the normal case,
   * not a corner of one.
   *
   ...
   * typed AIR record, and houhub's `turn_failed_*` verdict on the same turn —
   ...
   * list it adopted and houhub renders that verbatim, so a refused or downgraded
  ...
  const localizeBackendError = useCallback(
    (
      ...
      switch (code) {
        ...
        // `session/load` failed in a way houhub could not classify, so the
      ...
      switch (e.type) {
        ...
        case "background_activity": {
          ...
          if (e.settled && e.settled.length > 0) {
            ...
            //    launching tool call's `[[houhub-background-task]]` marker from
  ...
  // (HOUHUB_ACP_IDLE_TIMEOUT_SECS, default 180s) would reap connections
