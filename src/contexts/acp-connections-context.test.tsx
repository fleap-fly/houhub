...
describe("streaming flush window widens with the run it re-renders", () => {
  ...
  // HouHub runs several agents at once by design. A window sized from what one
...
describe("AIR session failures are told as notifications", () => {
  ...
  it("tells one failed turn once — the typed record first, then houhub's verdict", async () => {
    const handlers = await connectOwner()
    startTurn(handlers)
    emit(handlers, { type: "session_failure", record: failure() })
    emit(handlers, {
  ...
  it("tells one failed turn once — houhub's verdict first, then the typed record", async () => {
    const handlers = await connectOwner()
    startTurn(handlers)
    emit(handlers, {
      type: "error",
