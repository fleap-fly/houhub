...
// of every browser tab (WKContentWorld "houhub" on macOS, a CDP isolated world
...
//
// The host defines `__houhubSend(string)` before this script runs. Messages
// are `{ kind, payload }` JSON strings; the host treats every field as
// untrusted input.
;(function houhubBrowserHelper() {
  "use strict"
  if (typeof globalThis.__houhubSend !== "function") return
  if (globalThis.__houhubHelperInstalled) return
  globalThis.__houhubHelperInstalled = true

  var send = globalThis.__houhubSend
  ...
  function trusted(event) {
    return !!event.isTrusted || globalThis.__houhubAcceptUntrusted === true
  }
  ...
  // as a `houhub:console` event whose detail is one JSON string — the one kind
  ...
  // CDP), the host sets `__houhubEngineConsole` before this script runs and
  ...
  var engineConsole = globalThis.__houhubEngineConsole === true
