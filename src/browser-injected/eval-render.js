...
// source text and can do anything, and houhub's isolated world is where every
...
// where the page IS comes from a separate evaluation in houhub's own world.
...
var __houhubEvalClip = function (text, limit) {
...
var __houhubEvalEnvelope = function (kind, text) {
  var clipped = __houhubEvalClip(text, 4000)
...
var __houhubEvalError = function (e) {
  var text
  try {
...
var __houhubEvalRender = function (v) {
  if (v === undefined) return __houhubEvalEnvelope("undefined", "undefined")
  if (v === null) return __houhubEvalEnvelope("null", "null")
  var kind = typeof v
  if (kind === "string") return __houhubEvalEnvelope("string", v)
  if (kind === "boolean" || kind === "number" || kind === "bigint") {
    return __houhubEvalEnvelope(kind, String(v))
  }
  if (kind === "symbol") return __houhubEvalEnvelope("symbol", String(v))
  if (kind === "function") {
    var name = ""
    try {
      name = String(v.name || "")
    } catch {}
    return __houhubEvalEnvelope(
      "function",
  ...
  try {
    if (typeof v.nodeType === "number" && v.nodeName) {
      ...
      return __houhubEvalEnvelope("node", "<" + described + ">")
    }
  } catch {}
  var text
  try {
  ...
  return __houhubEvalEnvelope(array ? "array" : "object", text)
}
