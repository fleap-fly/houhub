...
export interface ConversationRuntimeSession {
  ...
  // ACP `session/load` failed in a way houhub cannot paper over: the agent
...
type Action =
  ...
  | {
      ...
      // the launching tool_result's `[[houhub-background-task]]` marker. If the
    ...
    switch (block.type) {
      ...
      case "tool_call": {
        ...
        currentBlocks.push({
          ...
          // read delegation state (`meta["houhub.delegation"]`) for
...
/**
 * Rewrite the launching tool call's `[[houhub-background-task]]` marker in a turn
