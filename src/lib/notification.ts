 ...
 * may well have HouHub switched off in System Settings and we cannot see it.
 ...
 * `houhub-server` reached over plain `http://` on a LAN address is not a secure
...
export async function deliverSystemNotification(
  ...
  if (isDesktop()) {
    ...
    // `houhub-server` binary never registers — so every notification in those
 ...
 * user can see under "houhub" govern nothing.
 */
