 ...
 *
 * The houhub-mcp `ask_user_question` tool serializes into a session transcript
...
export interface AskQuestion {
   ...
   *  the houhub-mcp ask, which the card matches by header+question). */
  id?: string
}
 ...
 * than as the houhub-mcp array envelope:
 ...
 * houhub-mcp / Claude result (whose `answers` is an ARRAY) never lands here.
 */
 ...
 * note, so codex's record-valued map (`parseCodexAnswers`) and the houhub-mcp
...
function parseOutcomeJson(output: string): AskQuestionOutcome | null {
  ...
  // array-shaped envelope above so a houhub-mcp result never falls through here.
  const codex =
