...
/** A `houhub://` link the backend resolved before the workspace was listening. */
 ...
 * subscribed, so a cold-start `houhub://session/<id>` — resolved in Rust while
 ...
 * transport targets a `houhub-server`, which has no such command (nor a local
