...
export const CONFIG_EXPORT_EXTENSION = "houhubcfg.json"

/** `houhub-config-2026-05-04-11-32-07.houhubcfg.json` — sortable, and obvious
 *  in a downloads folder six months later. */
export function defaultExportFileName(now: Date = new Date()): string {
  const stamp = now.toISOString().slice(0, 19).replace(/[:T]/g, "-")
  return `houhub-config-${stamp}.${CONFIG_EXPORT_EXTENSION}`
}
...
export async function exportConfigToFile(): Promise<ConfigExportSummary | null> {
  ...
  const destPath = await save({
    ...
    filters: [{ name: "HouHub config", extensions: ["json"] }],
  })
