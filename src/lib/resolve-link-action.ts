...
//      window bound to a REMOTE houhub-server, or a loopback http address seen
//      from a browser whose houhub-server bridges its ports. These cannot be
...
export interface LinkSurface {
  ...
  /** The window is bound to a remote houhub-server (`isRemoteDesktopMode()`). */
  ...
  /** Web mode, and the houhub-server bridges its loopback ports so a dev
...
export type LinkAction =
  ...
  | {
       ...
       *  loopback or private and only reachable from the houhub host (a
       *  remote-workspace window, or a browser with the port bridge). */
      remoteOverride: boolean
      /** The address lives on the remote houhub host of a remote-workspace
