...
describe("groupConsecutiveDelegationStatus", () => {
  ...
  it("matches host-prefixed historical names", () => {
    const out = groupConsecutiveDelegationStatus([
      poll("mcp__houhub-mcp__get_delegation_status", "t1"),
      poll("mcp__houhub-delegate__get_delegation_status", "t1"),
      poll("houhub-delegate/get_delegation_status", "t1"),
    ])
    expect(out).toHaveLength(1)
    expect(pollsOf(out[0])).toHaveLength(3)
  })
...
describe("adaptMessageTurn — image tool results", () => {
  ...
  it("renders a Read whose result carries an image as a generated image part", () => {
    const adapted = adaptMessageTurn(
      {
        id: "read-img",
        role: "assistant",
  ...
  it("emits one generated image part per image", () => {
    const adapted = adaptMessageTurn(
      {
        id: "read-pdf",
        role: "assistant",
  ...
  it("leaves a normal text Read result as a tool card", () => {
    const adapted = adaptMessageTurn(
      {
        id: "read-text",
        role: "assistant",
  ...
  it("keeps the running tool card when the image result's tool is still in flight", () => {
    const adapted = adaptMessageTurn(
      {
        id: "read-img-live",
        role: "assistant",
...
const GOOGLE_URI = "houhub://embedded/https%3A%2F%2Fwww.google.com%2F"
/** …and the one the chip for the site it came from is keyed by. */
const GOOGLE_SITE = "houhub://embedded/https%3A%2F%2Fwww.google.com"
/** The same pair for linux.do. */
const LINUX_DO_URI = "houhub://embedded/https%3A%2F%2Flinux.do%2F"
const LINUX_DO_SITE = "houhub://embedded/https%3A%2F%2Flinux.do"
...
describe("extractUserResourcesFromText — houhub references stay inline", () => {
  it("keeps a houhub://agent link inline (the @-prefixed label no longer lifts it to a chip)", () => {
    const input = "ask [@Codex](houhub://agent/codex) to review"
    const { text, resources } = extractUserResourcesFromText(input)
    expect(resources).toEqual([])
    expect(text).toBe(input)
  })

  it("keeps houhub://session and houhub://commit links inline", () => {
    const session = extractUserResourcesFromText(
      "see [#42](houhub://session/claude_code_abc)"
    )
    expect(session.resources).toEqual([])
    expect(session.text).toBe("see [#42](houhub://session/claude_code_abc)")

    const commit = extractUserResourcesFromText(
      "from [a1b2c3d](houhub://commit/%2Frepo@a1b2c3ddeadbeef)"
    )
  ...
  it("keeps a houhub://session link inline even when its label starts with @ (a session titled '@…')", () => {
    const input = "ping [@周报](houhub://session/codex_99)"
    const { text, resources } = extractUserResourcesFromText(input)
    expect(resources).toEqual([])
    expect(text).toBe(input)
  })
  ...
  it("chips a houhub://embedded attachment while keeping its inert badge inline", () => {
    const { text, resources } = extractUserResourcesFromText(
      "here [report.pdf](houhub://embedded/abc-123) ok"
    )
    expect(resources).toEqual([
      { name: "report.pdf", uri: "houhub://embedded/abc-123", mime_type: null },
    ])
    expect(text).toBe("here [report.pdf](houhub://embedded/abc-123) ok")
  })
  ...
  it("lists two pages of one site under it once", () => {
    const topic = "houhub://embedded/https%3A%2F%2Flinux.do%2Ft%2Ftopic%2F1"
    const latest =
      "houhub://embedded/https%3A%2F%2Flinux.do%2Flatest%3Fpage%3D2"
    const { resources } = extractUserResourcesFromText(
  ...
  it("keeps both file:// and session links inline; only the file is also chipped", () => {
    const { text, resources } = extractUserResourcesFromText(
      "compare [foo.ts](file:///x/foo.ts) with [#42](houhub://session/codex_abc)"
    )
    expect(resources).toEqual([
      { name: "foo.ts", uri: "file:///x/foo.ts", mime_type: null },
    ])
    expect(text).toContain("[#42](houhub://session/codex_abc)")
  ...
  it("does not read the page's own markup as the sender's references", () => {
    ...
    expect(resources).toEqual([
      {
        name: "shop.test",
        uri: "houhub://embedded/https%3A%2F%2Fshop.test",
        mime_type: null,
      },
    ])
    expect(text).toBe("hi")
...
describe("adaptMessageTurn — user reference resources", () => {
  ...
  it("keeps an agent reference inline in the user turn (no chip row)", () => {
    const adapted = adaptMessageTurn(
      {
        ...
        blocks: [
          {
            type: "text",
            text: "ask [@Codex](houhub://agent/codex) to review",
          },
        ],
      },
      msgText
    ...
    expect(part.text).toContain("[@Codex](houhub://agent/codex)")
  })
    ...
    const adapted = adaptMessageTurn(
      {
        ...
        blocks: [
          {
            type: "text",
            text: "compare these [#42](houhub://session/codex_abc)",
          },
  ...
  it("names a screenshot the way the composer did, and lists its page", () => {
    ...
    const uri = "houhub://embedded/https%3A%2F%2Flinux.do%2Ft%2Ftopic%2F1"
  ...
  it("names a page's console lines by how many there were", () => {
    ...
    const uri = "houhub://embedded/http%3A%2F%2F127.0.0.1%3A3000%2F"
    expect(adapted.content).toEqual([
    ...
    expect(adapted.userResources).toEqual([
      {
        name: "127.0.0.1:3000",
        uri: "houhub://embedded/http%3A%2F%2F127.0.0.1%3A3000",
        mime_type: null,
      },
    ])
  })
  ...
  // What houhub's own projection sends a viewer on another client: the block's
  ...
  it("reads a projected page whose title wrote the block's own tag", () => {
    ...
    expect(adapted.content).toEqual([
      {
        type: "text",
        text: "[Page screenshot](houhub://embedded/http%3A%2F%2Fx.test%2F)",
      },
    ])
    expect(adapted.userResources).toEqual([
      {
        name: "x.test",
        uri: "houhub://embedded/http%3A%2F%2Fx.test",
        mime_type: null,
      },
    ])
  })
  ...
  it("keeps an address the person typed that matches a block's", () => {
    ...
    expect(adapted.content).toEqual([
      {
        type: "text",
        text: "[a.title.raw-link.raw-topic-link](houhub://embedded/https%3A%2F%2Flinux.do%2F) https://linux.do/",
      },
    ])
  })
})
