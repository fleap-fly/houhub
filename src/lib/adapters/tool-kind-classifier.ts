...
/**
 * The houhub-mcp workbench companions, which own `HouhubMcpToolCard` (and, for
 ...
 *
 * MUST stay in sync with `HOUHUB_MCP_WORKBENCH_TOOLS` in `@/lib/houhub-mcp-tool`
 ...
 */
const HOUHUB_MCP_WORKBENCH_NAMES: ReadonlySet<string> = new Set([
  "get_session_info",
  "task_progress",
  "task_complete",
  "create_automation",
...
const HOUHUB_MCP_WORKBENCH_SUFFIX_RE =
...
export function isAgentLikeToolName(toolName: string): boolean {
  ...
  if (
    ...
    name === "create_goal" ||
    name === "update_goal" ||
    // houhub-mcp ask_user_question — owns the AskQuestionResultCard, so it must
    ...
    // houhub-mcp check_user_feedback — owns the FeedbackCheckResultCard capsule,
  ...
  if (HOUHUB_MCP_WORKBENCH_NAMES.has(name)) return true
  if (HOUHUB_MCP_WORKBENCH_SUFFIX_RE.test(name)) return true
  return false
}
