...
export type AdaptedToolCallPart = {
   ...
   * `<DelegatedSubThread>` takes `meta["houhub.delegation"]` as a binding
 ...
 *  ref (file / houhub / non-resource link), or "" for a moved-out `@mention`. */
function handleMarkdownLink(
  ...
  // (`href.startsWith("houhub:")`): an unrecognized houhub path is parsed back to
  // null there and degrades to a plain inline link — still in-flow, never a chip.
  // (The `@`-prefixed agent link `[@label](houhub://agent/…)` would otherwise be
  // caught by `hasMentionLabel` below.)
  if (normalizedUri.toLowerCase().startsWith("houhub:")) {
    // A `houhub://embedded/…` ref is a path-less pasted attachment — still an
    // attached file, so it is COPIED to the row too (kept inline as its inert
    // badge). Other houhub refs are not attachments: inline only.
    if (normalizedUri.toLowerCase().startsWith("houhub://embedded/")) {
 ...
 *
 * The same `houhub://embedded/…` shape the composer mints for one
...
function embeddedDisplayUri(ref: string): string {
  return `houhub://embedded/${encodeURIComponent(ref)}`
}
 ...
 *    codex's own search (`CODEX_SEARCH_ACTION_META_KEY`). houhub advertises
 ...
 * `rawInput.plan` on the plan-review permission request HouHub seeds a tool call
    ...
    if (block.type === "tool_use") {
      ...
      if (matchedResult) {
        ...
        if (positionalResult) {
          ...
          // An unmatched tool in a persisted turn is normally historical, but
          // a caller may prove that its turn is still in flight even when this
          // client is not the streaming client. Preserve the running state in
          // that case so status/delegation cards do not claim completion.
          adaptedContent.push({
            type: "tool-call",
            toolCallId,
            toolName: block.tool_name,
