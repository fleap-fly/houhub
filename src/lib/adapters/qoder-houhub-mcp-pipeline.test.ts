 ...
 * `houhub-mcp-tool-card.test.tsx` pins the rendering, but the passes BETWEEN them
 * are where a houhub-mcp call would silently disappear: `dropEmptyInFlightToolCalls`
 ...
 * named `houhub-mcp` wired into `session/new`, driving one live turn:
 *
 *   TOOL_CALL        {"title":"get_delegation_status (houhub-mcp MCP Server)",
...
describe("Qoder live frames reach their card through every adapter pass", () => {
  ...
  it("does not let a run of houhub-mcp calls collapse into one tool-group", () => {
