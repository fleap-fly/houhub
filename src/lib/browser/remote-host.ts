// Which addresses belong to the remote houhub host a window is bound to.
//
...
/** The remote houhub-server's own host name, in a remote-workspace window;
 *  null anywhere else. */
export function remoteServerHost(): string | null {
  return isRemoteDesktopMode() ? hostnameOf(getServerBaseUrl()) : null
}

/** The remote houhub-server's host as a person recognises it — null when it
...
/** Whether `url`, opened in this window, names a place on the remote houhub
