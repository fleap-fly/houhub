...
// a dev server on the houhub host (`http://localhost:3000` as an agent printed
...
export interface BridgeStatus {
   ...
   *  own port (`{port}.houhub.example.com`, `auto`). */
...
export async function probeBridge(
  ...
  try {
    const response = await fetch(`${origin}/__houhub_bridge/ping`, {
      mode: "cors",
      credentials: "omit",
      cache: "no-store",
