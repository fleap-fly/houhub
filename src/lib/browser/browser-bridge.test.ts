...
const grant: BridgeGrant = {
  ...
  bridgePort: 3081,
  bridgeHost: null,
  entryPath: "/__houhub_bridge/enter/cap123",
  publicHost: null,
  path: "/docs?x=1",
}
...
const hostGrant: BridgeGrant = {
  ...grant,
  bridgePort: null,
  bridgeHost: "3000.houhub.example",
}
...
describe("bridge URLs", () => {
  it("use the page's host and scheme with the bridge port", () => {
    ...
    expect(bridgeEntryUrl(grant, page)).toBe(
      "http://192.168.1.5:3081/__houhub_bridge/enter/cap123?to=%2Fdocs%3Fx%3D1"
    )
  })
  ...
  it("prefer the server's public host and bracket IPv6", () => {
    expect(
      bridgeOrigin(
        ...
        { protocol: "https:", hostname: "houhub.example", port: "" }
      )
  ...
  it("use the server's hostname for this target on the page's own port", () => {
    // Named by hostname the bridge answers on houhub's own listener, so the
    // browser keeps the port it is already talking to — including none at
    // all, which is what a deployment behind 443 looks like.
    const page = { protocol: "https:", hostname: "houhub.example", port: "" }
    expect(bridgeOrigin(hostGrant, page)).toBe("https://3000.houhub.example")
    expect(bridgeEntryUrl(hostGrant, page)).toBe(
      "https://3000.houhub.example/__houhub_bridge/enter/cap123?to=%2Fdocs%3Fx%3D1"
    )
    expect(bridgeOrigin(hostGrant, { ...page, port: "8443" })).toBe(
      "https://3000.houhub.example:8443"
    )
    ...
    ).toBe("http://3000.houhub.example:3080")
  })
  ...
  it("send an empty path to the root and keep the address's fragment", () => {
    ...
    expect(bridgeEntryUrl({ ...grant, path: "" }, page)).toBe(
      "http://h:3081/__houhub_bridge/enter/cap123?to=%2F"
    )
    expect(bridgeEntryUrl(grant, page, "#install")).toBe(
      "http://h:3081/__houhub_bridge/enter/cap123?to=%2Fdocs%3Fx%3D1%23install"
    )
    expect(bridgeEntryUrl(grant, page, "")).toBe(
      "http://h:3081/__houhub_bridge/enter/cap123?to=%2Fdocs%3Fx%3D1"
    )
...
describe("probeBridge", () => {
  it("is true only for an ok answer from the ping route", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://h:3081/__houhub_bridge/ping",
