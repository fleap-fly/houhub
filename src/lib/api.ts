 ...
 * `mode` "apikey" writes the houhub-managed ~/.kimi-code/config.toml provider/model
...
export type AntigravityLoginOutcome = {
  ...
  message: string | null
  /**
   * Whether the same link still works. True only when HouHub rejected the paste
   ...
   */
  retryable: boolean
  /** Where the credential landed, when HouHub can name the file. */
 ...
 * on a headless server (HouHub deployed on Linux, no desktop) the first session
 ...
 * cannot reach — HouHub can, so it performs the redirect on their behalf. Only
 * the OAuth parameters are taken from the paste; the target itself comes from
 * the link HouHub issued.
 */
...
// instead of ones houhub ships hand-written support for.
// ---------------------------------------------------------------------------
...
export interface RegistryCatalogAgent {
  ...
  /** houhub already ships hand-written support for this agent. */
  builtin: boolean
...
/**
 * Detect, per houhub-supported agent, whether the HyperFrames skills are already
...
export async function uploadAttachment(
  ...
  if (file.size === 0) {
    ...
    // (the server records it under `~/.houhub/uploads/<bucket>/...`), and
...
// houhub-server bound to the current window. The Tauri-native drag-drop event
...
export interface DelegationSettings {
  ...
  /** Optional per-agent overrides applied when houhub-mcp spawns a subagent.
...
// ─── houhub-mcp service status ──────────────────────────────────────────

/** Headline verdict from Rust `HouHubMcpServiceState`. Ordered by which problem
 * to solve first: only `stopped` is repairable from this process. */
export type HouHubMcpServiceState =
  | "stopped"
  | "unavailable"
  | "disabled"
  | "running"
...
export interface HouHubMcpToolGroup {
  key: string
  enabled: boolean
}

/** Mirror of Rust `HouHubMcpServiceStatus`. */
export interface HouHubMcpServiceStatus {
  state: HouHubMcpServiceState
  /** Whether the broker socket answered a liveness ping just now. */
  listening: boolean
  socket_path: string
  /** Resolved `houhub-mcp` path; `null` when the lookup came up empty. */
  binary_path: string | null
  tool_groups: HouHubMcpToolGroup[]
...
export async function getHouHubMcpServiceStatus(): Promise<HouHubMcpServiceStatus> {
  return getTransport().call("get_houhub_mcp_service_status")
}

/** Bind the broker socket if it isn't already answering. Idempotent. */
export async function startHouHubMcpService(): Promise<void> {
  return getTransport().call("start_houhub_mcp_service")
}
...
export async function setHouHubMcpToolGroup(
  key: string,
  enabled: boolean
): Promise<void> {
  return getTransport().call("set_houhub_mcp_tool_group", { key, enabled })
}
 ...
 * houhub-mcp will receive when a subagent is spawned for delegation.
 *
...
export interface BackupManifest {
  ...
  /** Which houhub-owned sections this archive claims to manage. */
...
export async function exportBackupDesktop(
  ...
  const ext = encrypted ? "houhubbak" : "houhub.zip"
...
/** Discard a staged restore that has not yet been applied. */
