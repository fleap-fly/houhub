...
describe("MONACO_UNICODE_HIGHLIGHT_OPTIONS", () => {
  ...
  it("keeps invisible-character highlighting at its default (still useful)", () => {
    ...
    // boxes legible text and helps catch invisible copy-paste artifacts.
...
describe("EDITOR_CANVAS_BG", () => {
  ...
  it("encodes color and mode in the theme name", () => {
    expect(monacoThemeName("neutral", false)).toBe("houhub-light-neutral")
    expect(monacoThemeName("blue", true)).toBe("houhub-dark-blue")
  })
})
