...
describe("HouHub update release configuration", () => {
  ...
  it("publishes signed desktop updater assets and latest.json", () => {
    ...
    expect(workflow).toContain("Create stable installer aliases")
    expect(workflow).toContain("Sync production update manifest")
    expect(workflow).toContain("HOUHUB_PRODUCTION_DEPLOY_KEY")
