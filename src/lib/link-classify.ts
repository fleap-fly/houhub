...
import { isProjectSpaceFilePath } from "@/lib/file-open-target"
...
function isLocalPathLike(path: string): boolean {
  ...
  return (
    ...
    path.startsWith("../") ||
    path.startsWith("~/") ||
    // A mounted project space (`ps://<projectId>/…`) is not a path on
    // disk, but it IS a file the workspace can open and it names its own
    // root — so it routes through the file opener, never a URL handler.
...
export function parseLocalFileTarget(rawUrl: string): LocalFileTarget | null {
  ...
  // Before the scheme test: `ps://<projectId>/…` reads as a URL scheme but is
  // a workspace path, so it must not be rejected as an unsupported protocol.
