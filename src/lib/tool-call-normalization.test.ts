...
describe("inferLiveToolName meta.claudeCode.toolName override", () => {
  ...
  it("resolves delegate_to_agent from broker delegation meta on identity-less wires", () => {
    ...
    // meta["houhub.delegation"] write is the only live identity signal.
    expect(
      inferLiveToolName({
        title: "MCP: tool",
        kind: "other",
  ...
  it("keeps input-shape priority for calls without delegation meta", () => {
    ...
    // the delegation resolution is scoped to the HouHub-minted marker.
    expect(
      inferLiveToolName({
        title: "MCP: tool",
        kind: "other",
  ...
  it("resolves delegation companion tools from meta over the input-shape heuristic", () => {
    ...
    expect(
      inferLiveToolName({
        title: "mcp__houhub-delegate__get_delegation_status",
        kind: "other",
        rawInput: JSON.stringify({ task_ids: ["t1"], wait_ms: 1000 }),
        meta: {
          claudeCode: {
            toolName: "mcp__houhub-delegate__get_delegation_status",
          },
        },
      })
    ...
    expect(
      inferLiveToolName({
        title: "mcp__houhub-delegate__cancel_delegation",
        kind: "other",
        rawInput: JSON.stringify({ task_id: "t1" }),
        meta: {
          claudeCode: { toolName: "mcp__houhub-delegate__cancel_delegation" },
        },
      })
    ...
    expect(
      inferLiveToolName({
        title: "mcp__houhub-delegate__delegate_to_agent",
        kind: "other",
        rawInput: JSON.stringify({ agent_type: "codex", task: "do it" }),
        meta: {
          claudeCode: { toolName: "mcp__houhub-delegate__delegate_to_agent" },
        },
      })
    ).toBe("delegate_to_agent")
  })
  ...
  it("resolves Grok companion tools from the unwrapped title over the input shape", () => {
    ...
    expect(
      inferLiveToolName({
        title: "houhub-mcp__cancel_delegation",
        kind: "other",
        rawInput: JSON.stringify({ task_id: "t1" }),
    ...
    expect(
      inferLiveToolName({
        title: "houhub-mcp__get_delegation_status",
        kind: "other",
    ...
    expect(
      inferLiveToolName({
        title: "houhub-mcp__delegate_to_agent",
        kind: "other",
  ...
  it("still classifies a query as websearch when the wire names websearch", () => {
    expect(
      inferLiveToolName({
        title: "web_search",
        kind: "other",
        rawInput: JSON.stringify({ query: "HouHub" }),
      })
    ).toBe("websearch")
  ...
  it("recognizes Codex web-search action frames", () => {
    ...
    expect(
      inferLiveToolName({
        title: "Search",
        kind: "other",
        rawInput: JSON.stringify({ type: "webSearch", query: "HouHub" }),
      })
    ).toBe("websearch")
  })
...
describe("normalizeToolName collapses delegate_to_agent across hosts", () => {
  // The houhub multi-agent delegation MCP tool is named the same across hosts
  ...
  it.each([
    ...
    "mcp__houhub-delegate__delegate_to_agent",
    "mcp__houhub__delegate_to_agent",
    "houhub-mcp/delegate_to_agent",
    "houhub-delegate/delegate_to_agent",
    "houhub-delegate.delegate_to_agent",
    "houhub-delegate:delegate_to_agent",
    "houhub_delegate__delegate_to_agent",
...
describe("normalizeToolName collapses delegation companion tools across hosts", () => {
  it.each([
    ...
    "mcp__houhub-mcp__get_delegation_status",
    "mcp__houhub-delegate__get_delegation_status",
    "mcp__houhub__get_delegation_status",
    "houhub-mcp/get_delegation_status",
    "houhub-delegate/get_delegation_status",
    "houhub-delegate.get_delegation_status",
    "houhub-delegate:get_delegation_status",
  ...
  it.each([
    "cancel_delegation",
    "mcp__houhub-mcp__cancel_delegation",
    "mcp__houhub-delegate__cancel_delegation",
    "mcp__houhub__cancel_delegation",
    "houhub-mcp/cancel_delegation",
    "houhub-delegate/cancel_delegation",
    "houhub-delegate.cancel_delegation",
    "houhub-delegate:cancel_delegation",
...
describe("normalizeToolName collapses check_user_feedback across hosts", () => {
  it.each([
    ...
    "mcp__houhub__check_user_feedback",
    "houhub-mcp/check_user_feedback",
    "houhub-mcp.check_user_feedback",
    "houhub-mcp:check_user_feedback",
...
describe("inferLiveToolName cursor task and MCP shapes", () => {
  ...
  it("resolves cursor MCP calls to <provider>__<tool> instead of bash", () => {
    ...
    expect(
      inferLiveToolName({
        title: "houhub-mcp: delegate_to_agent",
        kind: "other",
        rawInput: JSON.stringify({
          providerIdentifier: "houhub-mcp",
...
describe("inferLiveToolName codex plan_review marker", () => {
  it("classifies the seeded plan-review call from _meta.codex.kind", () => {
    // codex-acp ≥1.1.8 (#351): HouHub seeds this tool call from the permission
...
// the broker's own `houhub.delegation` marker — while every other houhub-mcp
...
describe("inferLiveToolName resolves Qoder's authoritative _meta.qoder.toolName", () => {
  const qoderMcpCall = (tool: string, input: unknown) => ({
    title: `${tool} (houhub-mcp MCP Server)`,
    kind: "other",
    rawInput: JSON.stringify(input),
    meta: { qoder: { toolName: `mcp__houhub-mcp__${tool}` } },
  })
  ...
  it("resolves the frame captured verbatim from qodercli 1.1.25", () => {
    ...
    // `houhub-mcp` — copied byte-for-byte from the session/update payload:
    //   {"title":"get_delegation_status (houhub-mcp MCP Server)","kind":"other",
    //    "_meta":{"qoder":{"toolName":"mcp__houhub-mcp__get_delegation_status"}},
    //    "rawInput":{"task_ids":["081139e7"]}}
    expect(
      inferLiveToolName({
        title: "get_delegation_status (houhub-mcp MCP Server)",
        kind: "other",
        rawInput: JSON.stringify({ task_ids: ["081139e7"] }),
        meta: {
          qoder: { toolName: "mcp__houhub-mcp__get_delegation_status" },
        },
      })
    ).toBe("get_delegation_status")
  })
  ...
  it("proves the title alone could never have resolved these", () => {
    ...
    expect(
      normalizeToolName("get_delegation_status (houhub-mcp MCP Server)")
  ...
  it("rescues cancel_delegation from the generic 'task' input shape", () => {
    ...
    expect(
      inferLiveToolName({
        title: "cancel_delegation (houhub-mcp MCP Server)",
        kind: "other",
        rawInput: JSON.stringify({ task_id: "t1" }),
        meta: null,
      })
...
// The historical path reads the raw `mcp__houhub-mcp__<tool>` name straight out
...
describe("normalizeToolName collapses the houhub-mcp workbench companions", () => {
  const TOOLS = [
    "get_session_info",
    "task_progress",
    "task_complete",
...
describe("inferLiveToolName meta.opencode.toolName override", () => {
  ...
  it("still routes a delegation companion by its own markers", () => {
    ...
    expect(
      call("houhub-mcp_get_delegation_status", "status", "other", {
        task_ids: ["t1"],
      })
    ).toBe("get_delegation_status")
    expect(
      call("houhub-mcp_ask_user_question", "asked", "other", { questions: [] })
    ).toBe("question")
  })
