...
describe("formatConversationTitle", () => {
  ...
  it("reduces session/commit/agent links (houhub:// uris) to their label", () => {
    expect(
      formatConversationTitle("[My chat](houhub://session/codex_abc)")
    ).toBe("My chat")
    expect(
      formatConversationTitle("[abc1234](houhub://commit/%2Frepo@abc)")
    ).toBe("abc1234")
    // An agent reference keeps the `@` — it lives inside the bracket text.
    expect(formatConversationTitle("[@Codex](houhub://agent/codex)")).toBe(
      "@Codex"
    )
  })
