/**
 * Parsing helpers for the houhub-mcp *workbench* companion tools — the ones that
 * report back into houhub itself rather than driving a sub-agent:
 ...
 * `"task_progress (houhub-mcp MCP Server)"`. This module resolves the one detail
 * each call is actually about (which session, which message, which verdict) plus
 * its result text, so `HouhubMcpToolCard` can state it in one line.
 *
 ...
 * collapse every host spelling (`mcp__houhub-mcp__task_progress`,
 * `houhub-mcp/task_progress`, …) onto exactly these.
 */
export const HOUHUB_MCP_WORKBENCH_TOOLS = [
  "get_session_info",
  "task_progress",
  "task_complete",
  "create_automation",
...
export type HouhubMcpWorkbenchTool = (typeof HOUHUB_MCP_WORKBENCH_TOOLS)[number]

const TOOL_SET: ReadonlySet<string> = new Set(HOUHUB_MCP_WORKBENCH_TOOLS)

/** Whether `name` is one of the workbench companions, in canonical form. */
export function isHouhubMcpWorkbenchTool(
  name: string
): name is HouhubMcpWorkbenchTool {
...
export interface HouhubMcpToolModel {
  /**
 ...
 * mcp__houhub-mcp__create_automation") instead of the automation's. Safe for all
...
/**
 * Resolve everything `HouhubMcpToolCard` renders from a tool call's raw input,
 ...
 */
export function parseHouhubMcpToolCall(params: {
  tool: HouhubMcpWorkbenchTool
  input?: string | null
  output?: string | null
