...
import { toErrorMessage as formatAppErrorMessage } from "@/lib/app-error"
...
// Cloud (PS) project spaces have no FS watcher; poll the tree on an interval.
const PS_POLL_INTERVAL_MS = 15_000
...
function toWorkspaceErrorMessage(error: unknown, isProjectSpace = false): string {
  ...
  if (/workbench is not signed in/i.test(message)) {
    return "请先登录工作台后再打开项目空间。"
  }

  if (
    /Failed to acquire connection\s*from pool/i.test(message) ||
    /Failed to acquire connectionfrom pool/i.test(message) ||
    /Connection pool timed out/i.test(message)
  ) {
    return "工作台项目空间暂时无法连接数据库：连接池超时。请稍后重试；如果持续出现，请联系管理员检查 Project System 数据库连接池。"
  }
...
class WorkspaceStateStore {
  ...
  private readonly isPs: boolean
  private psPollTimer: ReturnType<typeof setInterval> | null = null

  constructor(rootPath: string) {
    this.rootPath = rootPath
    this.normalizedRootPath = normalizeComparePath(rootPath)
    this.isPs = isPsPath(rootPath)
    this.state = {
      ...EMPTY_STATE,
      rootPath,
      isGitRepo: !this.isPs,
  ...
  // ── PS (cloud project space) support ──
  // PS roots have no filesystem watcher; we fetch the tree via the ps-aware
  // getFileTree and refresh it on an interval. There is no git surface.
  private psRefreshTree = async () => {
    try {
      const tree = await getFileTree(this.rootPath)
      ...
      this.patchState((prev) => ({
        ...prev,
        health: "degraded",
        error: toWorkspaceErrorMessage(error, true),
      }))
    }
  }

  private psStart = async (lifecycleId: number) => {
    await this.psRefreshTree()
    if (!this.isLifecycleActive(lifecycleId)) return
    this.started = true
    this.hasBaselineSnapshot = true
    if (!this.psPollTimer) {
      this.psPollTimer = setInterval(() => {
        if (this.tokenCount > 0) void this.psRefreshTree()
      }, PS_POLL_INTERVAL_MS)
    }
  }

  private psStopPolling = () => {
    if (this.psPollTimer) {
    ...
    const run = async () => {
      ...
      try {
        ...
        this.patchState((prev) => ({
          ...prev,
          health: "degraded",
          error: toWorkspaceErrorMessage(error),
        }))
      }
    }
  ...
  private ensureStarted = async (lifecycleId: number) => {
    ...
    const start = async () => {
      ...
      if (this.isPs) {
        await this.psStart(lifecycleId)
        return
      }
