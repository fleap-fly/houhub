 ...
 *
 * `supported` is false for an agent houhub's skill store can't manage — today
 * only a pi pointed at a custom `PI_CODING_AGENT_DIR`, whose skills live in a
 * per-agent dir houhub's default-dir store never touches. For such an agent
...
export function useEnabledSkillIds(agentType: AgentType | null): {
  ...
  // A pi pointed at a custom PI_CODING_AGENT_DIR isn't managed by houhub's
