...
/** All `houhub://skill/…` anchors anywhere in the tree. */
function skillAnchors(node: Node): Node[] {
  ...
  const walk = (n: Node) => {
    if (
      ...
      n.properties.href.startsWith("houhub://skill/")
    ) {
      out.push(n)
    }
...
describe("rehypeCommandBadges", () => {
  it("badges a `/command` token, keeping the literal prefix", () => {
    ...
    expect(anchors[0].properties?.href).toBe("houhub://skill/review")
  ...
  it("badges a `$skill` token", () => {
    ...
    const anchors = skillAnchors(tree)
    expect(anchors).toHaveLength(1)
    expect(anchors[0].properties?.href).toBe("houhub://skill/deploy")
  ...
  it("skips text inside an existing link (no nested anchors)", () => {
    ...
    // Only the original non-houhub link remains; no houhub://skill anchor added.
    expect(skillAnchors(tree)).toEqual([])
  })
