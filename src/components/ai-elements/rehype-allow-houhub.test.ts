...
describe("rehypePluginsAllowingHouhub", () => {
  it("adds `houhub` to the sanitize schema's href protocol allow-list", () => {
    ...
    const href = hrefProtocols(
      rehypePluginsAllowingHouhub(defaultRehypePlugins)[sanitizeIndex]
    )
    expect(href).toContain("houhub")
    // Exactly once — no duplicate even if re-derived.
    expect(href?.filter((p) => p === "houhub")).toHaveLength(1)
  ...
  it("preserves plugin count and order, passing raw/harden through by reference", () => {
    ...
    const result = rehypePluginsAllowingHouhub(defaultRehypePlugins)
  ...
  it("clones rather than mutating the shipped sanitize schema", () => {
    // The shipped default must not already contain houhub, else the fix is moot.
    expect(hrefProtocols(defaultRehypePlugins.sanitize)).not.toContain("houhub")
    rehypePluginsAllowingHouhub(defaultRehypePlugins)
