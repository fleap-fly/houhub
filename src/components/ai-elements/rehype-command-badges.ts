...
// wrapping each in a `houhub://skill/<slug>` link, which MarkdownLink renders as a
...
/** A `houhub://skill/<slug>` link whose text keeps the literal `/`·`$` prefix. */
function badgeAnchor(token: string): HastNode {
  const slug = token.slice(1)
  return {
    type: "element",
    tagName: "a",
    properties: { href: `houhub://skill/${encodeURIComponent(slug)}` },
