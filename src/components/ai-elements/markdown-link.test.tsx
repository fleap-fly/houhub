...
describe("MarkdownLink", () => {
  ...
  describe("houhub:// reference badges", () => {
    it("renders a session link as a session badge (conversation glyph, no agent icon or status dot)", () => {
      render(
        <MarkdownLink href="houhub://session/codex_abc">My chat</MarkdownLink>
      )
    ...
    it("renders a legacy numeric session link as a session badge", () => {
      render(<MarkdownLink href="houhub://session/123">Login</MarkdownLink>)
    ...
    it("renders a commit link as a commit badge", () => {
      render(
        <MarkdownLink href="houhub://commit/%2Frepo@abc1234def">
          abc1234
        </MarkdownLink>
      )
    ...
    it("renders an agent link as an agent badge", () => {
      render(<MarkdownLink href="houhub://agent/codex">@Codex</MarkdownLink>)
      const badge = screen.getByRole("img", { name: "agent: Codex" })
      expect(badge).toHaveAttribute("data-ref-type", "agent")
      expect(badge.querySelector("svg")).not.toBeNull()
    })

    it("leaves a non-reference houhub uri as a normal link", () => {
      render(<MarkdownLink href="houhub://unknown/x">x</MarkdownLink>)
  ...
  describe("embedded attachment badges", () => {
    it("renders a houhub://embedded link as an inert file badge", () => {
      // Path-less pasted bytes serialize to this inert display uri; the badge
      // name is the link text the composer wrote.
      render(
        <MarkdownLink href="houhub://embedded/abc-123">report.pdf</MarkdownLink>
      )
