...
/**
 * A caller-supplied replacement for the default link-safety config, for
 * surfaces whose link routing is scoped to something the default cannot see —
 * a cloud session's outputs, a mounted project space. Set through
 * `StreamdownLinkSafetyProvider`; the descendant transcript's
 * `useStreamdownLinkSafety` reads it instead of the default.
 */
...
export function openLinkWithSafety(
  ...
  // Streamdown's contract also allows a promise. houhub's own config never
...
// True when the opener needs no folder context at all: absolute paths, `~/`
// paths (openFilePreview expands the home dir and routes them by absolute
// path) and mounted project-space `ps://<id>/…` paths, which name their own
// root — inside a registered folder or not.
...
/**
 * Supply a link-safety config to every transcript below, instead of the default
 * one. Used by the cloud/space surfaces, whose links resolve against their own
 * roots before falling back to the ordinary opener.
 */
