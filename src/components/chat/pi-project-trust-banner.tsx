 ...
 *
 * houhub used to write that trust automatically on every pi launch, which silently
 ...
 *   older houhub wrote without asking, and possibly inherited from an ancestor it
...
export function PiProjectTrustBanner({
  ...
  const decide = async (trusted: boolean | null) => {
    ...
    setApplying(true)
    try {
      // The backend keeps houhub's acknowledgement in step with the verdict: a
  ...
  /**
   * Keep an existing grant. pi's trust store is untouched — only houhub's record
