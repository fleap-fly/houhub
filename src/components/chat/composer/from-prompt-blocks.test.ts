...
describe("blocksToRestoredDraft", () => {
  ...
  it("restores a houhub session link as a session reference", () => {
    const { segments } = blocksToRestoredDraft(
      [
        {
          type: "resource_link",
  ...
  it("recovers the agent type from a new-format session link", () => {
    const { segments } = blocksToRestoredDraft(
      [
        {
          type: "resource_link",
          uri: "houhub://session/codex_sess1",
          name: "My chat",
          mime_type: null,
          description: null,
        },
  ...
  it("restores a houhub commit link as a commit reference (hash after @)", () => {
    const { segments } = blocksToRestoredDraft(
      [
        {
          type: "resource_link",
          uri: "houhub://commit/%2Frepo%20a@abc1234def5678",
          name: "abc1234",
          mime_type: null,
          description: null,
        },
