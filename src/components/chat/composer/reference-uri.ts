...
const AGENT_URI = /^houhub:\/\/agent\/(.+)$/i
const SESSION_URI = /^houhub:\/\/session\/(.+)$/i
const COMMIT_URI = /^houhub:\/\/commit\/.*@(.+)$/i
...
const SKILL_URI = /^houhub:\/\/skill\/(.+)$/i
...
// message-input's `embeddedPayloadsRef`). `houhub://` (not `file://`) is used on
...
const EMBEDDED_URI_PREFIX = "houhub://embedded/"
...
/**
 * Parse a composer reference uri (`file://` / `houhub://…`) back into
...
export function parseHouhubReferenceUri(
  ...
  if (session) {
    const id = session[1]
    // Current format is `houhub://session/<conversation_id>` (a bare numeric id),
    ...
    // LEGACY links `houhub://session/<agent_type>_<external_id>` (still present in
