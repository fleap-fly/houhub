...
describe("docToPromptBlocks", () => {
  ...
  it("keeps an agent reference with a houhub uri inline as a markdown link", () => {
    editor
      .chain()
      .insertContent("ask ")
      .insertReference(
    ...
    const blocks = docToPromptBlocks(editor)
    expect(links(blocks)).toHaveLength(0)
    expect(textBlock(blocks)).toContain("[@Codex](houhub://agent/codex)")
  })
  ...
  it("keeps a session reference inline as a houhub:// link (no resource_link)", () => {
    editor
      .chain()
      .insertContent("see ")
      .insertReference(
        ref({
          ...
          uri: "houhub://session/1",
        })
      )
      .run()
    const blocks = docToPromptBlocks(editor)
    expect(links(blocks)).toHaveLength(0)
    expect(textBlock(blocks)).toContain("houhub://session/1")
  })

  it("keeps a commit reference inline as a houhub:// link (no resource_link)", () => {
    editor.commands.insertReference(
      ref({
        refType: "commit",
        id: "abc1234def",
        label: "abc1234",
        uri: "houhub://commit/%2Frepo@abc1234def",
      })
    )
    const blocks = docToPromptBlocks(editor)
    expect(links(blocks)).toHaveLength(0)
    expect(textBlock(blocks)).toContain("houhub://commit/")
  })

  it("does not lift a file-typed reference carrying a non-file (houhub) uri", () => {
    // A pasted/forged node could be refType "file" with a houhub: uri (the node's
    // parseHTML allow-list permits houhub:). It must stay inline, never become an
    ...
    editor.commands.insertReference(
      ref({
        ...
        id: "x",
        label: "x",
        uri: "houhub://session/9",
      })
    )
    const blocks = docToPromptBlocks(editor)
    expect(links(blocks)).toHaveLength(0)
    expect(textBlock(blocks)).toContain("houhub://session/9")
  })

  it("drops an embedded-attachment reference from the prose without lifting it", () => {
    // A path-less pasted attachment badge carries an inert houhub://embedded uri;
    ...
    expect(text).not.toContain("houhub://embedded")
...
describe("serializeDocToDisplayText vs serializeDocToText (embedded)", () => {
  ...
  it("display KEEPS an embedded badge inline; send DROPS it", () => {
    ...
    expect(sent).not.toContain("houhub://embedded")
    expect(sent).not.toContain("report.pdf")
    // Display text: the sender still sees the file they attached, as the same
    // `[label](houhub://embedded/…)` link the transcript renders back to a badge —
    ...
    expect(shown).toContain("[report.pdf](houhub://embedded/abc-123)")
