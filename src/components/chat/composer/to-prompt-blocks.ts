 ...
 *   into trailing `resource_link` blocks: houhub keeps no copy of the user's
 ...
 * - **session / commit** references (a `houhub://` uri the agent can't fetch) and
 *   **agent / skill** references stay inline as their text/link form, unchanged.
 * - **embedded** references (a `houhub://embedded/…` display uri for path-less
...
/** A display-only embedded-attachment reference (`houhub://embedded/…`): dropped
