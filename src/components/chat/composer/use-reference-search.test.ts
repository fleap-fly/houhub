...
describe("buildReferenceGroups", () => {
  ...
  it("adapts sessions into houhub://session references", () => {
    const groups = buildReferenceGroups(
      "login",
      emptySources({
        sessions: [
    ...
    expect(sessions[0].reference.uri).toBe("houhub://session/7")
  })
  ...
  it("adapts commits and filters by hash / message / author", () => {
    ...
    expect(commits[0].reference.uri).toBe("houhub://commit/%2Frepo@abc12340000")
  })
