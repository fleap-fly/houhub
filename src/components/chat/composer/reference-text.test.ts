...
describe("referenceToMarkdown", () => {
  ...
  it("renders a session as a markdown link to its houhub:// uri", () => {
    expect(
      referenceToMarkdown(
        ref({ refType: "session", label: "Login", uri: "houhub://session/123" })
      )
    ).toBe("[Login](houhub://session/123)")
  })
    ...
    expect(
      referenceToMarkdown(
        ref({
          refType: "commit",
          label: "abc1234",
          uri: "houhub://commit/repo@abc1234def",
        })
      )
    ).toBe("[abc1234](houhub://commit/repo@abc1234def)")
  })
  ...
  it("renders an agent with a uri as a [@label](houhub://agent/…) link", () => {
    expect(
      referenceToMarkdown(
        ref({
          refType: "agent",
  ...
  describe("markdown injection is neutralized", () => {
    it("escapes brackets and parens in link text so a label cannot break out", () => {
      ...
      ).toBe("[a\\]\\(http://evil\\) x](houhub://session/1)")
    })

    it("escapes backticks in link text", () => {
      expect(
        referenceToMarkdown(
          ref({ refType: "session", label: "a`b", uri: "houhub://session/2" })
        )
      ).toBe("[a\\`b](houhub://session/2)")
    })
    ...
    it("escapes link-breaking chars in a uri-bearing agent label", () => {
      ...
      ).toBe("[@a\\]\\(http://evil\\) x](houhub://agent/codex)")
    })
