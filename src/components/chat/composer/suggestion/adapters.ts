...
/**
 * ACP agent → agent reference. Carries a `houhub://agent/<agent_type>` uri as a
 * routing anchor: it serializes inline as `[@label](houhub://agent/…)` and
...
export function agentToSuggestion(agent: AcpAgentInfo): SuggestionItem {
  return {
    reference: {
      ...
      uri: `houhub://agent/${agent.agent_type}`,
...
/**
 * Conversation → session reference. The serialization uri encodes houhub's
 * internal numeric conversation id as `houhub://session/<conversation_id>` — the
...
export function sessionToSuggestion(
  ...
  const uri = `houhub://session/${conversation.id}`
  return {
    reference: {
      refType: "session",
...
/**
 * Git commit → commit reference (`houhub://commit/<repoKey>@<fullHash>`).
...
export function commitToSuggestion(
  ...
  return {
    reference: {
      ...
      id: entry.full_hash,
      label: entry.hash,
      uri: `houhub://commit/${encodeURIComponent(repoKey)}@${entry.full_hash}`,
      meta: {
