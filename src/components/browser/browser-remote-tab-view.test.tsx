...
describe("BrowserRemoteTabView", () => {
  ...
  // Without a port, the remote name's default port is whatever fronts houhub
...
describe("BrowserTabView", () => {
  ...
  it("puts the card back with the reason when the open is refused, and asks again on request", () => {
    ...
    expect(
      screen.getByText(
        "The remote houhub-server has its browser tunnel switched off (HOUHUB_BROWSER_TUNNEL)."
      )
    ).toBeVisible()
