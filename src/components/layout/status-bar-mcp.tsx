...
import {
  getHouHubMcpServiceStatus,
  openSettingsWindow,
  setHouHubMcpToolGroup,
  startHouHubMcpService,
  type HouHubMcpServiceState,
  type HouHubMcpServiceStatus,
} from "@/lib/api"
...
type IndicatorState = HouHubMcpServiceState | "unknown"
...
/**
 * houhub-mcp service indicator, bottom-right of the workspace.
 *
 * The companion is what carries houhub's own tools into an agent session —
...
export function StatusBarMcp() {
  ...
  const [status, setStatus] = useState<HouHubMcpServiceStatus | null>(null)
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const next = await getHouHubMcpServiceStatus()
  ...
  const handleToggle = async (key: string, next: boolean) => {
    ...
    setActionError(null)
    try {
      await setHouHubMcpToolGroup(key, next)
