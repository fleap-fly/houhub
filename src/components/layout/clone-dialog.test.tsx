...
describe("CloneDialog clone path preview", () => {
  ...
  it("joins a Windows directory with a backslash", () => {
    renderDialog()
    fillForm("https://github.com/fleap-fly/houhub", "C:\\work")
    // Previously rendered `C:\work/HouHub` — a native prefix with a stray
    // forward slash bolted on, which is what the bug report showed.
    expect(previewedPath()).toBe("C:\\work\\houhub")
  })

  it("joins a POSIX directory with a forward slash", () => {
    renderDialog()
    fillForm("https://github.com/fleap-fly/houhub", "/home/me/work")
    expect(previewedPath()).toBe("/home/me/work/houhub")
  })

  it("does not double a separator the directory already ends with", () => {
    renderDialog()
    fillForm("https://github.com/fleap-fly/houhub.git", "C:\\work\\")
    expect(previewedPath()).toBe("C:\\work\\houhub")
  })
  ...
  it("clones into the same path it previewed", async () => {
    ...
    expect(api.cloneRepository).toHaveBeenCalledWith(
      "https://github.com/fleap-fly/houhub",
      previewed,
      undefined
    )
  })
