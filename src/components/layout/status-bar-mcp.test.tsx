...
import type { HouHubMcpServiceStatus } from "@/lib/api"

const getHouHubMcpServiceStatus = vi.fn<() => Promise<HouHubMcpServiceStatus>>()
const startHouHubMcpService = vi.fn<() => Promise<void>>()
const setHouHubMcpToolGroup = vi.fn<(k: string, e: boolean) => Promise<void>>()
const openSettingsWindow = vi.fn()

vi.mock("@/lib/api", () => ({
  getHouHubMcpServiceStatus: () => getHouHubMcpServiceStatus(),
  startHouHubMcpService: () => startHouHubMcpService(),
  setHouHubMcpToolGroup: (k: string, e: boolean) => setHouHubMcpToolGroup(k, e),
...
function makeStatus(
  overrides: Partial<HouHubMcpServiceStatus> = {}
): HouHubMcpServiceStatus {
  return {
    state: "running",
    listening: true,
    socket_path: "/tmp/houhub-delegation-4242.sock",
    binary_path: "/Applications/houhub.app/Contents/MacOS/houhub-mcp",
    tool_groups: [
...
async function mount() {
  ...
  await waitFor(() => expect(getHouHubMcpServiceStatus).toHaveBeenCalled())
}
...
beforeEach(() => {
  getHouHubMcpServiceStatus.mockReset()
  startHouHubMcpService.mockReset()
  setHouHubMcpToolGroup.mockReset()
  openSettingsWindow.mockReset()
  getHouHubMcpServiceStatus.mockResolvedValue(makeStatus())
  startHouHubMcpService.mockResolvedValue(undefined)
  setHouHubMcpToolGroup.mockResolvedValue(undefined)
...
describe("StatusBarMcp", () => {
  ...
  it("keeps the socket and binary paths on the state badge", async () => {
    ...
    const glyph = screen.getByTitle(/houhub-delegation-4242\.sock/)
    expect(glyph).toHaveAttribute(
      "title",
      expect.stringContaining(
        "/Applications/houhub.app/Contents/MacOS/houhub-mcp"
      )
    )
  })
  ...
  it("reverts a switch whose write was refused", async () => {
    setHouHubMcpToolGroup.mockRejectedValue(new Error("database is locked"))
    await mount()
    await openPopover()
  ...
  it("offers the start button when stopped and refetches after starting", async () => {
    ...
    await waitFor(() => expect(startHouHubMcpService).toHaveBeenCalledTimes(1))
    // Status is re-read after the start so the popover reflects the new truth
    // rather than the one that motivated the click.
    await waitFor(() =>
      expect(getHouHubMcpServiceStatus.mock.calls.length).toBeGreaterThan(1)
    )
    await waitFor(() =>
      expect(
  ...
  it("does not offer a start button for a missing companion binary", async () => {
    ...
    expect(
      screen.getByText("The houhub-mcp binary is not on disk.")
    ).toBeInTheDocument()
    expect(
      screen.queryByRole("button", { name: /Start service/ })
    ).not.toBeInTheDocument()
  ...
  it("surfaces a failed start without claiming the service is broken", async () => {
    ...
    startHouHubMcpService.mockRejectedValue(new Error("address already in use"))
    await mount()
    await openPopover()
  ...
  it("falls back to an unknown state when the status call fails", async () => {
    getHouHubMcpServiceStatus.mockRejectedValue(new Error("transport offline"))
    await mount()
    await openPopover2()
