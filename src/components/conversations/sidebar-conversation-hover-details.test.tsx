...
function folder(overrides: Partial<FolderDetail> = {}): FolderDetail {
  return {
    id: 1,
    name: "houhub",
    path: "/Users/dev/projects/houhub",
    git_branch: null,
...
describe("SidebarConversationHoverDetails", () => {
  ...
  it("names the folder and shows its absolute path", () => {
    ...
    expect(fieldValue("Folder")).toBe("houhub")
    expect(fieldValue("Path")).toBe("/Users/dev/projects/houhub")
  })
  ...
  it("renders an aliased folder as `alias [ name ]`", () => {
    ...
    expect(fieldValue("Folder")).toBe("My Project [ houhub ]")
  })
  ...
  it("shows the original path when the source worktree was removed", () => {
    seed()
    renderBubble(conv({ origin_cwd: "/Users/dev/projects/houhub-feature-x" }))

    expect(fieldValue("Source worktree removed")).toBe(
      "/Users/dev/projects/houhub-feature-x"
    )
  })
