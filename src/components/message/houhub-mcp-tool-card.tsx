...
/**
 * Inline card for the houhub-mcp *workbench* companion tools — `get_session_info`,
 ...
 * language as `DelegationStatusCard` / `DelegationStatusRow` so every houhub-mcp
 ...
 *
 * Parsing lives in `@/lib/houhub-mcp-tool` — it handles the per-host argument
...
import {
  parseHouhubMcpToolCall,
  type HouhubMcpWorkbenchTool,
} from "@/lib/houhub-mcp-tool"
...
const ICONS: Record<HouhubMcpWorkbenchTool, typeof BookOpen> = {
...
export function HouhubMcpToolCard({
  tool,
  input,
  output,
  errorText,
  ...
  const t = useTranslations("Folder.chat.houhubMcpTool") as unknown as (
    key: string,
  ...
  const model = useMemo(
    () => parseHouhubMcpToolCall({ tool, input, output, errorText, state }),
  ...
  return (
    <div
      data-testid="houhub-mcp-tool-card"
      data-tool={tool}
      className={cn(
