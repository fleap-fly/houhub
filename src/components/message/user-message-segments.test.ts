...
describe("parseUserMessageSegments", () => {
  ...
  describe("reference links → badges", () => {
    ...
    it("parses an agent link and strips the leading @", () => {
      const attrs = onlyReference("[@Codex](houhub://agent/codex)")
    ...
    it("parses a session link", () => {
      const attrs = onlyReference("re [My chat](houhub://session/42)")
      expect(attrs.refType).toBe("session")
      expect(attrs.uri).toBe("houhub://session/42")
    })
    ...
    const cases: Array<[string, ReferenceAttrs]> = [
      ...
      [
        ...
        ref({
          ...
          id: "a1b2c3ddeadbeef",
          label: "a1b2c3d",
          uri: "houhub://commit/%2Frepo@a1b2c3ddeadbeef",
