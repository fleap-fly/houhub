  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-1 py-1">
      ...
      <div
        ...
        // `houhub-composer-chrome` paints the text I-beam over the dead space.
        onMouseDown={(e) => {
          if (!isComposerChromeClick(e.target)) return
          e.preventDefault()
          editorRef.current?.focusAtCoords(e.clientX, e.clientY)
        }}
        className="houhub-composer-chrome relative rounded-xl border border-input bg-background transition-colors focus-within:border-ring focus-within:ring-[3px] focus-within:ring-inset focus-within:ring-ring/50"
      >
