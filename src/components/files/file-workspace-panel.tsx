      ...
      <Streamdown
        ...
        components={{
          ...
          a: ({ node, href, children, ...aProps }) => {
            ...
            return externalHref ? (
              <BrowserLink {...aProps} href={externalHref}>
                {children}
              </BrowserLink>
            ) : (
              <span {...aProps}>{children}</span>
            )
          },
        }}
      >
...
function createAddToChatPill(
  ...
  dom.className =
    "houhub-add-to-chat-pill rounded-md border border-border bg-popover px-2 py-0.5 text-xs font-medium text-popover-foreground shadow-md cursor-pointer select-none hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
  ...
  dom.innerHTML =
    ...
    '<span class="houhub-add-to-chat-label"></span>' +
    "</span>"
  const labelSpan = dom.querySelector(".houhub-add-to-chat-label")
...
export function FileWorkspacePanel() {
      ...
      .map((lineNumber) => ({
        ...
        options: {
          ...
          linesDecorationsClassName:
            "houhub-dirty-diff-glyph houhub-dirty-diff-added",
        },
      }))
    const modifiedDecorations = modified
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: true,
          linesDecorationsClassName:
            "houhub-dirty-diff-glyph houhub-dirty-diff-modified",
        },
      }))
    const deletedDecorations = deleted
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: false,
          linesDecorationsClassName:
            "houhub-dirty-diff-glyph houhub-dirty-diff-deleted",
        },
      }))
    const decorations = [
      ...addedDecorations,
  ...
  const handleEditorMount: OnMount = useCallback(
    (editorInstance, monaco) => {
      ...
      // floating pill. All three are gated by the `houhubSelectionAttachable`
