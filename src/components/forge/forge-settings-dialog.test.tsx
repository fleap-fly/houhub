...
beforeEach(() => {
  ...
  useAppWorkspaceStore.setState({
    folders: [
      { id: 4, name: "houhub", parent_id: null, kind: "regular" },
    ] as never,
  })
...
describe("ForgeSettingsDialog folder scope", () => {
  ...
  it("opens a folder that has nothing of its own on the global row", async () => {
    ...
    expect(
      screen.getByText("How issues and changes in houhub are handled.")
    ).toBeInTheDocument()
    expect(
  ...
  it("re-seeds when the scope picker moves to the global row", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "houhub" }))
