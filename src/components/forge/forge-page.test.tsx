...
const REMOTE: ForgeRemote = {
  ...
  owner_repo: "fleap-fly/houhub",
  remote_url: "https://github.com/fleap-fly/houhub.git",
  provider: "github",
  supported: true,
}
...
function issue(number: number, title: string): ForgeIssueRow {
  return {
    ...
    updated_at: null,
    html_url: `https://github.com/fleap-fly/houhub/issues/${number}`,
    is_pr: false,
    comments: 0,
  }
}
...
beforeEach(() => {
  ...
  useAppWorkspaceStore.setState({
    folders: [
      { id: 1, name: "houhub", parent_id: null, kind: "regular" },
    ] as never,
  })
...
describe("ForgePage list failures", () => {
  ...
  /**
   * Issue #611: a self-hosted GitLab that houhub had classified as a GitHub
  ...
  it("silently retries a wrong-forge failure instead of showing it", async () => {
    ...
    expect(
      screen.queryByText(/not the forge houhub had it classified as/)
    ).not.toBeInTheDocument()
  ...
  it("shows a wrong-forge failure that survives the retry", async () => {
    ...
    expect(
      await screen.findByText(
        "git.corp.com is a GitLab, not the forge houhub had it classified as. The detection has been corrected — reload to retry."
      )
    ).toBeInTheDocument()
  })
...
/**
 * A repository houhub cannot read at all — Bitbucket, Gitee, somebody's Gitea.
 *
...
describe("repoWebUrl", () => {
  ...
  it("falls back to the canonical page for an unopenable remote", () => {
    ...
    for (const url of ["git@github.com:o/r.git", "ssh://git@github.com/o/r"]) {
      expect(
        repoWebUrl(at({ remote_url: url, owner_repo: "fleap-fly/houhub" }))
      ).toBe("https://github.com/fleap-fly/houhub")
    }
  })
})
...
describe("ForgePage settings button", () => {
  ...
  it("opens the panel's preferences from the window chrome", async () => {
    ...
    expect(
      await screen.findByText("How issues and changes in houhub are handled.")
    ).toBeInTheDocument()
  })
...
describe("ForgePage header", () => {
  ...
  it("names the repository without the host, and links to it", async () => {
    ...
    const repo = await screen.findByRole("button", {
      name: /fleap-fly\/houhub/,
    })
    expect(repo).toHaveAccessibleName("fleap-fly/houhub")
    expect(repo).toHaveAttribute(
      "title",
      "Open the repository · github.com/fleap-fly/houhub"
    )
    expect(screen.queryByText("github.com/fleap-fly/houhub")).toBeNull()
  })
  ...
  it("keeps the picker when the folder has no remote", async () => {
    ...
    mount()
    expect(
      await screen.findByRole("button", { name: /houhub/, expanded: false })
    ).toBeInTheDocument()
    expect(
      screen.queryByRole("button", { name: /fleap-fly\/houhub/ })
    ).toBeNull()
  })
...
describe("ForgePage loading coordination", () => {
  ...
  it("drops the previous repository's rows when the folder changes", async () => {
    ...
    const settle = deferredListing()
    mount()
    await settle(listOf([issue(1, "row from houhub")]))
    await screen.findByText("row from houhub")

    await user.click(
      screen.getByRole("button", { name: /houhub/, expanded: false })
    )
    await user.click(
      await screen.findByRole("option", { name: /other-project/ })
    )

    expect(screen.queryByText("row from houhub")).toBeNull()
    expect(screen.getByTestId("forge-list-skeleton")).toBeInTheDocument()
  })
...
describe("ForgePage detail panel", () => {
  ...
  it("closes when the folder changes", async () => {
    ...
    vi.mocked(forgeListIssues).mockResolvedValue(
      listOf([withBody(1, "row from houhub", "body")])
    )
    mount()
    await openDetail("row from houhub")
  ...
  it("closes when the last folder leaves the workspace", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "row from houhub" })
    ).not.toBeInTheDocument()
  })
