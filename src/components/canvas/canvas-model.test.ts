...
describe("deriveFlowGraph — the folder a card names in its footer", () => {
  ...
  it("names the conversation's own folder on a pinned card", () => {
    ...
    const { nodes } = deriveFlowGraph({
      ...
      allFolders: [folder(3, { name: "houhub" })],
      ...NO_DRAG,
    })
    expect((nodes[0].data as ConversationCardData).folderName).toBe("houhub")
  })

  it("names the parent repo for a conversation living in a worktree", () => {
    // The worktree's own directory name is `houhub-fix-abc`; what pairs with the
    ...
    const { nodes } = deriveFlowGraph({
      ...
      allFolders: [
        folder(3, { name: "houhub" }),
        folder(4, { name: "houhub-fix-abc", parent_id: 3 }),
      ],
      ...NO_DRAG,
    })
    const member = nodes.find((n) => n.type === "conversationCard")!
    expect((member.data as ConversationCardData).folderName).toBe("houhub")
  })
