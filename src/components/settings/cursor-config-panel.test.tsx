...
describe("buildCursorEnv", () => {
  it("API-key mode writes the Cursor key + model and scrubs the IDE-only base URL", () => {
    ...
    // Agent CLI does not consume the IDE's OpenAI base URL setting.
...
describe("cursorLoginRunsOnHouHubHost", () => {
  it("is false only for a window whose backend is this machine", () => {
    expect(cursorLoginRunsOnHouHubHost(true, false)).toBe(false)
    // A web page and a remote-desktop window both talk to a houhub server.
    expect(cursorLoginRunsOnHouHubHost(false, false)).toBe(true)
    expect(cursorLoginRunsOnHouHubHost(true, true)).toBe(true)
  })
})
...
describe("cursorLoginIsHeadless", () => {
  ...
  it("treats a web page and a remote-desktop window as another host", () => {
    // Both talk to a houhub server: `login` would run there, with no display.
...
describe("CursorConfigPanel", () => {
  ...
  it("subscription mode shows the runnable login command and saves login-only env", async () => {
    vi.mocked(acpCursorAuthStatus).mockResolvedValue({
      ...
      error: null,
      binary_path:
        "/Users/x/Library/Caches/com.houflow.houhub/acp-binaries/cursor/dist-package/cursor-agent",
    })
    ...
    await screen.findByText(
      "/Users/x/Library/Caches/com.houflow.houhub/acp-binaries/cursor/dist-package/cursor-agent login"
    )
  ...
  it("offers a command a headless houhub server can actually run", async () => {
    // The reported deployment: houhub-server on a Linux box, panel open in a
  ...
  it("still says where to run it when the host is Windows", async () => {
    // A Windows houhub host cannot take the POSIX `VAR=1 cmd` prefix — but it
