...
describe("buildVersionCheck", () => {
  ...
  // HouHub cannot know whether npm has something newer, because nothing polls
  // in the background.
  it("keeps Upgrade available for an installed latest-channel npx agent", () => {
    const check = buildVersionCheck(
      makeAgent({
        ...
        env: { HOUHUB_ADAPTER_CHANNEL: "latest" },
      })
    )
    expect(check?.status).toBe("pass")
...
describe("patchCodexConfigTomlText — houhub's requires_openai_auth default", () => {
  ...
  function authFlagOf(configTomlText: string): boolean | undefined {
    ...
    const value = parsed.model_providers?.houhub?.requires_openai_auth
  ...
  const ENTRY_POINTS: Array<{
    ...
    { label: "model provider", patch: { modelProvider: "houhub" } },
  ]
  ...
  // append a `[model_providers.houhub]` section that redefines the same table.
  ...
  it("reads the header out of a root-level dotted key too", () => {
    const toml = [
      'model_provider = "houhub"',
      'model_providers.houhub.base_url = "https://old.example/v1"',
      'model_providers.houhub.http_headers."x-openai-actor-authorization" = "local-image-extension"',
    ].join("\n")
...
describe("host-tools toggle — hand the fs/terminal channels back to the agent", () => {
  const KEY = "HOUHUB_ACP_HOST_TOOLS"
  ...
  it("round-trips on and back off", () => {
    ...
    // would let a process-wide `HOUHUB_ACP_HOST_TOOLS=agent` keep winning, so
  ...
  it("overrides an inherited value in both directions", () => {
    // The backend resolves env_json first, then houhub's process env. Whatever
...
describe("adapter-channel control — opt into the latest adapter release", () => {
  const KEY = "HOUHUB_ADAPTER_CHANNEL"
