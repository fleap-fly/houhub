#!/usr/bin/env bash
#
# Houhub Server installer
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/fleap-fly/houhub/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/fleap-fly/houhub/main/install.sh | bash -s -- --version v0.5.0
#

set -euo pipefail

REPO="fleap-fly/houhub"
INSTALL_DIR="${HOUHUB_INSTALL_DIR:-/usr/local/bin}"
WEB_DIR="${HOUHUB_WEB_DIR:-/usr/local/share/houhub/web}"
VERSION=""
# Stale houhub-server / houhub-mcp binaries elsewhere in PATH are removed by
# default so the user's `houhub-server` command always runs the freshly
# installed binary AND the runtime locates the matching companion via the
# exe-sibling lookup. Set HOUHUB_NO_CLEANUP=1 (or pass --no-cleanup) to
# disable.
CLEANUP_CONFLICTS=1
if [ "${HOUHUB_NO_CLEANUP:-0}" = "1" ]; then
  CLEANUP_CONFLICTS=0
fi

# Names of binaries this installer manages. `houhub-server` is the user-facing
# entry point; `houhub-mcp` is the stdio MCP companion that the server's ACP
# layer spawns per session for delegation. Both must live in the same
# directory — `locate_houhub_mcp_binary()` in src-tauri/src/acp/connection.rs
# resolves the companion as a sibling of the running server executable.
MANAGED_BINS=(houhub-server houhub-mcp)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help)
      ...
      echo "  --no-cleanup  Keep stale houhub-server binaries found elsewhere in PATH"
      echo "                (default: remove them so the new install is what runs)"
      echo ""
      echo "Environment:"
      echo "  HOUHUB_INSTALL_DIR  Same as --dir"
      echo "  HOUHUB_NO_CLEANUP   Set to 1 to behave like --no-cleanup"
      exit 0
      ;;
...
ARTIFACT="houhub-server-${PLATFORM}-${ARCH_SUFFIX}"
...
# Read the version of a houhub-server binary (with a 3s timeout for old binaries
...
# be tested for a not-yet-created path (e.g. /usr/local/share/houhub/web, whose
# parent /usr/local/share/houhub also doesn't exist on a fresh install).
...
# ── Scan PATH for houhub-server binaries that shadow the target install ──
#
# A binary "shadows" the install only if it appears in PATH BEFORE the
# destination directory: that's the binary `command -v houhub-server` would
...
DEST_BIN="${INSTALL_DIR}/houhub-server"
DEST_BIN_REAL="$(canon_path "$DEST_BIN")"
INSTALL_DIR_REAL="$(canon_path "$INSTALL_DIR")"

# Scan PATH for both managed binaries — a stale `houhub-mcp` in an earlier
# PATH entry would be picked by the runtime's `which` fallback once
# `houhub-server` was upgraded out from under it, breaking delegation in
...
# the new binary just won't be reachable as `houhub-server`. Drop any collected
# entries; the post-install check will tell the user to fix PATH instead.
if [ "$DEST_IN_PATH" -eq 0 ]; then
  PATH_CONFLICTS=()
fi

# What does `houhub-server` actually resolve to right now in PATH?
ACTIVE_BIN=""
if command -v houhub-server >/dev/null 2>&1; then
  ACTIVE_BIN="$(command -v houhub-server)"
fi
...
if [ -n "$CURRENT_VERSION" ] && [ "$CURRENT_VERSION" = "$TARGET_VER" ] \
  ...
  echo "houhub-server is already at version ${TARGET_VER} with web assets in place, nothing to do."
  exit 0
fi

if [ -n "$CURRENT_VERSION" ] && [ "$CURRENT_VERSION" = "$TARGET_VER" ]; then
  echo "houhub-server is already at ${TARGET_VER}; reinstalling to repair the existing install..."
elif [ -n "$CURRENT_VERSION" ]; then
  echo "Upgrading houhub-server: ${CURRENT_VERSION} -> ${TARGET_VER}..."
else
  echo "Installing houhub-server ${VERSION} (${PLATFORM}/${ARCH_SUFFIX})..."
fi

# ── Warn about houhub-server binaries shadowing the target install ──

if [ "${#PATH_CONFLICTS[@]}" -gt 0 ]; then
  echo ""
  echo "Found other houhub-server binaries in PATH that may shadow ${DEST_BIN}:"
  ...
  if [ "$CLEANUP_CONFLICTS" = "1" ]; then
    ...
    echo "typing 'houhub-server' runs the new install at ${DEST_BIN}."
  fi
  echo ""
fi
...
#
# Stop houhub-mcp too: on Unix `cp` over a running binary succeeds (the
...
RESTARTED_PIDS=""
if pgrep -x houhub-server >/dev/null 2>&1; then
  echo "Stopping running houhub-server process(es)..."
  RESTARTED_PIDS=$(pgrep -x houhub-server || true)
  if kill $RESTARTED_PIDS 2>/dev/null; then
    ...
    if pgrep -x houhub-server >/dev/null 2>&1; then
      echo "Force stopping houhub-server..."
      kill -9 $RESTARTED_PIDS 2>/dev/null || true
      sleep 1
    fi
  fi
  echo "houhub-server stopped."
fi

if pgrep -x houhub-mcp >/dev/null 2>&1; then
  echo "Stopping running houhub-mcp companion process(es)..."
  MCP_PIDS=$(pgrep -x houhub-mcp || true)
  if [ -n "$MCP_PIDS" ]; then
    ...
    if pgrep -x houhub-mcp >/dev/null 2>&1; then
      kill -9 $(pgrep -x houhub-mcp) 2>/dev/null || true
    fi
  fi
fi
...
if ! resolve_priv "$INSTALL_DIR"; then
  ...
  echo "       Re-run as root, install sudo, or set HOUHUB_INSTALL_DIR/HOUHUB_WEB_DIR to writable"
  echo "       paths (e.g. \$HOME/.local/bin and \$HOME/.local/share/houhub/web)."
  exit 1
fi
if [ -n "$PRIV" ]; then
...
if [ -d "$WEB_SRC" ]; then
  ...
  if ! resolve_priv "$WEB_DIR"; then
    ...
    echo "       Re-run as root, install sudo, or set HOUHUB_WEB_DIR to a writable path."
    exit 1
  fi
...
if [ "${#PATH_CONFLICTS[@]}" -gt 0 ] && [ "$CLEANUP_CONFLICTS" = "1" ]; then
  echo ""
  echo "Removing stale houhub-server binaries..."
  for _c in "${PATH_CONFLICTS[@]}"; do
    _parent="$(dirname "$_c")"
    _rm_ok=0
    ...
    if [ "$_rm_ok" -eq 1 ]; then
      ...
      echo "  failed to remove $_c (remove it manually so 'houhub-server' resolves to the new install)"
      EXIT_STATUS=1
    fi
  done
fi
...
if [ -n "$RESTARTED_PIDS" ]; then
  echo ""
  echo "Note: houhub-server was stopped for the upgrade."
  echo "Please restart it manually to ensure your environment variables (HOUHUB_PORT, HOUHUB_TOKEN, etc.) are preserved:"
  echo "  HOUHUB_STATIC_DIR=${WEB_DIR} houhub-server"
fi

# ── Done ──

echo ""
echo "houhub-server installed to ${INSTALL_DIR}/houhub-server"
echo "houhub-mcp    installed to ${INSTALL_DIR}/houhub-mcp"
INSTALLED_VER=$("${INSTALL_DIR}/houhub-server" --version 2>/dev/null || echo "${TARGET_VER}")
echo "Version: ${INSTALLED_VER}"

# Final smoke: houhub-mcp must exist next to houhub-server so the runtime's
# `locate_houhub_mcp_binary()` exe-sibling lookup hits. A failure here means
...
if [ ! -x "${INSTALL_DIR}/houhub-mcp" ]; then
  echo ""
  echo "Error: ${INSTALL_DIR}/houhub-mcp missing or not executable after install."
  echo "       Delegation (sub-agent tooling) will not work. Re-run the installer."
  EXIT_STATUS=1
fi

# Verify the user's `houhub-server` command actually resolves to the new binary.
ACTIVE_BIN_AFTER=""
if command -v houhub-server >/dev/null 2>&1; then
  ACTIVE_BIN_AFTER="$(command -v houhub-server)"
fi
ACTIVE_BIN_AFTER_REAL="$(canon_path "$ACTIVE_BIN_AFTER")"

if [ -z "$ACTIVE_BIN_AFTER" ]; then
  echo ""
  echo "Note: ${INSTALL_DIR} is not on your PATH. Add it so 'houhub-server' resolves directly:"
  echo "  export PATH=\"${INSTALL_DIR}:\$PATH\""
  EXIT_STATUS=1
elif [ "$ACTIVE_BIN_AFTER_REAL" != "$DEST_BIN_REAL" ]; then
  echo ""
  echo "Warning: typing 'houhub-server' still runs ${ACTIVE_BIN_AFTER}, not ${DEST_BIN}."
  ...
  echo ""
  echo "Tip: if you ran houhub-server earlier in this shell, run 'hash -r' (bash/zsh) to clear the path cache."
fi
...
echo "  HOUHUB_PORT=3080 HOUHUB_TOKEN=your-secret HOUHUB_STATIC_DIR=${WEB_DIR} houhub-server"
echo ""
echo "The auth token is printed to stderr on startup if not set via HOUHUB_TOKEN."
