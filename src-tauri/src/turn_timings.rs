//! Per-turn wall-clock observations recorded by HouHub's own ACP connection
...
//! rendered nothing.
//!
//! Every ACP session of such an agent runs THROUGH HouHub, so the connection
...
//! store yields no clock. The data is honest — HouHub's own measurement, not a
//! fabrication from unrelated fields.
//!
//! ## File layout
//!
//! `<paths::houhub_turn_timings_root()>/<agent>/<session-id>.jsonl`, one JSON
//! object per line:
//!
//! ```jsonc
...
//!
//! Sessions never run through this HouHub install (copied from another
...
pub fn read_turn_timings(agent: &str, session_id: &str) -> Vec<TurnTiming> {
    read_turn_timings_in(&crate::paths::houhub_turn_timings_root(), agent, session_id)
}
