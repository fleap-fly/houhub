//! Web-mode port bridge: shows a dev server that runs on the houhub host
...
//! listens on extra ports next to houhub's own and forwards each of them to one
...
//!
//! ## Authentication: a same-site cookie, not houhub's token
//!
//! An iframe navigation cannot carry a bearer header, and the page's own
//! requests must not carry houhub's token either. The workbench asks the API
...
//! cookie is sent to houhub's own port too, where nothing reads cookies; the
...
//! `HOUHUB_BRIDGE_HOST_PATTERN` names the targets by hostname instead —
//! `3000.houhub.example.com` for port 3000 — and then the bridge binds nothing
//! of its own: those requests arrive on houhub's own listener, are recognised
//! by their `Host` before anything else looks at them, and are answered here
//! and nowhere else in houhub. A deployment publishes one port, and a reverse
...
//! `3000.houhub.example.com` is never sent to `3001.houhub.example.com` at all,
...
//!
//! Sharing houhub's listener means houhub has to be sure which requests are
...
//! a hostname houhub actually handed out for it (`Listener::hosts`), read
...
//! the target, and houhub's own pages must never be served through it.
...
/// Entry URL prefix on a bridge listener: `/__houhub_bridge/enter/{cap}?to=/path`.
pub const ENTER_PREFIX: &str = "/__houhub_bridge/enter/";
...
pub const PING_PATH: &str = "/__houhub_bridge/ping";
const COOKIE_PREFIX: &str = "houhub-bridge-";
/// The workbench's own cookies (locale preferences) live on the same host
/// and are not the dev server's business either.
const WORKBENCH_COOKIE_PREFIX: &str = "houhub.";
/// Ports above houhub's own that the bridge takes when `HOUHUB_BRIDGE_PORTS`
/// is not set.
...
pub struct BridgeConfig {
    /// Address the listeners bind to: the same one houhub's API listener uses.
    ...
    /// Name the targets by hostname on houhub's own listener instead of giving
    /// each one a port of its own.
    pub host_pattern: Option<HostPattern>,
    /// Ports the bridge refuses to forward to: houhub's own listener.
    pub reserved: Vec<u16>,
}

impl BridgeConfig {
    /// Read `HOUHUB_BRIDGE_PORTS` / `HOUHUB_BRIDGE_HOST_PATTERN` /
    /// `HOUHUB_BRIDGE_PUBLIC_HOST`. `None` when the bridge is switched off
    /// (`HOUHUB_BRIDGE_PORTS=off`).
    pub fn from_env(bind_host: &str, houhub_port: u16) -> Option<Self> {
        let ports = std::env::var("HOUHUB_BRIDGE_PORTS").ok();
        let pattern = std::env::var("HOUHUB_BRIDGE_HOST_PATTERN").ok();
        let public_host = std::env::var("HOUHUB_BRIDGE_PUBLIC_HOST").ok();
        Self::from_values(
            bind_host,
            houhub_port,
            ports.as_deref(),
    ...
    /// operator asked for that (`HOUHUB_BRIDGE_PORTS=off`), or wrote something
    ...
    pub fn from_values(
        ...
        let ports = match ports_raw {
            Some(raw) => parse_ports(raw, houhub_port)?,
            None => default_ports(houhub_port),
        };
        ...
        Some(Self {
            ...
            public_host,
            host_pattern,
            reserved: vec![houhub_port],
        })
    }
}

/// How the browser addresses one target port when the bridge answers on
/// houhub's own listener instead of binding a port per target.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HostPattern {
    /// `auto`: the port is one label in front of the host the workbench
    /// itself was reached at — `3000.houhub.example.com` for a workbench on
    /// `houhub.example.com`, `3000.localhost` for one on `localhost`.
    Subdomain,
    /// A template naming `{port}`: `{port}.preview.example.com`,
    /// `p{port}-houhub.example.com`.
...
/// The ten ports after houhub's own, stopping at the end of the port space.
pub fn default_ports(houhub_port: u16) -> Vec<u16> {
    (1..=DEFAULT_POOL_SIZE)
        .filter_map(|offset| houhub_port.checked_add(offset))
        .collect()
}

/// `3081-3090`, `3081,3082,3090`, a mix of both, `auto` (any free port), or
/// `off` / `none` / `disabled` / empty (no bridge; returns `None`). HouHub's
...
pub fn parse_ports(raw: &str, houhub_port: u16) -> Option<Vec<u16>> {
    let raw = raw.trim();
    ...
    for item in raw.split(',') {
        ...
        for port in range.0..=range.1 {
            if port != houhub_port && !ports.contains(&port) {
                ports.push(port);
            }
        }
    }
...
pub enum BridgeError {
    ...
    Disabled,
    #[error("port {0} is houhub's own listener")]
    Reserved(u16),
...
struct Listener {
    ...
    /// addressed by hostname and its requests arrive on houhub's listener.
    bridge_port: Option<u16>,
    /// Bridge hostnames handed out for this target — one per host the
    /// workbench has been reached at. A request on houhub's listener is this
    ...
    /// shape is not a promise that houhub ever handed the name out.
    hosts: Mutex<HashSet<String>>,
    /// Believe `X-Forwarded-Host` / `X-Forwarded-Proto`: only when the
    /// operator declared a proxy in front (`HOUHUB_BRIDGE_PUBLIC_HOST`); a
...
struct Bridge {
    ...
    /// worker behind, and houhub's own pages served under that name would be
    ...
    /// asking for it means holding houhub's token.
...
/// Whether any request on houhub's own listener could be the bridge's. Read
/// on every request houhub serves, so it stays out of the config mutex.
...
/// reverse proxy that replaced it is declared by `HOUHUB_BRIDGE_PUBLIC_HOST`.
fn open_by_host(
    config: &BridgeConfig,
    pattern: &HostPattern,
    target_port: u16,
    ...
    // record outlives the target, so the name never becomes houhub's again.
...
fn serve(socket: tokio::net::TcpListener, listener: Arc<Listener>) {
    ...
    let router = Router::new()
        ...
        .route("/__houhub_bridge/enter/{cap}", get(enter))
...
// ─── Routing on houhub's own listener ───────────────────────────────────

/// Answer here, and nowhere else in houhub, when the request was addressed to
/// a bridge hostname. The outermost layer of houhub's router: a bridged
...
/// houhub's own pages and API are never reached under that hostname.
///
/// Off unless `HOUHUB_BRIDGE_HOST_PATTERN` is set, and then a hostname the
/// pattern does not describe passes straight through to houhub.
pub async fn route_by_host(request: Request, next: Next) -> Response {
    ...
    if live.is_some() {
        // A name houhub handed out for a live target is that target's,
...
/// houhub's own pages served under that name would be served through it.
...
/// bridge every name there, so houhub has no business answering. `auto` gets
/// no such benefit of the doubt — it describes `<port>.<anything>`, a shape
/// that covers names houhub was never asked to take, a workbench on a
...
/// houhub's pages under the bridge's own origin.
...
/// request that arrived on houhub's listener instead of one of our own.
...
fn bounce_page(to: &str) -> String {
    ...
    format!(
        "<!doctype html><html><head><meta charset=\"utf-8\">\
         <meta http-equiv=\"refresh\" content=\"0;url={attribute}\"><title>houhub</title></head>\
...
async fn forward_request(listener: Arc<Listener>, request: Request) -> Response {
    ...
    if parts.uri.path().starts_with("/__houhub_bridge/") {
...
fn forbidden_page() -> Response {
    html_page(
        ...
        "Reopen the page from houhub to start a new preview session.",
    )
}
...
fn cross_origin_page() -> Response {
    html_page(
        ...
        "A dev server shown through houhub only answers its own page. Open the address from houhub to view it.",
    )
}
...
fn bad_gateway_page(target: u16, err: &dyn std::fmt::Display) -> Response {
    ...
    html_page(
        ...
        &format!("houhub cannot reach port {target} on its host"),
...
mod tests {
    ...
    fn port_pools_parse_ranges_lists_and_switches() {
        ...
        // Only houhub's own port: nothing left.
        assert_eq!(parse_ports("3080", 3080), None);
    }

    #[test]
    fn default_pool_is_the_ten_ports_above_houhub() {
    ...
    fn cookies_are_read_and_filtered() {
        ...
        headers.append(
            header::COOKIE,
            HeaderValue::from_static("a=1; houhub-bridge-3081=cap-one; b=2"),
        );
        headers.append(
            header::COOKIE,
            HeaderValue::from_static("houhub-bridge-3082=cap-two; houhub.locale=zh-CN"),
        );
        assert_eq!(cookie_value(&headers, "houhub-bridge-3081").as_deref(), Some("cap-one"));
        assert_eq!(cookie_value(&headers, "houhub-bridge-3082").as_deref(), Some("cap-two"));
        assert_eq!(cookie_value(&headers, "houhub-bridge-3083"), None);
    ...
    fn origin_and_referer_point_at_the_target() {
        let mut headers = HeaderMap::new();
        headers.insert(header::ORIGIN, HeaderValue::from_static("http://houhub.example:3081"));
        headers.insert(
            header::REFERER,
            HeaderValue::from_static("http://houhub.example:3081/app/page?tab=2"),
        );
        headers.insert(header::COOKIE, HeaderValue::from_static("houhub-bridge-3081=c; sid=9"));
    ...
    fn status_and_wire_names() {
        let json = serde_json::to_value(BridgeGrant {
            ...
            bridge_host: None,
            entry_path: "/__houhub_bridge/enter/abc".into(),
            public_host: None,
        })
        .unwrap();
        assert_eq!(json["bridgePort"], 3081);
        assert_eq!(json["entryPath"], "/__houhub_bridge/enter/abc");
        ...
        let json = serde_json::to_value(BridgeGrant {
            target_port: 3000,
            bridge_port: None,
            bridge_host: Some("3000.houhub.example".into()),
            entry_path: "/__houhub_bridge/enter/abc".into(),
            public_host: None,
        })
        .unwrap();
        assert!(json["bridgePort"].is_null());
        assert_eq!(json["bridgeHost"], "3000.houhub.example");

        let json = serde_json::to_value(BridgeStatus {
            enabled: true,
            ports: Vec::new(),
            public_host: None,
            host_pattern: Some("{port}.houhub.example".into()),
        })
        .unwrap();
        assert_eq!(json["hostPattern"], "{port}.houhub.example");
    }
    ...
    fn host_patterns_parse_or_are_refused() {
        ...
        assert_eq!(
            HostPattern::parse("P{port}-houhub.example.com"),
            Some(HostPattern::Template {
                prefix: "p".into(),
                suffix: "-houhub.example.com".into()
            })
        );
        for raw in [
    ...
    fn a_hostname_names_one_target_port() {
        ...
        let template = HostPattern::parse("p{port}-houhub.example.com").unwrap();

        assert_eq!(auto.render(3000, "houhub.example.com").as_deref(), Some("3000.houhub.example.com"));
        assert_eq!(auto.render(3000, "LOCALHOST").as_deref(), Some("3000.localhost"));
        assert_eq!(auto.target_of("3000.houhub.example.com"), Some(3000));
        assert_eq!(auto.target_of("3000.LOCALHOST"), Some(3000));
        assert_eq!(template.render(5173, "ignored").as_deref(), Some("p5173-houhub.example.com"));
        assert_eq!(template.target_of("p5173-houhub.example.com"), Some(5173));
        ...
        assert_eq!(auto.target_of("houhub.example.com"), None);
        assert_eq!(auto.target_of("localhost"), None);
        assert_eq!(template.target_of("houhub.example.com"), None);
        assert_eq!(template.target_of("p-houhub.example.com"), None);
        // Neither is a near miss, in either direction.
        assert_eq!(template.target_of("p5173-houhub.example.com.evil.test"), None);
        assert_eq!(template.target_of("evil.p5173-houhub.example.com"), None);
    ...
    fn a_hostname_is_read_out_of_the_authority() {
        assert_eq!(hostname_of("3000.houhub.example:8080").as_deref(), Some("3000.houhub.example"));
        assert_eq!(hostname_of(" 3000.HouHub.Example ").as_deref(), Some("3000.houhub.example"));
    ...
    fn every_authority_a_request_carries_is_read() {
        ...
        assert_eq!(direct(&[("host", "3000.HouHub.Test:8080")]), ["3000.houhub.test"]);
        ...
        assert_eq!(
            proxied(&[("host", "3000.houhub.test"), ("x-forwarded-host", "houhub.test")]),
            ["3000.houhub.test", "houhub.test"]
        );
        // A proxy that appends rather than replaces, and one that sends the
        // header twice: every value counts.
        assert_eq!(
            proxied(&[("host", "houhub:3080"), ("x-forwarded-host", "evil.test, 3000.houhub.test")]),
            ["houhub", "evil.test", "3000.houhub.test"]
        );
        // Without a declared proxy in front, forwarding headers are nobody's
        // word for anything.
        assert_eq!(
            direct(&[("host", "3000.houhub.test"), ("x-forwarded-host", "houhub.test")]),
            ["3000.houhub.test"]
        );
        // HTTP/2 carries the authority in the request line, with no `Host`
        // header at all.
        assert_eq!(
            addressed_hostnames(&request(&[], "http://3000.houhub.test:8080/x"), false),
            ["3000.houhub.test"]
        );
        ...
        assert_eq!(
            addressed_hostnames(
                &request(&[("host", "3000.houhub.test")], "http://9.houhub.test/x"),
                false
            ),
            ["3000.houhub.test"]
        );
    ...
    fn a_name_no_target_holds_stays_the_bridges_once_handed_out() {
        ...
        let handed_out = HashSet::from(["3000.houhub.test".to_string()]);
        ...
        assert!(unclaimed_is_the_bridges(
            Some(&template),
            &names(&["houhub.example.com", "3000.preview.example.com"]),
            &none
        ));
        assert!(!unclaimed_is_the_bridges(Some(&template), &names(&["houhub.example.com"]), &none));
        ...
        // take a workbench on `3000.houhub.test` away from houhub.
        assert!(!unclaimed_is_the_bridges(Some(&auto), &names(&["3000.houhub.test"]), &none));
        ...
        assert!(unclaimed_is_the_bridges(Some(&auto), &names(&["3000.houhub.test"]), &handed_out));
        assert!(!unclaimed_is_the_bridges(Some(&auto), &names(&["3001.houhub.test"]), &handed_out));
        ...
        assert!(unclaimed_is_the_bridges(None, &names(&["3000.houhub.test"]), &handed_out));
        assert!(!unclaimed_is_the_bridges(None, &names(&["3001.houhub.test"]), &handed_out));
    ...
    fn a_listeners_cookie_is_named_after_the_way_it_is_reached() {
        ...
        assert_eq!(listener(Some(3081)).cookie_name(), "houhub-bridge-3081");
        ...
        assert_eq!(listener(None).cookie_name(), "houhub-bridge-3000");
    }
    ...
    fn only_the_listeners_own_page_may_ask() {
        ...
        assert!(proxied(&[
            ("host", "127.0.0.1:3081"),
            ("x-forwarded-host", "houhub.example"),
            ("x-forwarded-proto", "https"),
            ("origin", "https://houhub.example"),
        ]));
        assert!(!proxied(&[
            ("host", "127.0.0.1:3081"),
            ("x-forwarded-host", "houhub.example"),
