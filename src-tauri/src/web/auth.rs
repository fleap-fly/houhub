...
pub const WS_EVENT_PROTOCOL: &str = "houhub-events";
...
mod tests {
    ...
    fn parses_token_from_ws_protocols() {
        ...
        assert_eq!(
            token_from_ws_protocols(&format!("houhub-events, houhub-token.{encoded}")).as_deref(),
            Some(token)
        );
    }
    ...
    fn ignores_invalid_ws_protocol_token() {
        assert!(token_from_ws_protocols("houhub-events, houhub-token.not-valid-@@@@").is_none());
    }
}
