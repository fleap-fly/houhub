...
//! here (with houhub's token) for a grant, then loads the bridge listener's
...
pub async fn browser_bridge_open(
    ...
    // which is what a bridge hostname is built on (`HOUHUB_BRIDGE_HOST_PATTERN`
...
/// without TLS, and a private-network or public host would make houhub a
