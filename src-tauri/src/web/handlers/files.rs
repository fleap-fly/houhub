...
use crate::paths::{houhub_uploads_root, simplify_verbatim_path};
...
/// by the next.
const UPLOAD_TOTAL_BYTES_ENV: &str = "HOUHUB_UPLOAD_MAX_TOTAL_BYTES";

/// Opt-in fail-closed mode for the quota config. When truthy and
/// `HOUHUB_UPLOAD_MAX_TOTAL_BYTES` parses as `Invalid`, startup aborts
...
const UPLOAD_STRICT_ENV: &str = "HOUHUB_UPLOAD_QUOTA_STRICT";

/// Outcome of parsing `HOUHUB_UPLOAD_MAX_TOTAL_BYTES`. Carries enough
...
/// Strict-mode validation: returns `Err` when `HOUHUB_UPLOAD_QUOTA_STRICT`
...
///   * `houhub-server` (single-purpose process) — call early in `main`
...
///
/// **Scope:** this counter is process-local. Multiple `houhub-server`
...
/// own counter and can collectively exceed the cap. houhub is designed
...
/// `paths::houhub_uploads_root` for the operator-facing version of
/// this contract.
...
pub async fn purge_upload_staging() {
    let tmp_dir = houhub_uploads_root().join(".tmp");
...
/// for operators with `HOUHUB_UPLOAD_MAX_TOTAL_BYTES` below `UPLOAD_MAX_BYTES`:
...
pub async fn upload_attachment(
    ...
    let uploads_root = houhub_uploads_root();
...
mod tests {
    ...
    // `HOUHUB_UPLOAD_MAX_TOTAL_BYTES` from a test would race the harness's
    // parallel runner.
    ...
    fn upload_quota_strict_error_display_includes_env_names_and_value() {
        ...
        assert!(msg.contains("HOUHUB_UPLOAD_MAX_TOTAL_BYTES"));
        assert!(msg.contains("HOUHUB_UPLOAD_QUOTA_STRICT"));
