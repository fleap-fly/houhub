...
) -> Result<Json<houhub_temp_reclaim::LeakedTempScan>, AppCommandError> {
...
) -> Result<Json<houhub_temp_reclaim::LeakedTempReclaim>, AppCommandError> {
...
pub struct AcpPiSetProjectTrustParams {
    pub workspace: String,
    /// `null` clears houhub's entry for this exact folder (revoke), rather than
...
mod tests {
    ...
    fn an_older_clients_agent_mentions_sidecar_is_ignored_not_rejected() {
        ...
        let params: AcpPromptParams = serde_json::from_value(serde_json::json!({
            ...
            "blocks": [{
                "type": "text",
                "text": "ask [@Antigravity](houhub://agent/antigravity)"
            }],
