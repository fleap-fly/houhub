...
impl WebServerState {
    ...
    /// callers (e.g. `houhub-server`) can pass it to `build_router`.
    pub fn shutdown_signal(&self) -> Arc<ShutdownSignal> {
        self.shutdown_signal.clone()
    }

    /// Mark the server as running from outside the Tauri command path.
    /// `houhub-server` calls `axum::serve` directly without going through
    ...
    /// True when the serve task is owned externally (e.g. by `houhub-server`
...
/// service — a non-empty `HOUHUB_TOKEN` override wins; otherwise reuse the value
    ...
    if let Err(e) = app_metadata_service::upsert_value(conn, WEB_SERVICE_TOKEN_KEY, &token).await {
        tracing::warn!(
            "[SERVER][WARN] could not persist the generated access token ({e}); it will rotate on \
             restart and self-update success detection may be unreliable — set HOUHUB_TOKEN to pin it"
        );
    }
    token
}
...
pub fn find_static_dir_standalone(explicit: Option<&str>) -> PathBuf {
    if let Some(dir) = explicit {
        ...
        if p.join("index.html").exists() {
            tracing::info!(
                "[WEB] Serving static files from HOUHUB_STATIC_DIR: {}",
                p.display()
            );
            return p;
        }
...
/// literal: the standalone `houhub-server` binds via `ToSocketAddrs`, so it
...
/// on the same one), the ports after its own unless `HOUHUB_BRIDGE_PORTS`
/// says otherwise — or no port of its own at all when
/// `HOUHUB_BRIDGE_HOST_PATTERN` names the targets by hostname.
...
pub const BRIDGE_OFF: &str = "off (HOUHUB_BRIDGE_PORTS / HOUHUB_BRIDGE_HOST_PATTERN)";
...
pub fn describe_bridge(config: &browser_bridge::BridgeConfig) -> String {
    match &config.host_pattern {
        Some(pattern) => format!(
            "hostnames {} on this port (HOUHUB_BRIDGE_HOST_PATTERN)",
            pattern.to_text()
        ),
        None => format!(
            "ports {} (HOUHUB_BRIDGE_PORTS)",
...
pub(crate) async fn do_start_web_server_tauri(
    ...
    let app_state = Arc::new(AppState {
        ...
        // `HOUHUB_DATA_DIR` reaches the credential helper and any HTTP
