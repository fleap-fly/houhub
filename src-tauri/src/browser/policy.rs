...
/// `HOUHUB_POLICY_FILE` when set (tests, unusual deployments), else the
/// machine-wide location for the platform. The file is optional.
pub fn managed_policy_path() -> PathBuf {
    if let Some(explicit) = std::env::var_os("HOUHUB_POLICY_FILE") {
        return PathBuf::from(explicit);
    }
    if cfg!(target_os = "macos") {
        PathBuf::from("/Library/Application Support/houhub/policy.json")
    } else if cfg!(target_os = "windows") {
        let base = std::env::var_os("PROGRAMDATA")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from(r"C:\ProgramData"));
        base.join("houhub").join("policy.json")
    } else {
        PathBuf::from("/etc/houhub/policy.json")
    }
}
...
mod tests {
    ...
    fn subframes_may_hold_opaque_content_but_not_local_schemes() {
        ...
        for bad in ["file:///etc/passwd", "tauri://localhost/", "houhub-doc://g/x", "about:config"] {
    ...
    fn wire_names() {
        let json = serde_json::to_value(BrowserPolicyStatus {
            ...
            managed_source: Some("/etc/houhub/policy.json".into()),
        })
        .unwrap();
        assert_eq!(json["managedRules"][0]["action"], "block");
        assert_eq!(json["managedSource"], "/etc/houhub/policy.json");
