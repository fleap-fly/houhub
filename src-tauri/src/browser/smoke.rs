...
//! drives the app from the inside instead: when `HOUHUB_BROWSER_SMOKE_DIR` is
...
pub fn spawn_if_enabled(app: AppHandle) {
    let Ok(dir) = std::env::var("HOUHUB_BROWSER_SMOKE_DIR") else {
        return;
    };
...
async fn execute(app: &AppHandle, cmd: &Value) -> Result<Value, String> {
    ...
    match op.as_str() {
        ...
        "browser_pick_element" => {
            ...
            // `__houhubAcceptUntrusted` is set here, in a world no page script
            // can reach.
            let driver = match (
            ...
            if let Some(driver) = driver {
                ...
                let arm = format!(
                    "(function(){{ globalThis.__houhubAcceptUntrusted = true; var n = 0; \
                     var t = setInterval(function(){{ n += 1; \
                       if (n > 100) {{ clearInterval(t); return }} \
                       if (!globalThis.__houhubPicker) return; \
        ...
        // `houhub-mcp` binary can be driven against this running app the way an
        ...
        // build, and the puppet does nothing without `HOUHUB_BROWSER_SMOKE_DIR`.
