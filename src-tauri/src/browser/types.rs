...
/// `houhub-doc:` guest (see `doc_guest`).
...
pub enum BrowserErrorKind {
    ...
    /// that address (`HOUHUB_BROWSER_TUNNEL=private`).
    RemoteNotAllowed,
...
pub struct BrowserCapabilities {
    ...
    /// Local HTML files can be shown through the `houhub-doc:` document guest
    ...
    /// its houhub-server (`browser::remote`): a profile of their own to proxy
...
/// A local server houhub started has just announced its address; the workspace
...
pub enum ServiceSource {
    /// A houhub terminal: the terminal panel, or a canvas terminal card.
    Terminal,
    /// A terminal an agent asked houhub to run (ACP `terminal/create`).
    Agent,
}
...
pub struct DetectedService {
    ...
    /// The terminal that printed it — a houhub terminal id, or an ACP one.
    ///
