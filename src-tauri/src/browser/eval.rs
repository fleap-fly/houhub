...
//!
//! **The code runs in the page's own world, not houhub's.** The tempting thing
...
//! page *is* comes from a separate evaluation in houhub's own world, which no
...
pub struct EvalOutcome {
    ...
    pub truncated: bool,
    /// Where the page was, read in houhub's own world after the code ran — not
...
/// `__houhubEvalRender` lying around would know houhub had run something, which
...
mod tests {
    ...
    fn a_trailing_line_comment_cannot_eat_the_wrapper() {
        ...
        assert!(after.contains("__houhubEvalRender"));
    }
    ...
    fn every_call_carries_its_renderer_and_leaves_nothing_behind() {
        ...
        assert!(js.ends_with("})()"));
        assert!(js.contains("var __houhubEvalClip"));
        assert!(js.contains("__houhubEvalError"));
