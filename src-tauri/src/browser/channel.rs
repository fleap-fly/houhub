...
pub const HELPER_JS_ENGINE_CONSOLE: &str = concat!(
    "globalThis.__houhubEngineConsole = true;\n",
...
#[cfg(any(target_os = "macos", target_os = "linux"))]
pub const PREFIX_SCRIPT: &str = "globalThis.__houhubSend = function (m) { window.webkit.messageHandlers.houhubBrowser.postMessage(String(m)); };";
...
#[cfg(target_os = "linux")]
pub const FRAME_PREFIX_SCRIPT: &str = "if (window.top !== window) { globalThis.__houhubSend = function (m) { window.webkit.messageHandlers.houhubBrowserFrame.postMessage(String(m)); }; }";
...
mod tests {
    ...
    fn helper_is_bundled_and_self_contained() {
        assert!(HELPER_JS.contains("houhubBrowserHelper"));
