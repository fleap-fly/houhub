...
//! - `doc_guest`  — the `houhub-doc:` guest that shows a local HTML file
...
// and they are compiled in BOTH runtimes. The houhub-mcp plumbing that carries
        ...
        for entry in std::fs::read_dir(&dir).expect("capabilities dir") {
            ...
            for key in ["windows", "webviews"] {
                ...
                for pattern in scoped.iter().filter_map(|w| w.as_str()) {
                    ...
                    for forbidden in [TAB_LABEL_PREFIX, "browser-popup-", "houhub-doc-"] {
                        assert!(
