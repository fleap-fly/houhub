...
pub enum NotGrantable {
    ...
    NoOrigin,
    /// A document guest (`houhub-doc:`). It shows a local file — usually one
    /// the agent wrote — under an origin that is minted per document and
    /// spelled differently on every platform (`houhub-doc://doc-<token>/…`
    /// under WebKit, `http://houhub-doc.doc-<token>/…` under WebView2), so
...
pub const AGENT_GLOBAL: &str = "__houhubAgent";
...
/// `JSON.stringify(__houhubAgent.snapshot({…}))`.
///
...
/// `JSON.stringify(__houhubAgent.act(generation, ref, action))`, or
...
/// `JSON.stringify(__houhubAgent.locate(generation, ref))`: where a pointer
...
/// `JSON.stringify(__houhubAgent.rectOf(generation, ref))`: the element's
...
mod tests {
    ...
    fn a_blank_tab_is_listed_and_a_document_guest_is_not() {
        ...
        assert_eq!(
            summarize_tab(&tab(Some("https://houhub-doc.localhost"), TabKind::Document)),
            None
        );
    }
    ...
    fn only_a_web_origin_can_be_shared() {
        ...
        assert_eq!(
            grantable_origin(&tab(Some("https://houhub-doc.localhost"), TabKind::Document)),
            Err(NotGrantable::DocumentGuest)
        );
        assert_eq!(
            grantable_origin(&tab(Some("houhub-doc://abc"), TabKind::Page)),
            Err(NotGrantable::NoOrigin)
        );
    }
    ...
    fn a_tab_with_no_web_origin_cannot_be_shared_at_all() {
        ...
        let mut guest = tab(Some("https://houhub-doc.localhost"), TabKind::Document);
        assert_eq!(
    ...
    fn the_expression_probes_before_it_calls() {
        ...
        assert!(js.starts_with("typeof globalThis.__houhubAgent === 'undefined'"));
        assert!(js.contains(r#""absent""#));
        assert!(js.contains(r#""epoch":"7.2""#));
        assert!(js.contains(r#""maxChars":2000"#));
        assert!(js.contains("JSON.stringify(globalThis.__houhubAgent.snapshot("));
    }
    ...
    fn the_installing_form_is_one_expression_ending_in_the_call() {
        ...
        assert!(js.contains(";return JSON.stringify(globalThis.__houhubAgent.snapshot("));
    ...
    fn the_act_call_carries_the_request_as_json_and_never_installs_the_engine() {
        ...
        assert!(call.starts_with("typeof globalThis.__houhubAgent === 'undefined' ? \"absent\" :"));
        ...
        assert!(locate.starts_with("typeof globalThis.__houhubAgent === 'undefined'"));
    }
