...
//! WebSocket to the remote houhub-server's tunnel (`web/browser_tunnel`), which
...
pub const PROBE_PATH: &str = "/houhub-egress-probe/";
...
pub enum EgressStatus {
    ...
    Down { reason: String },
    /// The remote houhub-server has no tunnel (an older version).
    Unsupported,
    /// The remote houhub-server's tunnel is switched off.
    Disabled,
}
...
fn probe_page(nonce: &str) -> String {
    format!(
        "<!doctype html><title>houhub egress probe</title><script>\
...
mod tests {
    ...
    fn probe_paths_carry_a_plain_nonce_and_nothing_else() {
        assert_eq!(probe_nonce("/houhub-egress-probe/abc-123"), Some("abc-123"));
        assert_eq!(probe_nonce("/houhub-egress-probe/abc/ws"), Some("abc"));
        assert_eq!(probe_nonce("/houhub-egress-probe/abc?x=1"), Some("abc"));
        assert_eq!(probe_nonce("/houhub-egress-probe/"), None);
        assert_eq!(probe_nonce("/houhub-egress-probe/a%20b"), None);
