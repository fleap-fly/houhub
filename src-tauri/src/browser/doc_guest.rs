//! The `houhub-doc:` document guest: a local HTML file shown through a webview
...
//!   cannot address `houhub-doc:` at all; and the handler is bound to one
...
pub const DOC_LABEL_PREFIX: &str = "houhub-doc-";
pub const DOC_SCHEME: &str = "houhub-doc";
...
///
/// - `houhub-doc.` as a PREFIX would make every registrable domain someone
///   owns a guest address (`https://houhub-doc.example.com/`), and a guest is
...
impl DocGrant {
    ...
    /// Windows filter is `http://houhub-doc.*`, and the macOS handler takes
...
mod tests {
    ...
    fn document_urls_and_navigation_policy() {
        ...
        assert_eq!(
            ...
            "houhub-doc://doc-1111/a%20b/c%23d.html"
        );
        assert_eq!(
            document_url(own, Path::new("index.html")),
            "houhub-doc://doc-1111/index.html"
        );
        let doc = Url::parse("houhub-doc://doc-1111/other.html").unwrap();
        assert!(is_document_url(&doc, own));
        assert!(is_document_url(&Url::parse("https://houhub-doc.doc-1111/x").unwrap(), own));
        ...
        assert!(!is_document_url(
            &Url::parse("https://houhub-doc.example.com/steal").unwrap(),
            own
        ));
        assert_eq!(
            guest_navigation(
        ...
        for other in [
            "houhub-doc://doc-2222/other.html",
            "https://houhub-doc.doc-2222/other.html",
        ] {
            let other = Url::parse(other).unwrap();
            assert!(!is_document_url(&other, own), "{other}");
        }
        assert_eq!(
            guest_navigation(&Url::parse("houhub-doc://doc-2222/x").unwrap(), own, true),
        ...
        assert!(!is_document_url(
            &Url::parse("houhub-doc://DOC-1111/x").unwrap(),
            own
        ));
        ...
        assert!(!is_document_url(
            &Url::parse("houhub-doc://doc-1111:8080/x").unwrap(),
            own
        ));
        assert!(!is_document_url(
            &Url::parse("houhub-doc://doc-1111:80/x").unwrap(),
            own
        ));
        assert!(!is_document_url(
            &Url::parse("http://houhub-doc.doc-1111:8080/x").unwrap(),
            own
        ));
        assert!(is_document_url(
            &Url::parse("http://houhub-doc.doc-1111:80/x").unwrap(),
            own
        ));
        assert_eq!(
            guest_navigation(
        ...
        if cfg!(target_os = "windows") {
            assert_eq!(engine.as_str(), "http://houhub-doc.doc-1111/other.html");
        } else {
    ...
    fn resolve_document_confines_to_the_root_or_the_file_directory() {
        ...
        assert_eq!(
            grant.document_url(),
            format!("houhub-doc://{}/docs/report.html", grant.host())
        );
    ...
    fn grants_are_shared_per_document_and_bound_per_tab() {
        ...
        assert_eq!(doc_label("t1"), "houhub-doc-t1");
    }
