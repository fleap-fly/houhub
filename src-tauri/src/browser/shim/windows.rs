...
pub const BINDING_NAME: &str = "__houhubSend";
...
/// invisible to houhub — out of an embedded tab.
...
struct SurfaceState {
    ...
    committed: Cell<bool>,
    /// `houhub`-world execution contexts: which frame each is for, by the CDP
    ...
    /// will have `__houhubSend` in it. Until then there is nothing to build one
...
/// Install the helper in the `houhub` isolated world and the binding it posts
...
fn on_event(key: usize, event: &str, session: &str, raw: &str) {
    ...
    match event {
        "Runtime.executionContextCreated" => {
            ...
            // A frame holds one `houhub` world at a time, so an older entry
...
/// Build the `houhub` world for a document the injection did not reach and run
/// the helper in it.
///
...
/// exists; the binding is registered by world NAME, so `__houhubSend` is in it
...
/// The `houhub` world of the main frame, once the helper has run there.
///
...
/// bar stays suppressed — houhub draws that. `callback` gets whether anything
/// matched.
pub fn find_string(
    webview: &wry::WebView,
    query: &str,
...
pub fn install_navigation_hooks(
    ...
    *state.navigation.borrow_mut() = Some(navigation);
    ...
    // happen and houhub would have no idea there was anything to show. Only
