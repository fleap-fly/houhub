...
//!
//! The helper script and the `houhubBrowser` message handler live in the
//! `WKContentWorld` named `houhub`: a separate JavaScript global for the same
...
define_class!(
    ...
    #[ivars = HandlerIvars]
    pub struct HouHubMessageHandler;

    unsafe impl NSObjectProtocol for HouHubMessageHandler {}

    unsafe impl WKScriptMessageHandler for HouHubMessageHandler {
        #[unsafe(method(userContentController:didReceiveScriptMessage:))]
        fn did_receive(
            this: &HouHubMessageHandler,
...
/// houhub sets no minimumSystemVersion.
...
pub fn install_world(
    ...
    let handler = HouHubMessageHandler::new(sink, mtm);
    ...
    unsafe {
        if supports_content_world(&controller) {
            let world = WKContentWorld::worldWithName(ns_string!("houhub"), mtm);
            controller.addScriptMessageHandler_contentWorld_name(
                proto,
                &world,
                ns_string!("houhubBrowser"),
            );
            ...
            controller.addScriptMessageHandler_name(proto, ns_string!("houhubBrowser"));
            for source in scripts {
...
/// Evaluate `expression` in the `houhub` world of the main frame. The result
...
thread_local! {
    ...
    /// is a weak property).
    static NAV_DELEGATES: RefCell<HashMap<usize, Retained<HouHubNavigationDelegate>>> =
...
define_class!(
    ...
    pub struct HouHubNavigationDelegate;

    unsafe impl NSObjectProtocol for HouHubNavigationDelegate {}

    impl HouHubNavigationDelegate {
    ...
    unsafe impl WKNavigationDelegate for HouHubNavigationDelegate {
...
pub fn install_navigation_delegate(webview: &wry::WebView, sink: NavigationSink) -> Result<(), String> {
    ...
    let delegate = HouHubNavigationDelegate::new(inner, sink, mtm);
...
thread_local! {
    ...
    static UI_DELEGATES: RefCell<HashMap<usize, Retained<HouHubUIDelegate>>> =
        RefCell::new(HashMap::new());
}
...
define_class!(
    ...
    pub struct HouHubUIDelegate;

    unsafe impl NSObjectProtocol for HouHubUIDelegate {}

    impl HouHubUIDelegate {
        #[unsafe(method(respondsToSelector:))]
        fn responds_to_selector(&self, selector: Sel) -> bool {
            self.class().responds_to(selector) || self.ivars().inner.respondsToSelector(selector)
        }
    ...
    unsafe impl WKUIDelegate for HouHubUIDelegate {
...
pub fn install_page_close_hook(webview: &wry::WebView, sink: PageCloseSink) -> Result<(), String> {
    ...
    let delegate = HouHubUIDelegate::new(inner, sink, mtm);
...
const LOOPBACK_RULES_ID: &str = "houhub-remote-egress-loopback";
...
mod network {
    ...
    /// from this machine (dev servers, houhub's own bridges) are the point of
