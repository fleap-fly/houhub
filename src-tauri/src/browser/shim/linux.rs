...
//! **script world** named `houhub`: `UserScript::for_world` injects the helper
//! there and `register_script_message_handler_in_world` puts
//! `window.webkit.messageHandlers.houhubBrowser` in the same world — a separate
...
//! frame with a primitive that posts through `houhubBrowser`, and once into every
//! frame with one that posts through `houhubBrowserFrame` — that second one
...
//!
//! Neither handler is reachable from page script: both live in the `houhub`
...
/// not the main frame.
pub const FRAME_HANDLER_NAME: &str = "houhubBrowserFrame";
...
/// Install the helper in the `houhub` world and the handler it posts through.
...
pub fn install_navigation_hooks(
    ...
    *state.navigation.borrow_mut() = Some(navigation);
    ...
    webview.connect_load_failed(move |_webview, event, uri, error| {
        ...
        // window the user is looking at should show. houhub's own message is in
