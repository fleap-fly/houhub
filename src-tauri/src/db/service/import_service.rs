...
///
/// Deleting a conversation in houhub only stamps `deleted_at`; neither the row
...
///   when it is strictly newer, so a conversation continued outside HouHub sorts
...
mod tests {
    ...
    fn summary(id: &str, title: Option<&str>) -> ConversationSummary {
        ConversationSummary {
            ...
            folder_path: Some("/tmp/houhub-import".to_string()),
            folder_name: None,
    ...
    async fn reimport_refreshes_a_changed_title() {
        ...
        let folder = seed_folder(&db, "/tmp/houhub-import").await;
    ...
    async fn reimport_marks_the_conversation_for_a_token_usage_re_parse() {
        ...
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-usage").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn import_inserts_a_pending_review_conversation() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-status").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_skips_an_unchanged_title() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-same").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_never_clobbers_a_manual_rename() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-lock").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_with_no_title_keeps_the_existing_one() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-empty").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_skips_a_soft_deleted_conversation() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-deleted").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn restore_brings_a_soft_deleted_conversation_back_in_place() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-restore").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn restore_is_idempotent_and_leaves_a_live_row_alone() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-restore-twice").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn restore_lands_the_conversation_in_the_import_target_folder() {
        let db = fresh_in_memory_db().await;
        let old_folder = seed_folder(&db, "/tmp/houhub-restore-old").await;
        let target = seed_folder(&db, "/tmp/houhub-restore-target").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn restore_never_touches_a_delegation_child() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-restore-child").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_adopts_newer_transcript_activity() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-activity").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_never_moves_updated_at_backwards() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-monotonic").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn activity_refresh_preserves_pin_status_and_locked_title() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-preserve").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn sync_imported_sessions_refreshes_in_place_and_never_inserts() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-sync-scan").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn sync_imported_sessions_skips_deleted_rows() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-sync-deleted").await;
        let at = AgentType::ClaudeCode;
    ...
    async fn reimport_skips_a_delegation_child() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-import-child").await;
        let at = AgentType::ClaudeCode;
