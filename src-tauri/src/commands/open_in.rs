...
mod tests {
    ...
    fn validate_rejects_missing_path() {
        let err = validate_open_in_code_path("/definitely/not/a/houhub/path").expect_err("missing");
    ...
    /// `&` is a legal file-name character on every platform HouHub ships to, and
    ...
    async fn open_in_code_core_errors_when_path_missing() {
        let err = open_in_code_core("/no/such/houhub/open-in-code-target".into())
            .await
