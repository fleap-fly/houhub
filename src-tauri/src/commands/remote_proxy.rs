...
//!
//! When a desktop window is opened against a remote houhub-server, every API
...
/// `sec-websocket-protocol` header looking for `houhub-token.{base64url}`.
const WS_EVENT_PROTOCOL: &str = "houhub-events";
const WS_TOKEN_PROTOCOL_PREFIX: &str = "houhub-token.";
...
/// Forward an HTTP API call to the remote houhub-server identified by
...
pub async fn remote_http_call(
    ...
    if !status.is_success() {
        ...
        // The remote houhub-server always returns `Json(AppCommandError)`
...
/// remote houhub-server identified by `connection_id`. Sibling of
...
//
// Issue #179 follow-up: a Tauri client bound to a remote houhub-server
...
fn partial_download_path(save_path: &str, transfer_id: &str) -> String {
    format!("{save_path}.houhub-download-{transfer_id}.part")
}
...
/// this connection. The remote houhub-server's `ws.rs` emits frames shaped
...
/// `protocol` (`houhub-events`, or the browser tunnel's). The remote server's
/// auth middleware (see `web/auth.rs`) accepts either `Authorization: Bearer …`
/// or a subprotocol entry shaped `houhub-token.{base64url(token)}`. The latter
...
mod tests {
    ...
    fn partial_download_path_is_unique_sibling() {
        ...
        assert_eq!(path, "/tmp/out.zip.houhub-download-abc.part");
    }
