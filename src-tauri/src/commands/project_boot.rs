...
fn validate_project_name(name: &str) -> Result<(), AppCommandError> {
    ...
    // "..\\x" as traversal — reject separators explicitly since houhub ships on
    // Windows too.
...
/// houhub's six supported agents, mapped to the `skills` CLI's `--agent` ids.
/// The launcher only ever targets these agents (never the CLI's full 55+ agent
/// universe via `--all`/`--agent '*'`), so installs stay scoped to tools houhub
...
/// speak) to houhub's internal `AgentType`, which drives skill-dir resolution.
...
/// directories. Reuses houhub's own skill-dir resolution + enumeration
