...
/// the general retry path would refuse a `houhub-doc:` address.
fn reload_document(
    app: &AppHandle,
    registry: &BrowserRegistry,
    tab_id: &str,
...
/// 2. where the page actually is, read in houhub's own world, held against the
...
/// Where the page is, as houhub's own world reports it, held against the tab's
...
async fn eval_on_shared_page(
    ...
    // Where the page is now, from houhub's world, held against the grant — and
...
/// in houhub's isolated world, and an agent's own code is the one thing that
...
pub async fn agent_open_tab_core(
    ...
    let tab_id = match answered {
        ...
        Ok(Err(_)) | Err(_) => {
            return Err(open_failed(
                "no houhub workspace window answered. There has to be one open to put a tab in",
            ))
        }
    };
...
/// drives commands, and because the tool surface in `houhub-mcp` reaches the
