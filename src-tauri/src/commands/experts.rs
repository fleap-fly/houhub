...
//!
//! Experts are curated skills (from obra/superpowers) that houhub bundles
//! into its binary via `include_dir!`. On startup they are extracted to a
//! central directory `~/.houhub/skills/<id>/`. Users can then enable an
...
//! database state, and updates propagate automatically when houhub upgrades
...
const CENTRAL_DIR_NAME: &str = ".houhub";
...
pub(crate) fn classify_link(link_path: &Path, expected_target: &Path) -> ExpertLinkState {
    ...
    match (resolved_link, resolved_expected) {
        ...
        (Some(l), Some(e)) if paths_equivalent(&l, &e) => ExpertLinkState::LinkedToHouhub,
...
fn ensure_central_experts_installed_blocking() -> InstallReport {
    ...
    manifest.houhub_version = env!("CARGO_PKG_VERSION").to_string();
...
mod tests {
    ...
    async fn apply_links_does_not_deadlock() {
        ...
        let ops = vec![
            LinkOp {
                expert_id: "zzz-houhub-batch-test-absent-aaa".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
                expert_id: "zzz-houhub-batch-test-absent-bbb".into(),
    ...
    async fn apply_links_collects_per_op_results_without_aborting() {
        let ops = vec![
            LinkOp {
                expert_id: "zzz-houhub-batch-test-absent".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
