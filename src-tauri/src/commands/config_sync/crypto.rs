...
//! [`crate::commands::backup::crypto`]'s `.houhubbak`. Three reasons: the remote
...
pub const ENVELOPE_MARKER: &str = "houhubConfigEncryption";
...
pub struct EncryptedPayload {
    ...
    pub houhub_config_encryption: u32,
    pub algo: String,
    pub kdf: String,
...
pub fn encrypt(plain: &[u8], passphrase: &str) -> Result<Vec<u8>, AppCommandError> {
    ...
    let payload = EncryptedPayload {
        houhub_config_encryption: ENVELOPE_VERSION,
        algo: ALGO.to_string(),
        kdf: KDF.to_string(),
        kdf_params,
...
pub fn decrypt(payload: &EncryptedPayload, passphrase: &str) -> Result<Vec<u8>, AppCommandError> {
    if payload.houhub_config_encryption > ENVELOPE_VERSION {
...
mod tests {
    ...
    fn a_newer_envelope_is_refused_rather_than_guessed_at() {
        let mut payload = seal(b"payload", "pw");
        payload.houhub_config_encryption = ENVELOPE_VERSION + 1;
