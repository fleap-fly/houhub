...
pub const DEFAULT_REMOTE_DIR: &str = "houhub";
...
mod tests {
    ...
    fn input() -> ConfigSyncSettingsInput {
        ConfigSyncSettingsInput {
            ...
            passphrase: None,
            encrypt: false,
            remote_dir: Some("houhub".to_string()),
    ...
    async fn a_password_written_by_an_older_build_is_migrated_out_of_the_row() {
        ...
        app_metadata_service::upsert_value(
            ...
            r#"{"enabled":true,"serverUrl":"https://dav.example.com/dav","username":"alice","password":"legacy-secret","remoteDir":"houhub","profile":"work","autoSync":true,"intervalMinutes":5}"#,
        )
        .await
    ...
    async fn the_migration_removes_the_password_and_nothing_else() {
        ...
        app_metadata_service::upsert_value(
            &db.conn,
            CONFIG_SYNC_SETTINGS_KEY,
            r#"{"enabled":true,"serverUrl":"https://dav.example.com/dav","username":"alice","password":"legacy-secret","remoteDir":"houhub","profile":"work","autoSync":true,"intervalMinutes":5,"futureField":"written by a newer build"}"#,
        )
        .await
    ...
    async fn the_keyring_copy_wins_over_a_stale_row_copy() {
        ...
        app_metadata_service::upsert_value(
            &db.conn,
            CONFIG_SYNC_SETTINGS_KEY,
            r#"{"enabled":true,"serverUrl":"https://dav.example.com/dav","username":"alice","password":"outdated","remoteDir":"houhub","profile":"work","autoSync":true,"intervalMinutes":5}"#,
        )
        .await
        .expect("seed row");
    ...
    async fn sync_state_survives_a_reload() {
        ...
        let state = ConfigSyncState {
            ...
            last_uploaded_target: Some("https://dav.example.com/dav\u{0}houhub\u{0}work".to_string()),
            last_sync_at: Some("2026-01-01T00:00:00Z".to_string()),
            last_error: None,
        };
