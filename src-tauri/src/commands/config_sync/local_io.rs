...
pub async fn build_export_core(
    ...
    Ok(ConfigExportFile {
        houhub_config_export: EXPORT_FORMAT_VERSION,
        manifest,
        config: snapshot,
    })
}
...
pub fn parse_export_bytes(
    ...
    let value: serde_json::Value = serde_json::from_slice(bytes).map_err(|e| {
        AppCommandError::invalid_input("Not a houhub config file")
            .with_detail(e.to_string())
            .with_i18n(CONFIG_SYNC_I18N_KEY_INVALID_SNAPSHOT, BTreeMap::new())
    })?;

    if value.get("houhubConfigExport").is_some() {
        let export: ConfigExportFile = serde_json::from_value(value).map_err(|e| {
            AppCommandError::invalid_input("Malformed houhub config export")
                .with_detail(e.to_string())
                .with_i18n(CONFIG_SYNC_I18N_KEY_INVALID_SNAPSHOT, BTreeMap::new())
        })?;
...
mod tests {
    ...
    async fn exported_file_imports_into_another_machine() {
        ...
        let dest = dir.path().join("nested").join("houhub-config.json");
    ...
    /// should not be told their own file is not a houhub config.
    #[tokio::test]
    ...
    /// `~/.houhub/config-snapshots`. The button showed nothing while the test
