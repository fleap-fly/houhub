...
static MARKETPLACE_HTTP_CLIENT: LazyLock<Result<reqwest::Client, String>> = LazyLock::new(|| {
    reqwest::Client::builder()
        ...
        .user_agent("houhub-mcp-market/1.0")
        .build()
...
/// The marketplace suffix houhub uses when toggling user-scope Claude Code
...
// whole point of its design). The store below is therefore houhub's own record
...
// houhub's own data dir so it travels with the rest of the DeepSeek state a
// user backs up or moves, and it holds houhub's canonical spec shape verbatim
...
/// mode that agent chose; this one is created by HOUHUB, so its mode is houhub's
...
// houhub always writes the explicit `mcpServers` wrapper.
//
...
/// Whether Antigravity's config holds an `mcpServers` value houhub refuses to
...
/// So houhub cannot just add the wrapper to a bare document; doing that would
...
/// check and rejects the WHOLE file, so houhub cannot know what the key means,
...
/// the root in that case, so neither may houhub: reporting the root's siblings
...
// servers aren't double-registered over `session/new`. The built-in `houhub-mcp`
// companion is injected separately by `inject_houhub_mcp`, so it still reaches
// Kimi regardless.
// ---------------------------------------------------------------------------
...
/// Convert HouHub canonical spec into a Kimi `mcpServers` entry.
///
...
// `houhub-mcp` companion is injected separately by `inject_houhub_mcp`, so it
...
// launch (registering each as an `mcp-<name>` toolset), so houhub manages that
...
// Translate to/from houhub's canonical spec, whose discriminator is `type`.
// ---------------------------------------------------------------------------

/// Convert one Hermes `mcp_servers` YAML entry into houhub's canonical spec.
...
/// Convert houhub's canonical spec into a Hermes `mcp_servers` YAML entry.
...
/// serde_yaml and so drops comments — consistent with houhub's existing Hermes
/// config edits.
...
mod tests {
    ...
    fn deepseek_mcp_json_round_trips_the_canonical_spec() {
        // `$DSH_HOME/mcp.json` is houhub's OWN store (deepseek-acp reads no MCP
    ...
    fn deepseek_mcp_store_is_not_world_readable() {
        ...
        // houhub CREATES this file (no other agent's store works that way), and
    ...
    fn antigravity_mcp_config_handles_both_document_shapes() {
        ...
        // WHOLE file — it does NOT fall back to the root — so houhub must
        ...
        assert!(
            seen.is_empty(),
            "the agent sees nothing here, so neither may houhub: {seen:?}"
        );
        assert!(
    ...
    fn qoder_settings_json_round_trips_and_preserves_foreign_keys() {
        // `~/.qoder/settings.json` is QODER'S file, not houhub's: the CLI owns
        ...
        // on read — houhub must skip it, never rewrite the file to "fix" it).
        std::fs::write(
            &path,
            serde_json::to_string_pretty(&json!({
                "securityScan": true,
