...
//!
//! The fourth "skill pack" in houhub's central store `~/.houhub/skills/<id>/`.
...
//! a folder into `~/.houhub/skills` and have it appear on refresh. The startup
...
fn collect_custom_ids() -> Result<Vec<String>, CustomSkillsError> {
    ...
    for entry in entries {
        ...
        // Skip houhub-owned dotfiles (`.manifest.json`, `.manifest.science.json`).
        if name.starts_with('.') {
            continue;
        }
    ...
    match create_link_raw(&central, &link_path) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AlreadyExists => {
            match classify_link(&link_path, &central) {
                ExpertLinkState::LinkedToHouhub => {} // idempotent success
...
/// (LinkedToHouhub / Broken) — foreign links and real dirs are left untouched.
fn unlink_one_locked(id: &str, agent_type: AgentType) -> Result<(), CustomSkillsError> {
    ...
    for dir in dirs {
        ...
        if matches!(
            state,
            ExpertLinkState::LinkedToHouhub | ExpertLinkState::Broken
        ) {
...
mod tests {
    ...
    async fn apply_links_does_not_deadlock() {
        let ops = vec![
            LinkOp {
                expert_id: "zzz-houhub-custom-batch-absent-aaa".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
                expert_id: "zzz-houhub-custom-batch-absent-bbb".into(),
    ...
    async fn apply_links_collects_per_op_results_without_aborting() {
        let ops = vec![
            LinkOp {
                ...
                expert_id: "zzz-houhub-custom-batch-absent".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
                // Enable of a skill with no central copy → fails its op.
                expert_id: "zzz-houhub-custom-not-installed".into(),
                agent_type: AgentType::ClaudeCode,
                enable: true,
            },
        ];
    ...
    async fn delete_absent_skills_report_per_id() {
        ...
        let results = custom_delete_skills(vec![
            "zzz-houhub-custom-delete-absent-1".into(),
            "zzz-houhub-custom-delete-absent-2".into(),
        ])
        .await
        .expect("batch returns Ok");
        assert_eq!(results.len(), 2);
    ...
    async fn import_from_agent_reports_missing_ids_without_mutating() {
        ...
        let results = custom_import_from_agent(
            ...
            vec![
                "zzz-houhub-custom-import-absent-1".into(),
                "zzz-houhub-custom-import-absent-2".into(),
            ],
        )
        .await
        .expect("batch returns Ok");
