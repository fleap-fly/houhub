...
pub(crate) fn seed_pi_workspace_trust(cwd: &Path, runtime_env: &BTreeMap<String, String>) {
    ...
    let canonical = match fs::canonicalize(cwd) {
        Ok(path) => path,
        Err(err) => {
            tracing::debug!("[pi] trust seed skipped: canonicalize {cwd:?} failed: {err}");
            return;
        }
    };
    ...
    let mut object = match fs::read_to_string(&path) {
        ...
        Err(err) => {
            tracing::debug!("[pi] trust seed skipped: read {path:?} failed: {err}");
            return;
        }
    };
    ...
    let serialized = match serde_json::to_string_pretty(&serde_json::Value::Object(object)) {
        Ok(value) => value,
        Err(err) => {
            tracing::debug!("[pi] trust seed skipped: serialize failed: {err}");
            return;
        }
    };
    if let Some(parent) = path.parent() {
        if let Err(err) = fs::create_dir_all(parent) {
            tracing::debug!("[pi] trust seed skipped: create {parent:?} failed: {err}");
            return;
        }
    }
    if let Err(err) = fs::write(&path, format!("{serialized}\n")) {
        tracing::debug!("[pi] trust seed write failed for {path:?}: {err}");
    }
}

#[cfg(unix)]
fn pi_path_is_executable(path: &Path) -> bool {
    use std::os::unix::fs::PermissionsExt;
    fs::metadata(path)
