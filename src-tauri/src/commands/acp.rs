...
/// app launched from Finder or the Dock. Ordered last, so a houhub-managed copy
...
/// (`~/.houhub/npm-global/`) so packages installed via the EACCES fallback are
/// found as well.
///
...
async fn detect_local_version(agent_type: AgentType) -> Option<String> {
    ...
    match meta.distribution {
        ...
        registry::AgentDistribution::Binary { cmd, dir_entry, .. } => {
            ...
            // Dir-tree agents: a user-installed CLI (no HouHub cache) still
...
struct AgentDiag {
    ...
    /// `~/.houhub/npm-global/bin/<cmd>` (EACCES fallback) when it exists.
...
async fn diag_terminal_probe(cmd: &str, app_path: &[String]) -> TerminalProbe {
    ...
    for line in stdout.lines() {
        if let Some(p) = line.strip_prefix("HOUHUB_PATH=") {
        ...
        if agent.db_version.is_some() || agent.detected_version.is_some() {
            if agent.user_prefix_bin.is_some() {
                return diag_verdict(
                    ...
                    "Installed into the EACCES fallback prefix (~/.houhub/npm-global), which is not on the app's PATH.",
                );
            }
...
fn render_plain_text(
    ...
    out.push_str("===== HouHub environment diagnostics =====\n");
...
mod diagnostics_tests {
    ...
    fn verdict_user_prefix_not_on_path() {
        ...
        a.user_prefix_bin = Some("/home/u/.houhub/npm-global/bin/codex-acp".to_string());
        inp.agent = Some(a);
    ...
    fn build_report_is_deterministic_and_includes_plain_text() {
        ...
        assert!(r.plain_text.contains("HouHub environment diagnostics"));
...
/// leaving users with npm's opaque `fetch failed` message. Other npm agents do
/// not perform this bootstrap and should retain their original error.
fn annotate_npm_bootstrap_failure(package: &str, err: AcpError) -> AcpError {
    if package_name_from_spec(package) != "hermes-agent" {
        return err;
    }
...
/// Fallback: install an npm package into a user-local prefix (`~/.houhub/npm-global/`)
...
/// Uninstall an npm package from the user-local prefix (`~/.houhub/npm-global/`).
...
fn codex_config_projection_from_toml(raw_toml: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    // session is running. Include them in the launch fingerprint so editing
    // them marks an existing session restart-required without leaking them
    // into the generic runtime environment.
    ...
    let workspace_write = sandbox.workspace_write;
    if !workspace_write.writable_roots.is_empty()
        || workspace_write.network_access
        || workspace_write.exclude_tmpdir_env_var
        || workspace_write.exclude_slash_tmp
    {
        if let Ok(value) = serde_json::to_value(&workspace_write) {
...
fn persist_codex_local_config(config_patch_json: Option<&str>) -> Result<(), AcpError> {
    ...
    let provider_name = table
        ...
        .map(str::to_string)
        .unwrap_or_else(|| "houhub".to_string());
    table.insert(
...
fn parse_grok_settings(raw_toml: &str) -> GrokSettings {
    ...
    // The HouHub-managed custom model = `[models].default`, but only when a
...
/// Write / rename / remove the HouHub-managed custom Grok model. The managed block
...
fn apply_grok_custom_model(
    ...
    // mistaken for a HouHub block and removed).
    let prev_id = doc
        .get("models")
...
//
// The houhub-managed block is keyed by the fixed names `houhub` / `houhub-managed`
...
/// The resolved houhub-managed provider/model block to write into config.toml.
struct KimiManagedSpec {
    interface_type: String,
    base_url: Option<String>,
    /// Direct `api_key` field (when the user picks "direct key" auth).
    api_key: Option<String>,
    /// `[providers.houhub.env]` sub-table entries — the env-sub-table API key, or
...
/// houhub-managed block. A clear on a non-existent file is a no-op (never creates
...
/// Project the houhub-managed config.toml block into a flat JSON object for the
...
/// `apikey` — write the houhub-managed `config.toml` provider/model block AND seed
...
/// houhub-managed provider; every save clears it to keep config.toml authoritative.
...
pub(crate) fn pi_project_trust_launch_block(
    ...
    Some(format!(
        "pi is allowed to load this project's own files from {}, which lets the repository run its .pi/extensions at startup.{via} \
         An earlier version of houhub granted this automatically when a folder was opened, so you may never have been asked. \
...
///
/// For headless deployments (HouHub on a Linux server, no desktop), where the
...
pub(crate) async fn acp_antigravity_sign_out_core(
    ...
    match crate::acp::connection::antigravity_effective_auth_type(&runtime_env) {
        ...
        AntigravityAuthType::Unreadable => {
            return Err(AcpError::protocol(
                "houhub cannot read the authentication method out of Antigravity's settings.json, \
                 ...
                 commas; houhub does not), or move it aside, then try again.",
            ));
        }
...
pub(crate) struct PiConfigUpdate {
    pub provider: String,
    pub model: String,
    /// All models offered by the selected provider. When a custom provider is
    /// written, these are merged into its native `models` array; the selected
    /// model receives `model_reasoning` below.
...
/// files are small and houhub-owned; corruption shouldn't abort a save (we
...
/// CLI, then a resolved npm-prefix binary, and finally the pinned npm bridge.
/// Returned as argv vectors so callers can shell-quote per platform for display
/// or execute them.
...
/// umask, the one real exposure for a first-time houhub-driven setup. An EXISTING
...
fn is_read_only_skill_path(agent_type: AgentType, skill_path: &Path) -> bool {
    let ro_root = match agent_type {
        ...
        // so editing/deleting through HouHub would silently be undone.
...
/// the central `~/.houhub/skills/` store.
...
pub(crate) async fn apply_model_provider_env(
    ...
    // cursor-agent supports only Cursor subscription login or a Cursor
    // Dashboard API key. It has no custom endpoint, so a legacy provider
    // binding must never inject third-party credentials into its runtime.
    ...
    if matches!(agent_type, AgentType::Grok | AgentType::Cursor)
        && runtime_env
            .get(model_key)
...
fn cascade_update_agent_config(
    ...
    match agent_type {
        ...
        AgentType::Cursor => {
            ...
            // into the runtime env. It has no separate native provider file to
            // rewrite here.
        }
        AgentType::Codex => {
            let auth_path = codex_auth_json_path();
...
pub async fn acp_connect(
    ...
    // Resolve through the effective data dir so a custom `HOUHUB_DATA_DIR`
...
/// houhub-mcp later spawns a subagent.
///
...
pub(crate) async fn acp_update_agent_preferences_core(
    ...
    let opencode_auth_json = opencode_auth_json.map(|raw| raw.trim().to_string());
...
"
    ...
    fn remove_codex_catalog_key_is_format_preserving_and_no_op_when_absent() {
        let home = Path::new("/tmp/houhub-test-codex-home");
        let base = config_with_catalog_ref(crate::acp::codex_model_catalog::CATALOG_REL);
        let next = remove_codex_catalog_key(&base, home)
            .unwrap()
            .expect("HouHub-owned key was present");
        ...
        assert!(next.contains("[model_providers.houhub]"));
    ...
    /// The cleanup must only ever reclaim HouHub's OWN generated file. A catalog
    ...
    fn remove_codex_catalog_key_preserves_user_owned_references() {
        ...
        for foreign in [
            ...
            "~/my-catalog.json",
            "nested/Houhub-model-catalog.json",
            "  ",
        ] {
        ...
        // An absolute path naming HouHub's own file is HouHub-owned.
        let abs = home
            .join(crate::acp::codex_model_catalog::CATALOG_REL)
            .to_string_lossy()
            .into_owned();
        assert!(is_houhub_owned_catalog_ref(&abs, home));
        assert!(is_houhub_owned_catalog_ref(
    ...
    fn parse_grok_settings_reads_custom_model_and_session() {
        ...
        let settings = parse_grok_settings(toml);
        assert_eq!(settings.custom_model_id.as_deref(), Some("foo"));
        assert_eq!(settings.custom_base_url.as_deref(), Some("https://api/v1"));
        assert_eq!(settings.custom_api_key.as_deref(), Some("sk-1"));
        assert_eq!(settings.custom_api_backend.as_deref(), Some("responses"));
    ...
    fn apply_grok_custom_model_round_trips_through_parse() {
        ...
        assert_eq!(settings.custom_model_id.as_deref(), Some("my-grok"));
        assert_eq!(settings.custom_base_url.as_deref(), Some("https://gw/v1"));
        assert_eq!(settings.custom_api_key.as_deref(), Some("sk-x"));
        assert_eq!(settings.custom_api_backend.as_deref(), Some("responses"));
    ...
    fn apply_grok_session_compaction_clamps_and_removes() {
        let clamped = apply_grok_structured_config(
            "",
            &GrokStructuredConfig {
        ...
        .unwrap();
        assert_eq!(
            parse_grok_settings(&clamped).auto_compact_threshold_percent,
            Some(100)
        );

        let cleared = apply_grok_structured_config(
            "[session]\nauto_compact_threshold_percent = 70\n",
            &GrokStructuredConfig::default(),
        )
        .unwrap();
    ...
    fn launch_block_for(agent_dir: &Path, houhub_home: &Path, cwd: &Path) -> Option<String> {
        temp_env::with_var(
            "HOUHUB_HOME",
            Some(houhub_home.to_string_lossy().to_string()),
    ...
    fn gated_workspace(tmp: &Path, decision: Option<bool>) -> (PathBuf, PathBuf, PathBuf) {
        ...
        (ws, agent_dir, tmp.join("houhub-home"))
    }
    ...
    fn pi_launch_is_blocked_by_an_unacknowledged_ancestor_grant() {
        ...
        let blocked = launch_block_for(&agent_dir, &tmp.path().join("houhub-home"), &ws);
    ...
    fn pi_launch_proceeds_when_the_repo_ships_no_resources() {
        ...
        assert_eq!(
            launch_block_for(&agent_dir, &tmp.path().join("houhub-home"), &ws),
            None,
        );
    }
...
"#;
        ...
        assert_eq!(
            p_houhub.get("modelProvider").and_then(|v| v.as_str()),
            Some("houhub")
        );
        assert_eq!(
        ...
        assert_eq!(p_houhub.get("apiBaseUrl"), p_other.get("apiBaseUrl"));
        assert_ne!(p_houhub, p_other);
        assert_eq!(codex_config_projection_from_toml(houhub), p_houhub);
    ...
    fn unique_test_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("houhub-acp-{name}-{}", uuid::Uuid::new_v4()));
  ...
  "permissions": { "allow": ["Shell(ls)"], "deny": [] }
    ...
    fn shell_quote_arg_leaves_spacefree_windows_paths_unquoted() {
        ...
        let path = r"C:\Users\Administrator\AppData\Local\app.houhub\acp-binaries\uv-tool\windows-x86_64\uvx.exe";
    ...
    async fn hermes_setup_argvs_address_the_hermes_bin() {
        // Every recipe form must drive the wrapper's `hermes` bin. In the npx
        // form `--package` is load-bearing because the package's default bin is
        // `hermes-agent`, not the ACP CLI.
        let (setup, model) = hermes_setup_argvs().await;
