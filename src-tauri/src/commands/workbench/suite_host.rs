...
pub async fn workbench_open_suite_core(
    ...
    if let Some(existing) = app.get_webview_window(&validated.window_label) {
        ...
        let _ = existing.show();
        existing.maximize().map_err(|error| {
            AppCommandError::window("Failed to maximize Workbench suite", error.to_string())
        })?;
...
mod tests {
    ...
    fn validates_same_origin_project_and_deterministic_window() {
        ...
        assert_eq!(validated.canonical_url.host_str(), Some("project.example.test"));
        assert_eq!(validated.project_id, "project_1");
        assert_eq!(validated.suite_code, "creative_design_studio");
