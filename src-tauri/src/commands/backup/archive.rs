...
/// manifest itself), paths must be unique, and a houhub backup must carry the
...
mod tests {
    ...
    fn build_read_extract_verify_roundtrip() {
        ...
        b.add_file("db/houhub.db", &src.join("db.bin"), &cancel, &mut prog)
            .unwrap();
        b.add_dir(
            "uploads",
            &src.join("uploads"),
    ...
    fn extract_rejects_unmanifested_file() {
        ...
        let mut b = ArchiveBuilder::create(&zip_path).unwrap();
        b.add_file("db/houhub.db", &src, &cancel, &mut null_progress())
            .unwrap();
        ...
        // Manifest still lists only db/houhub.db.
        manifest.entries.retain(|e| e.path == "db/houhub.db");
        let out = dir.path().join("out");
