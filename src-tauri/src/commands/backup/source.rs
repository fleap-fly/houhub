...
//! user takes to work through the restore dialog. That is accepted in houhub's
...
pub const PREPARED_DIR: &str = ".houhub-restore-prepared";
...
mod tests {
    ...
    fn plaintext_archive(dir: &Path) -> PathBuf {
        ...
        let zip = dir.join("backup.houhub.zip");
    ...
    async fn encrypted_source_decrypts_once_under_the_data_dir() {
        ...
        let enc = dir.path().join("backup.houhubbak");
