...
pub(crate) async fn create_backup_core(
    ...
    let db_snapshot = work.path().join("houhub.db");
    ...
    let prefs_json = crate::paths::houhub_home_dir().join("preferences.json");
    ...
    let manifest = tokio::task::spawn_blocking(move || -> Result<BackupManifest, AppCommandError> {
        ...
        builder.add_file("db/houhub.db", &db_snapshot_c, &cancel_c, &mut prog)?;
        builder.add_dir(
            "uploads",
            &uploads_root,
            &is_excluded_upload,
...
pub(crate) async fn inspect_backup_core(
    ...
    // "compatible" for a backup that stage will reject (missing db/houhub.db,
...
/// Exclude upload staging dirs (`uploads/.tmp/`) and any houhub-internal
/// `.houhub-*` directory (restore staging / safety snapshots) from the archive.
...
mod tests {
    ...
    async fn backup_roundtrip_plaintext() {
        ...
        let dest = dir.path().join("backup.houhub.zip");
        ...
        assert!(dest.exists());
        assert!(manifest.entries.iter().any(|e| e.path == "db/houhub.db"));
        ...
        archive::extract_all(&dest, &out, &manifest, &cancel, &mut archive::null_progress())
            .unwrap();
        assert_eq!(count_folders(&out.join("db/houhub.db")).await, 1);
    }
    ...
    async fn backup_roundtrip_encrypted() {
        ...
        let dest = dir.path().join("backup.houhubbak");
    ...
    async fn backup_then_stage_then_apply_roundtrip() {
        ...
        std::fs::create_dir_all(&uploads).unwrap();
        let dest = src_dir.path().join("backup.houhub.zip");
        ...
        .await
        .unwrap();
        assert!(PathBuf::from(&staged.staging_dir).join("db/houhub.db").exists());
        assert!(restore_dir.path().join(PENDING_MARKER).is_file());

        // Apply on "startup" → live DB carries the two seeded folders. Inject
        // temp uploads/preferences paths so the test never touches ~/.houhub.
