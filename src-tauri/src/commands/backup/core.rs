...
pub(crate) async fn create_backup_core(
    ...
    let manifest = tokio::task::spawn_blocking(move || -> Result<BackupManifest, AppCommandError> {
        ...
        // Every houhub-owned section, straight off the shared table — see
...
mod tests {
    ...
    /// the real `MANAGED_SECTIONS` table without touching the user's `~/.houhub`.
    fn inputs<'a>(
    ...
    async fn backup_then_stage_then_apply_roundtrip() {
        ...
        // temp section roots so the test never touches ~/.houhub.
    ...
    /// `~/.houhub/acp-transcripts`, so this is the round-trip that used to lose
