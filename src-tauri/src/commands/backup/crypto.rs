...
pub const ENVELOPE_MAGIC: &[u8; 8] = b"HOUHUB01";
...
/// Cleartext header at the front of a `.houhubbak` file. Carries everything
...
/// Encrypt the plaintext ZIP at `src` into a `.houhubbak` envelope at `dest`.
...
/// Decrypt a `.houhubbak` envelope at `src` into a plaintext ZIP at `dest`.
...
mod tests {
    ...
    fn roundtrip(plain: &[u8]) {
        ...
        let enc = dir.path().join("out.houhubbak");
