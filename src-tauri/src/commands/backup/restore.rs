...
//!
//! The DB connection pool holds `houhub.db` open (WAL sidecars), so swapping it
...
//!    archive into `<data_dir>/.houhub-restore-staging/<op_id>/`, then write a
...
pub const PENDING_MARKER: &str = ".houhub-restore-pending.json";
/// Root for staged (extracted, verified, not-yet-applied) restore payloads.
pub const STAGING_DIR: &str = ".houhub-restore-staging";
/// Root for pre-restore safety snapshots of the previous live data.
pub const SAFETY_DIR: &str = ".houhub-restore-backup";
...
pub const EXPORT_TMP_DIR: &str = ".houhub-backup-tmp";
/// Transient dir (server mode) holding uploaded archives awaiting inspect/stage.
pub const UPLOAD_TMP_DIR: &str = ".houhub-restore-upload";
...
/// (`houhub-dev.db` on debug desktop builds), so the two are mapped explicitly
/// at swap time rather than assumed equal.
pub const DB_STAGING_NAME: &str = "houhub.db";
/// Per-unit swap progress, written inside the staging dir so a crashed swap
/// can be resumed exactly once per unit.
const SWAP_STATE_DIR: &str = ".houhub-swap-state";
...
const SNAPSHOT_MANIFEST: &str = ".houhub-snapshot.json";
...
struct PendingRestore {
    ...
    /// Which houhub-owned sections this restore is allowed to replace.
    ///
...
pub struct SafetySnapshot {
    /// Directory name under `.houhub-restore-backup/`.
    pub id: String,
    pub path: String,
...
/// paths so they never touch the real `~/.houhub`.
...
/// filesystem boundaries (HOUHUB_HOME / HOUHUB_DATA_DIR may differ).
...
mod tests {
    ...
    async fn legacy_manifest_without_sections_leaves_new_roots_untouched() {
        ...
        write_archive(
            &src,
            None,
            &[("db/houhub.db", b"NEW-DB"), ("uploads/a.txt", b"A")],
        );
