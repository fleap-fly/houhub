//! The single table of houhub-owned data sections a backup packs and a restore
//! swaps back in.
//!
...
//! sync by hand, and they drifted: `~/.houhub/acp-transcripts` — the ONLY copy
...
//!
//! This is an ALLOWLIST, not `~/.houhub` minus a denylist: a future
//! `~/.houhub/<secrets>` must not be swept into an archive just because nobody
...
pub struct ManagedSection {
    ...
    /// resolves through `paths::*` (which honor `HOUHUB_HOME` /
    /// `HOUHUB_DATA_DIR`).
...
pub const MANAGED_SECTIONS: &[ManagedSection] = &[
    ManagedSection {
        ...
        live_path: |_| crate::paths::houhub_uploads_root(),
    },
    ManagedSection {
        ...
        kind: SectionKind::Dir,
        policy: SectionPolicy::AlwaysReplace,
        live_path: |_| crate::paths::houhub_acp_transcripts_root(),
    },
    ManagedSection {
        ...
        kind: SectionKind::Dir,
        policy: SectionPolicy::AlwaysReplace,
        live_path: |_| crate::paths::houhub_turn_timings_root(),
    },
    ManagedSection {
        ...
        kind: SectionKind::Dir,
        policy: SectionPolicy::AlwaysReplace,
        live_path: |_| crate::paths::houhub_backgrounds_root(),
    },
    ManagedSection {
        id: "pets",
        kind: SectionKind::Dir,
        policy: SectionPolicy::AlwaysReplace,
        live_path: |_| crate::paths::houhub_pets_root(),
    },
    ManagedSection {
        ...
        // and does NOT honor `HOUHUB_HOME`/`HOUHUB_DATA_DIR`; changing that would
    ...
    ManagedSection {
        ...
        kind: SectionKind::File,
        policy: SectionPolicy::ReplaceIfPresent,
        live_path: |_| crate::paths::houhub_home_dir().join("preferences.json"),
    },
];

/// `db/houhub.db` is deliberately NOT a managed section: its live filename
/// varies (`houhub-dev.db` on debug desktop builds) and it is captured with
...
/// Exclude houhub-internal scaffolding from any packed section: upload staging
/// (`uploads/.tmp/`) and any `.houhub-*` directory (restore staging / safety
