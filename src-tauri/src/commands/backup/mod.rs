...
/// The file is not an houhub backup, or its layout version is too new.
pub(crate) fn unknown_format_error() -> AppCommandError {
    AppCommandError::invalid_input("Not a recognized houhub backup archive")
...
pub(crate) fn newer_version_error(backup_version: &str, app_version: &str) -> AppCommandError {
    ...
    AppCommandError::invalid_input("Backup was created by a newer version of houhub")
...
mod tauri_commands {
    ...
    pub async fn backup_create(
        ...
        let inputs = BackupInputs {
            conn: &db.conn,
            data_dir: &data_dir,
            uploads_root: crate::paths::houhub_uploads_root(),
