...
/// Windows. A PATH lookup here searches the PATH of houhub's own process, while
...
/// PATH edited — or a shell installed — after houhub started. Both answer "which
/// `pwsh.exe`", and they differ only when those two PATHs name different
/// directories; what houhub passes to either spawner is the stored string
...
/// shell that isn't installed is still what houhub would try to spawn, and
...
/// Persist, apply, and broadcast the default shell through one shared path.
/// This keeps desktop and hosted workspace updates in sync with connections
...
mod tests {
    ...
    /// `ERR_INVALID_URL` while houhub's own reqwest calls kept working.
    #[test]
    ...
    fn the_reported_shell_follows_the_selection() {
        ...
        // Not installed is still what houhub would try to spawn — echoing it
