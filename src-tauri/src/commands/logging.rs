...
/// global level and per-target overrides, plus whether `HOUHUB_LOG`/`RUST_LOG`
...
pub async fn set_log_settings_core(
    ...
    app_metadata_service::upsert_value(conn, LOGGING_LEVEL_KEY, &serialized)
        .await
        .map_err(AppCommandError::from)?;
    // The env var (HOUHUB_LOG/RUST_LOG) owns the live level when set; persist the
...
mod tests {
    ...
    fn read_log_file_reads_full_and_tail() {
        ...
        temp_env::with_vars(
            [
                ("HOUHUB_HOME", None::<&str>),
                ("HOUHUB_DATA_DIR", Some(tmp.path().to_str().unwrap())),
            ],
            || {
                let logs = crate::paths::houhub_logs_root();
                std::fs::create_dir_all(&logs).unwrap();
                std::fs::write(logs.join("houhub.2026-01-01.log"), b"0123456789").unwrap();
                assert_eq!(
                    read_log_file_core("houhub.2026-01-01.log", None).unwrap(),
                    "0123456789"
                );
                // Capped read returns the newest tail.
                assert_eq!(
                    read_log_file_core("houhub.2026-01-01.log", Some(4)).unwrap(),
                    "6789"
                );
            },
        );
