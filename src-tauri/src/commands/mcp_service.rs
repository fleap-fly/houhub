//! Health report for the houhub-mcp service, and the one action that can fix
//! it from the UI.
//!
//! "The houhub-mcp service" is really three things stacked, and a session loses
//! its houhub tools if ANY of them is missing:
//!
//!   1. the **companion binary** on disk — `houhub-mcp`, which the agent CLI
//!      spawns as a stdio MCP server (see `acp::connection::inject_houhub_mcp`);
...
//! module collapses the three into one [`HouHubMcpServiceStatus`] the status-bar
//! indicator renders, with a single headline [`HouHubMcpServiceState`] so the
//! popover can offer exactly one next step.
//!
//! Only #2 is startable from here — #1 needs a reinstall. #3 is a settings
//! write, which [`set_houhub_mcp_tool_group_core`] performs through the very
...
pub enum HouHubMcpServiceState {
    /// The broker socket isn't answering. Companions would launch and fail to
    /// reach houhub. Fixable here — see [`start_houhub_mcp_service_core`].
    Stopped,
    /// The socket is fine but the `houhub-mcp` binary isn't on disk, so nothing
    /// gets injected into an agent's MCP config. Needs a reinstall or
    /// `HOUHUB_MCP_BIN`; there is nothing to start.
    Unavailable,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct HouHubMcpServiceStatus {
    pub state: HouHubMcpServiceState,
    ...
    /// Resolved `houhub-mcp` path, or `None` when the lookup came up empty.
    pub binary_path: Option<String>,
    /// Tool groups and their current switches, in companion `--features` order.
    pub tool_groups: Vec<HouHubMcpToolGroup>,
...
pub struct HouHubMcpStatusSources<'a> {
...
pub async fn houhub_mcp_service_status_core(
    sources: HouHubMcpStatusSources<'_>,
) -> HouHubMcpServiceStatus {
    ...
    let binary_path = crate::acp::connection::locate_houhub_mcp_binary()
    ...
    let tool_groups = vec![
        HouHubMcpToolGroup::group("delegation", delegation_cfg.enabled),
        HouHubMcpToolGroup::group("feedback", sources.feedback.is_enabled().await),
        HouHubMcpToolGroup::group("ask", sources.question.is_enabled().await),
        HouHubMcpToolGroup::group("sessions", sources.session_info.is_enabled().await),
        HouHubMcpToolGroup::group("automations", authoring_cfg.automations_enabled),
        HouHubMcpToolGroup::group("taskboard", authoring_cfg.work_tasks_enabled),
        HouHubMcpToolGroup::group("browser", browser_cfg.enabled),
    ...
    let state = if !listening {
        HouHubMcpServiceState::Stopped
    } else if binary_path.is_none() {
        HouHubMcpServiceState::Unavailable
    } else if !any_group_enabled {
        HouHubMcpServiceState::Disabled
    } else {
        HouHubMcpServiceState::Running
    };
...
pub async fn set_houhub_mcp_tool_group_core(
    conn: &DatabaseConnection,
    targets: HouHubMcpToolGroupTargets<'_>,
    ...
    match key {
        ...
        other => {
            return Err(AppCommandError::configuration_invalid(format!(
                "unknown houhub-mcp tool group: {other}"
            )))
        }
    }
    Ok(())
...
pub async fn start_houhub_mcp_service_core() -> Result<(), AppCommandError> {
    let Some(handle) = service::current() else {
        return Err(AppCommandError::configuration_invalid(
            "houhub-mcp broker socket is not managed by this process",
        ));
    };
    handle
        .ensure_running()
...
pub async fn get_houhub_mcp_service_status(
    ...
    {
        Ok(houhub_mcp_service_status_core(HouHubMcpStatusSources {
            broker: broker.inner(),
            tokens: tokens.inner(),
...
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub async fn start_houhub_mcp_service() -> Result<(), AppCommandError> {
...
mod tests {
    ...
    impl Fixture {
        ...
        fn sources(&self) -> HouHubMcpStatusSources<'_> {
            HouHubMcpStatusSources {
                broker: &self.broker,
                tokens: &self.tokens,
        ...
        async fn status(&self) -> HouHubMcpServiceStatus {
            houhub_mcp_service_status_core(self.sources()).await
        }
    ...
    async fn reports_stopped_and_unstartable_without_an_installed_handle() {
        ...
        assert_eq!(status.state, HouHubMcpServiceState::Stopped);
        assert!(!status.listening);
        assert!(!status.can_start);
        assert!(start_houhub_mcp_service_core().await.is_err());
    }
    ...
    async fn enabled_groups_do_not_override_a_dead_socket() {
        ...
        assert_eq!(f.status().await.state, HouHubMcpServiceState::Stopped);
    }
    ...
    async fn browser_eval_is_reported_under_the_group_it_belongs_to() {
        let f = Fixture::new();
        let row = |status: &HouHubMcpServiceStatus, key: &str| {
            status
                .tool_groups
                .iter()
                .find(|g| g.key == key)
