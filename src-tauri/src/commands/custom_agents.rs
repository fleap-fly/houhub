...
//! supplying ACP-registry information rather than agents houhub ships support
//! for.
//!
...
pub struct CustomAgentInfo {
    ...
    /// that decides whether houhub-mcp rides along on `session/new`.
...
/// Fetch the public ACP registry, annotated with which entries houhub already
...
pub async fn acp_add_registry_agent_core(
    ...
    {
        return Err(AcpError::protocol(format!(
            "{registry_id} is already built into houhub"
        )));
    }
...
/// Largest icon houhub will inline. Registry marks are 650 B–5 KB SVGs, so this
...
mod tests {
    ...
    fn skills_dir_normalization_expands_home_and_rejects_relative_paths() {
        ...
        temp_env::with_var("HOME", Some("/tmp/houhub-skills-home"), || {
            let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
