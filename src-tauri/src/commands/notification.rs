...
/// most users have never granted, so nothing appeared and the houhub switches
/// the user could see governed nothing. Asking for our own identifier works
/// whenever houhub is registered with LaunchServices (the usual case: it is
...
/// necessarily this app's own — sending the user to houhub's page while the OS
