...
//! houhub's composer shows for a DeepSeek session. Absent, the session inherits
...
//! houhub's own process env, not the per-agent `env_json`, which only reaches
//! the spawned child.
...
pub struct DeepSeekCatalogModel {
    ...
    ///
    /// Kept in the shape so a document an older houhub wrote still PARSES: the
    ...
    /// trip the unknown-key guard and refuse to edit a document houhub itself
    /// produced.
...
/// [`DeepSeekCatalogModel::image_detail`]. It is a key houhub once wrote, so
    ...
    for entry in entries {
        ...
        for key in entry.keys() {
            ...
            if unknown {
                ...
                return Err(format!(
                    "`{SECTION_KEY}.{MODELS_KEY}` uses a field houhub does not know \
...
/// section houhub writes is one the adapter will actually load. Returns the
...
/// one is `0600` — it is created by houhub, so its mode is houhub's
...
fn patch_settings_models(
    ...
    // A document houhub cannot parse is one it must not rewrite: the splice
...
mod tests {
    ...
    fn refuses_to_edit_entries_carrying_fields_it_would_drop() {
        // A save rewrites the whole block, so a field houhub does not model
        ...
        let doc = concat!(
            ...
            // Retired upstream, but still a key houhub itself once wrote — so it
    ...
    fn a_stored_image_detail_is_reported_shown_without_it_and_dropped_on_save() {
        // The full repair path for a document an older houhub wrote. Each step
    ...
    fn replaces_a_block_written_flush_left_under_its_key() {
        ...
        // document houhub (or a script) produced looks like this.
        let existing = "llm-deepseek:\n  models:\n  - id: old\n  thinking: enabled\n";
        let patched = patch_settings_models(existing, Some(&[model("new")])).expect("patch");
        assert_eq!(models_of(&patched)[0].id, "new");
        assert!(patched.contains("  thinking: enabled\n"));
