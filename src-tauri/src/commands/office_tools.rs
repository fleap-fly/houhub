...
//! (`~/.houhub/skills/<id>/`) used by built-in experts. Enabling a skill for
...
struct SkillDef {
    /// Canonical skill identity used throughout HouHub: the central-store
    /// directory name (`~/.houhub/skills/<id>/`) and the agent invocation
...
fn officecli_runtime_dependency_hint(stderr: &str) -> Option<String> {
    ...
    if missing_icu {
        return Some(
            "officecli could not start: the server is missing the ICU library its embedded .NET runtime needs. Install it in the runtime image and restart (Debian/Ubuntu: `apt-get install -y libicu72`; Alpine: `apk add icu-libs`), or upgrade to a houhub image that already includes it."
                .to_string(),
        );
    }
    if lower.contains("error while loading shared libraries") {
        return Some(
            "officecli could not start: a required system library is missing on the server. Install the library named in the error below and restart."
                .to_string(),
        );
    }
    None
        ...
        for def in skill_defs() {
            ...
            for dir in &dirs {
                ...
                let should_remove = match state {
                    ExpertLinkState::LinkedToHouhub => true,
    ...
    match create_link_raw(&central, &link_path) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AlreadyExists => {
            match classify_link(&link_path, &central) {
                ExpertLinkState::LinkedToHouhub => {}
...
/// HouHub's own process (not the agent's command sandbox), so it is unaffected
