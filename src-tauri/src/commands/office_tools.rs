...
/// already-running processes (HouHub and the agents it spawns). Returns `None`
/// once `officecli` is on `PATH` (the injection then self-deactivates) or when
/// it isn't installed. Also closes the latent gap where a GUI-launched HouHub on
    ...
    if missing_icu {
        return Some(
            "officecli could not start: the server is missing the ICU library its \
             ...
             or upgrade to a HouHub image that already includes it."
                .to_string(),
        );
    }
...
//
// HouHub installs OfficeCLI by running the vendor's official installer script —
