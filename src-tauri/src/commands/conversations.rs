...
pub(crate) async fn list_all_conversations_core_with_codex_titles(
    conn: &sea_orm::DatabaseConnection,
    emitter: &EventEmitter,
    chat_channel_manager: &crate::chat_channel::manager::ChatChannelManager,
    options: ListAllConversationsOptions,
    ...
    for (at, summary) in summaries {
        ...
        let entry = groups.entry(key.clone()).or_insert_with(|| {
            ...
            GroupAcc {
                ...
                exists_in_houhub: row.map(|r| !r.deleted).unwrap_or(false),
    ...
    let mut folders: Vec<ScanFolder> = groups
        ...
        .map(|mut g| {
            ...
            ScanFolder {
                path: g.path,
                name: g.name,
                exists_in_houhub: g.exists_in_houhub,
...
/// Broadcast and propagate title changes discovered outside HouHub (for
...
/// vs canonical `HOUHUB_DATA_DIR` naming the same storage) still matches — a live
...
pub(crate) async fn gc_orphan_chat_dirs_core_with_threshold(
    ...
    // symlinked vs canonical `HOUHUB_DATA_DIR` pointing at the same storage),
...
mod tests {
    ...
    fn inject_delegation_meta_populates_completed_child() {
        ...
        let inner = meta
            .get("houhub.delegation")
            .expect("houhub.delegation key");
    ...
    fn inject_delegation_meta_preserves_live_broker_meta() {
        ...
        let pre_existing = serde_json::json!({ "houhub.delegation": { "status": "running", "child_conversation_id": 999 } });
    ...
    async fn get_folder_conversation_core_injects_meta_for_real_child() {
        ...
        let folder_id = seed_folder(&db, "/tmp/houhub-inject-test").await;
    ...
    async fn create_conversation_core_happy_path() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-conv-test-1").await;
    ...
    async fn chat_folders_excluded_from_user_facing_lists_but_in_all_details() {
        let db = fresh_in_memory_db().await;
        let data_dir = tempfile::tempdir().expect("tempdir");
        let normal_id = seed_folder(&db, "/tmp/houhub-chat-list-test").await;
        let chat_id =
            create_chat_conversation_core(&db.conn, data_dir.path(), AgentType::Codex, None, None)
                .await
                .expect("chat")
    ...
    async fn list_all_conversations_core_syncs_codex_index_title_before_search() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-codex-index").await;
    ...
    async fn list_all_conversations_core_preserves_locked_codex_title() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-codex-locked").await;
        let row = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::Codex,
    ...
    async fn list_all_conversations_core_keeps_title_when_codex_index_is_missing() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-codex-no-index").await;
        let row = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::Codex,
    ...
    async fn list_all_conversations_core_returns_persisted_rows_when_title_sync_fails() {
        ...
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-codex-sync-failure").await;
        let row = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::Codex,
    ...
    async fn scan_importable_sessions_syncs_title_and_notifies_clients() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-scan-codex-title-sync").await;
        let row = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::Codex,
        ...
        let mut summary = scan_summary(
            "scan-session",
            AgentType::Codex,
            Some("/tmp/houhub-scan-codex-title-sync"),
            at(0),
        );
    ...
    async fn save_opened_tabs_core_persists_only_conversation_tabs_and_bumps_version() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tabs-test").await;
    ...
    async fn save_opened_tabs_core_rejects_stale_version_without_emitting() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tabs-stale").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
    ...
    async fn cleanup_tabs_for_deleted_conversation_removes_tab_and_emits() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tab-conv-del").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
        save_opened_tabs_core(
    ...
    {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tab-conv-del-noop").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
    ...
    async fn remove_folder_from_workspace_cleans_tabs_and_emits() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-folder-remove-tabs").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
        save_opened_tabs_core(
    ...
    async fn stale_save_after_conversation_cleanup_is_rejected_no_resurrection() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tab-cleanup-race").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
        let c2 = create_conversation_core(&db.conn, folder_id, AgentType::Codex, None)
    ...
    async fn stale_save_after_folder_removal_is_rejected_no_resurrection() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-folder-remove-race").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
        let saved = save_opened_tabs_core(
    ...
    async fn stale_save_referencing_deleted_conversation_is_rejected_no_ghost() {
        ...
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-tab-zero-row-race").await;
        let c1 = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("c1");
        let c2 = create_conversation_core(&db.conn, folder_id, AgentType::Codex, None)
    ...
    async fn update_conversation_status_core_invalid_string_errors() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-status-test").await;
    ...
    async fn update_conversation_title_core_roundtrip() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-title-test").await;
    ...
    async fn delete_conversation_core_soft_deletes() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-delete-test").await;
    ...
    async fn list_child_conversations_core_returns_empty_for_no_parent() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-children-empty").await;
    ...
    async fn list_child_conversations_core_returns_only_matching_children() {
        ...
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-list-children-match").await;
        ...
        assert_eq!(ids, vec![child_ids[1], child_ids[0]]);
    }
    ...
    async fn emit_conversation_upsert_broadcasts_full_root_summary() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-sync-upsert").await;
    ...
    async fn emit_conversation_upsert_carries_new_status_after_update() {
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-sync-status").await;
        let id = create_conversation_core(&db.conn, folder_id, AgentType::Codex, None)
            .await
            .expect("create");
    ...
    async fn emit_conversation_upsert_on_soft_deleted_row_is_silent() {
        ...
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-sync-deleted-silent").await;
    ...
    async fn emit_conversation_upsert_broadcasts_delegation_child_with_parent() {
        ...
        use crate::acp::delegation::spawner::DelegationLink;
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-sync-child-broadcast").await;
        let parent_id = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("parent");
        let child = conversation_service::create_with_delegation(
    ...
    async fn delete_child_re_emits_parent_for_child_count_convergence() {
        ...
        use crate::acp::delegation::spawner::DelegationLink;
        let db = fresh_in_memory_db().await;
        let folder_id = seed_folder(&db, "/tmp/houhub-delete-child-reemit").await;
        let parent_id = create_conversation_core(&db.conn, folder_id, AgentType::ClaudeCode, None)
            .await
            .expect("parent");
        let child = conversation_service::create_with_delegation(
    ...
    fn scan_groups_normalized_path_variants_into_one_folder() {
        ...
        assert!(!folder.exists_in_houhub);
