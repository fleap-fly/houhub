...
//! `houhub.exe`, and the rolling log simply stops mid-file: no message, no
//! location, no backtrace, nothing that names the code that failed. A report of
//! exactly that shape (0.30.6, faulting module `houhub.exe`, exception
...
/// `houhub_lib::logging` to `Off` so the logging stack cannot log about itself,
/// and a record emitted from `houhub_lib::logging::panic_hook` would inherit
...
pub const PANIC_TARGET: &str = "houhub_lib::panic";
...
/// Absent in the stderr-only modes (`houhub-mcp`, the `--supervise` supervisor,
...
mod tests {
    ...
    /// The record's target must escape the `houhub_lib::logging=off` backstop
    ...
    fn sample() -> PanicReport {
        PanicReport {
            ...
            backtrace: "0: houhub_lib::acp::idle_sweep::sweep".into(),
        }
    }
    ...
    fn the_append_budget_stops_the_file_growing_without_end() {
        ...
        let path = dir
            .path()
            .join(crate::logging::budget::daily_file_name("houhub", "log", now));
        ...
        for _ in 0..(MAX_APPEND_BYTES / line.len() + 100) {
            match reserve_append(&spent, line.len()) {
                ...
                Reservation::Granted => append_line(dir.path(), "houhub", "log", now, &line),
                Reservation::Last => {
                    append_line(dir.path(), "houhub", "log", now, &line);
                    notices += 1;
                }
            }
        }
    ...
    fn append_line_creates_todays_file_and_appends_to_it() {
        let dir = tempfile::tempdir().unwrap();
        let now = chrono::Utc::now();
        append_line(dir.path(), "houhub", "log", now, "first\n");
        append_line(dir.path(), "houhub", "log", now, "second\n");

        let name = crate::logging::budget::daily_file_name("houhub", "log", now);
    ...
    fn append_line_swallows_a_bad_destination_instead_of_panicking() {
        ...
        append_line(&missing, "houhub", "log", chrono::Utc::now(), "dropped\n");
    }
