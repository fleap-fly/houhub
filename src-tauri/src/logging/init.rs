...
//!    the default level from the `logging.level` KV value (unless `HOUHUB_LOG` /
...
/// `HOUHUB_LOG` override. Each entry is the MOST VERBOSE level that target may
...
///   no handler") on the per-message hot path at the DEFAULT level. houhub keeps
...
///   crate-level ceiling, so setting `HOUHUB_LOG` to
...
///   from the Settings UI or `HOUHUB_LOG` loses to this entry at any module
...
///   WARN and survive.
/// - `houhub_lib::logging` → `Off` — the logging stack must not log about itself
...
const TARGET_BACKSTOPS: &[(&str, LogLevel)] = &[
    ...
    ("houhub_lib::logging", LogLevel::Off),
];
...
/// `HOUHUB_LOG='tungstenite::handshake::client[{message}]=trace'` outranks the
...
/// expression like `houhub_lib=info` enables nothing else — `EnvFilter::builder()`
...
fn env_level_for_target(expr: &str, target: &str) -> LogLevel {
    ...
    for part in expr.split(',') {
        ...
        let (directive_target, level) = match part.split_once('=') {
            ...
            // a directive `EnvFilter` never accepted. `HOUHUB_LOG=off,agent_client_protocol=bogus`
...
fn is_valid_target(target: &str) -> bool {
    if target.is_empty()
        || target == "houhub_lib::logging"
        || target.starts_with("houhub_lib::logging::")
    {
        return false;
    }
    target
...
/// overridable via `HOUHUB_LOG_MAX_FILES`.
///
...
fn file_retention() -> usize {
    std::env::var("HOUHUB_LOG_MAX_FILES")
        .ok()
...
/// `HOUHUB_LOG` then `RUST_LOG`. Single source of truth for env-level precedence,
/// so the startup filter and the env-lock checks can't diverge — e.g. an empty
/// `HOUHUB_LOG` must not mask a valid `RUST_LOG` (a `var().or_else(var)` would
/// treat the empty `Ok("")` as present and skip `RUST_LOG`).
fn env_level_override() -> Option<String> {
    ["HOUHUB_LOG", "RUST_LOG"].iter().find_map(|key| {
        std::env::var(key)
            .ok()
...
/// Whether an explicit `HOUHUB_LOG` / `RUST_LOG` is set (and non-empty). When
...
fn build_subscriber(
    ...
    // houhub-mcp stdout is the JSON-RPC channel, and elsewhere the migrated
    ...
    // the server, `houhub-mcp`, the supervisor and the credential helper, and it
...
/// Phase 1 for the desktop `houhub` binary: stderr + file (`houhub.<date>.log`) +
/// buffer. Default level is env-or-`info`; [`apply_persisted_level`] refines it
/// once the DB is open.
pub fn init_desktop() -> LogGuard {
    init_with_file("houhub")
}

/// Phase 1 for `houhub-server`: stderr + file (`houhub-server.<date>.log`) +
/// buffer.
pub fn init_server() -> LogGuard {
    init_with_file("houhub-server")
}

fn init_with_file(prefix: &str) -> LogGuard {
    let dir = crate::paths::houhub_logs_root();
...
/// `houhub-mcp`, the `--supervise` supervisor, and the `--credential-helper`
...
///
/// stderr (not stdout) is mandatory: in `houhub-mcp` and the credential helper,
...
/// Phase 1 for `houhub-mcp`. See [`init_stderr_only`].
pub fn init_mcp() -> LogGuard {
    init_stderr_only()
}

/// Phase 2: override the default level from the persisted `logging.level` KV
/// value, unless an explicit `HOUHUB_LOG` / `RUST_LOG` is set (env wins). No-op
...
mod tests {
    ...
    fn build_env_filter_includes_per_target_directives() {
        ...
        assert!(
            rendered.contains("houhub_lib::acp=debug"),
            "missing acp override: {rendered}"
        );
        assert!(
            rendered.contains("houhub_lib::web=warn"),
            "missing web override: {rendered}"
        );
        assert!(
            rendered.contains("houhub_lib::logging=off"),
    ...
    fn build_env_filter_rejects_invalid_and_logging_targets() {
        let settings = LogSettings {
            ...
            targets: vec![
                ...
                TargetDirective {
                    target: "houhub_lib::logging".into(), // backstop override attempt → dropped
                    level: LogLevel::Trace,
                },
                TargetDirective {
                    target: "houhub_lib::acp".into(),
        ...
        let rendered = build_env_filter(&settings).to_string();
        assert!(rendered.contains("houhub_lib::acp=debug"), "{rendered}");
        assert!(!rendered.contains("bad-target"), "{rendered}");
        // The logging module stays off; the override attempt is dropped.
        assert!(rendered.contains("houhub_lib::logging=off"), "{rendered}");
        assert!(
            !rendered.contains("houhub_lib::logging=trace"),
            "{rendered}"
        );
    }
    ...
    fn a_global_trace_cannot_capture_the_websocket_handshake_dump() {
        ...
        assert!(
            ...
            "HOUHUB_LOG=trace re-opened the handshake dump: {env}"
        );
    }
    ...
    fn the_handshake_dump_cannot_be_reopened_even_by_naming_its_target() {
        for asked in [
            ...
            assert!(
                ...
                "HOUHUB_LOG={asked} reopened the dump: {rendered}"
            );
        }
    ...
    fn the_file_sink_skips_only_the_record_the_panic_hook_already_wrote() {
        ...
        for other in [
            "houhub_lib::acp::connection",
            "houhub_lib::panic_hook",
            "houhub_lib::panicky",
            "houhub_lib",
            "panic",
        ] {
    ...
    fn the_denial_layer_drops_the_dump_and_nothing_else() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::trace!(target: "houhub_lib::acp", "unrelated");
        });
        ...
        assert!(
            seen.iter().any(|t| t == "houhub_lib::acp"),
    ...
    fn the_file_layer_alone_loses_the_record_the_hook_already_wrote() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info!(target: "houhub_lib::acp", "ordinary event");
        });
        ...
        assert_eq!(
            seen.iter()
                .filter(|(_, target)| target == "houhub_lib::acp")
                .count(),
            2,
    ...
    fn env_override_appends_backstops_so_debug_cannot_reopen_them() {
        // The explicit RUST_LOG/HOUHUB_LOG path bypasses build_env_filter, so it
        ...
        assert_eq!(
            ...
            "debug,kill_tree=warn,agent_client_protocol=warn,tungstenite=debug,\
             tungstenite::handshake::client=debug,sqlx::query=warn,houhub_lib::logging=off"
        );
    ...
    fn off_means_off_and_backstops_never_raise_verbosity() {
        ...
        assert_eq!(
            ...
            "OFF,kill_tree=off,agent_client_protocol=off,tungstenite=off,\
             tungstenite::handshake::client=off,sqlx::query=off,houhub_lib::logging=off",
    ...
    fn env_override_clamps_per_target_not_just_bare_levels() {
        ...
        // doesn't name.
        let compound = env_override_directives("off,houhub_lib::acp=trace");
        assert!(compound.contains("agent_client_protocol=off"), "{compound}");
        assert!(!compound.contains("agent_client_protocol=warn"), "{compound}");
        let error_compound = env_override_directives("error,houhub_lib::acp=info");
        ...
        let targeted = env_override_directives("houhub_lib::acp=info");
    ...
    fn is_valid_target_grammar() {
        assert!(is_valid_target("houhub_lib::acp"));
        assert!(is_valid_target("houhub_lib::acp::delegation"));
        ...
        assert!(!is_valid_target("houhub_lib::logging"));
        assert!(!is_valid_target("houhub_lib::logging::hub"));
    }
    ...
    fn env_level_override_precedence() {
        ...
        temp_env::with_vars(
            [("HOUHUB_LOG", None::<&str>), ("RUST_LOG", None::<&str>)],
            || {
                assert_eq!(env_level_override(), None);
                assert!(!env_level_is_set());
            },
        );
        // Empty HOUHUB_LOG must NOT mask a valid RUST_LOG (the bug Codex caught).
        temp_env::with_vars(
            [("HOUHUB_LOG", Some("")), ("RUST_LOG", Some("debug"))],
            || {
        ...
        temp_env::with_vars([("HOUHUB_LOG", Some("  ")), ("RUST_LOG", Some(""))], || {
            assert_eq!(env_level_override(), None);
            assert!(!env_level_is_set());
        });
        // HOUHUB_LOG wins when both are non-empty.
        temp_env::with_vars(
            [("HOUHUB_LOG", Some("trace")), ("RUST_LOG", Some("debug"))],
            || {
