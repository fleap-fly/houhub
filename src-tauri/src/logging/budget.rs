...
pub const MAX_BYTES_ENV: &str = "HOUHUB_LOG_MAX_BYTES";
...
/// push bypasses the subscriber, so the `houhub_lib::logging=off` backstop can't
        ...
        if let Some(hub) = crate::logging::hub::log_hub() {
            hub.record(crate::logging::hub::LogRecord {
                ...
                timestamp_ms: std::time::SystemTime::now()
                    ...
                    .unwrap_or(0),
                level: "WARN",
                target: "houhub_lib::logging".to_string(),
                message,
...
mod tests {
    ...
    fn daily_file_name_matches_the_appenders_scheme() {
        ...
        assert_eq!(daily_file_name("houhub", "log", day), "houhub.2026-09-09.log");
        assert_eq!(
            daily_file_name("houhub-server", "log", day),
            "houhub-server.2026-09-09.log"
        );
    }
    ...
    fn resume_point_measures_todays_file_and_ignores_other_days() {
        ...
        std::fs::write(dir.path().join(format!("houhub.{today}.log")), vec![b'x'; 77]).unwrap();
        // A different day's file, and an unrelated file, must not be counted.
        std::fs::write(dir.path().join("houhub.1999-01-01.log"), vec![b'x'; 5000]).unwrap();
        std::fs::write(dir.path().join("houhub-server.log"), vec![b'x'; 5000]).unwrap();

        let (day, existing) = resume_point(dir.path(), "houhub", "log");
        ...
        let (_, none) = resume_point(dir.path(), "houhub-server", "log");
        assert_eq!(none, 0);
    }
