...
pub struct SystemProxySettings {
    ...
    /// houhub always adds (see [`crate::network::proxy::no_proxy_env_value`]).
...
pub struct SystemTerminalSettings {
    ...
    /// text.
    ///
    /// Off by default, and deliberately so: the only lever houhub has is the
    /// AGENT process's env (the agent runs its own bash tool in-process — houhub
    ...
    /// terminfo lookup both need — colors the output houhub renders AND the
...
/// is what makes the other two discoverable: houhub has always hidden to tray,
