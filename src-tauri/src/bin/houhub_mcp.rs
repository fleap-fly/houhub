//! `houhub-mcp` — the per-launch stdio MCP companion that an agent CLI runs
//! to surface houhub's tools to its LLM: the multi-agent delegation tools
...
//!
//! The agent's MCP config (injected by houhub via `load_mcp_servers_for_agent`)
...
//! `houhub_lib::acp::delegation::{companion, transport}` so it's
...
use houhub_lib::acp::delegation::companion::{
...
use houhub_lib::acp::delegation::parent_watcher::{wait_for_parent_exit, DEFAULT_POLL_INTERVAL};
...
struct Args {
    ...
    socket_path: String,
    token: String,
    /// Optional PID of the houhub / houhub-server process that owns this
    /// session. When set, houhub-mcp exits as soon as the parent is gone so
...
async fn main() -> ExitCode {
    ...
    let _log_guard = houhub_lib::logging::init::init_mcp();

    let args = match parse_args() {
        Ok(a) => a,
        Err(e) => {
            let _ = writeln!(std::io::stderr(), "houhub-mcp: {e}");
        ...
        tokio::select! {
            ...
            line_result = lines.next_line() => {
                let line = match line_result {
                    ...
                    Err(e) => {
                        let _ = writeln!(std::io::stderr(), "houhub-mcp: read stdin: {e}");
                ...
                match action {
                    LineAction::Respond(resp) => {
                        if let Err(e) = write_response(&stdout, &resp).await {
                            let _ = writeln!(std::io::stderr(), "houhub-mcp: write stdout: {e}");
                            return ExitCode::from(1);
                        }
                    }
