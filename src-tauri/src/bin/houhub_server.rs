...
use houhub_lib::app_state::AppState;
use houhub_lib::web::event_bridge::{EventEmitter, WebEventBroadcaster};
use houhub_lib::web::{
...
fn main() -> ExitCode {
    ...
    houhub_lib::update::runtime::prime_self_exe();
    ...
    // lifecycle (PID 1 in Docker). It spawns `houhub-server` without this
    ...
    if args.iter().any(|a| a == "--supervise") {
        ...
        let _log_guard = houhub_lib::logging::init::init_stderr_only();
        houhub_lib::supervise::run();
    }
    ...
    if args.iter().any(|a| a == "--credential-helper") {
        ...
        let _log_guard = houhub_lib::logging::init::init_stderr_only();
        houhub_lib::git_credential::run_credential_helper();
        return ExitCode::SUCCESS;
    }
    ...
    houhub_lib::process::ensure_node_in_path();
    houhub_lib::process::ensure_user_npm_prefix_in_path();

    // Resolve and pin `HOUHUB_DATA_DIR` before any threads exist.
    //
    ...
    //    find `tokens.json`. A relative `HOUHUB_DATA_DIR=data` would
    ...
    //    `paths::houhub_uploads_root`, `paths::houhub_pets_root`,
    ...
    //    `dirs::data_dir()/houhub` but uploads under `~/.houhub/uploads`,
    ...
    let resolved_data_dir = std::env::var("HOUHUB_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_data_dir());
    let resolved_data_dir = houhub_lib::git_credential::absolutize(&resolved_data_dir);
    std::env::set_var("HOUHUB_DATA_DIR", &resolved_data_dir);

    // Install the logging subscriber now that HOUHUB_DATA_DIR is pinned (the logs
    // dir resolves from it). Doing it here — before the first server diagnostic
    // below — captures even the HOUHUB_HOME warning and the FATAL upload-quota
    // abort. Hold the guard for the whole process so buffered file lines flush
    // on a graceful exit.
    let _log_guard = houhub_lib::logging::init::init_server();

    // `HOUHUB_HOME` overrides `HOUHUB_DATA_DIR` for uploads/pets inside
    // `paths::houhub_*_root` (legacy `~/.houhub/` layout). If both are set
    ...
    if let Some(home) = std::env::var_os("HOUHUB_HOME").filter(|s| !s.is_empty()) {
        let home_path = houhub_lib::git_credential::absolutize(std::path::Path::new(&home));
        if home_path != resolved_data_dir {
            tracing::warn!(
                "[paths][WARN] HOUHUB_HOME ({}) and HOUHUB_DATA_DIR ({}) point at different roots. \
                 Uploads/pets follow HOUHUB_HOME; the database follows HOUHUB_DATA_DIR. \
    ...
    houhub_lib::web::handlers::files::log_upload_quota_config_at_startup();
    if let Err(err) = houhub_lib::web::handlers::files::validate_upload_quota_config() {
...
async fn async_main() -> ExitCode {
    ...
    let port: u16 = std::env::var("HOUHUB_PORT")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(3080);
    let host = std::env::var("HOUHUB_HOST").unwrap_or_else(|_| "0.0.0.0".to_string());
    // HOUHUB_DATA_DIR was already resolved and absolutized in `main()` so
    // all path resolvers across the process see the same root. Read it
    // back rather than re-deriving the default.
    let data_dir =
        PathBuf::from(std::env::var("HOUHUB_DATA_DIR").expect("HOUHUB_DATA_DIR set by main()"));
    let static_dir_env = std::env::var("HOUHUB_STATIC_DIR").ok();
    ...
    // unproven version).
    if houhub_lib::update::runtime::is_supervised() {
        ...
        if houhub_lib::update::install::upgrade_staged() {
            let trial = houhub_lib::update::runtime::upgrade_trial_secs();
            tokio::spawn(async move {
                tokio::time::sleep(std::time::Duration::from_secs(trial)).await;
                let _ = houhub_lib::update::install::take_upgrade_staged();
            });
        }
    } else {
    ...
    houhub_lib::logging::init::apply_persisted_level(&db.conn).await;
    ...
    // rotate it). An empty/whitespace HOUHUB_TOKEN is treated as unset.
    let mut token_generated = false;
    let token = resolve_persisted_server_token(
        &db.conn,
        std::env::var("HOUHUB_TOKEN").ok(),
        &mut token_generated,
    )
    .await;
    if token_generated {
        ...
        eprintln!(
            "[SERVER] No HOUHUB_TOKEN set; generated an access token (persisted): {token}"
        );
        eprintln!("[SERVER] Pin your own by setting the HOUHUB_TOKEN environment variable.");
    }
    ...
    houhub_lib::init_proxy_from_db(&db.conn).await;
    ...
    // Background, non-blocking; failures are logged but non-fatal.
    {
        ...
        tokio::spawn(async move {
            match houhub_lib::commands::conversations::gc_orphan_chat_dirs_core(
                &gc_conn,
                &gc_data_dir,
            )
            .await
    ...
    let event_bus_metrics = Arc::new(houhub_lib::acp::EventBusMetrics::default());
    let acp_event_bus = Arc::new(houhub_lib::acp::InternalEventBus::new(
        event_bus_metrics.clone(),
    ));
    let emitter = EventEmitter::web_only(broadcaster.clone(), acp_event_bus.clone());

    // Build AppState
    let pet_state_handle = houhub_lib::pet_state_mapper::new_pet_state_handle();
    let connection_manager = houhub_lib::app_state::default_connection_manager();
    let (
        delegation_broker,
        delegation_tokens,
    ...
    let state = Arc::new(AppState {
        db,
        connection_manager,
        terminal_manager: houhub_lib::app_state::default_terminal_manager(),
        event_broadcaster: broadcaster,
        acp_event_bus: acp_event_bus.clone(),
        emitter,
        data_dir,
        web_server_state: WebServerState::new(),
        chat_channel_manager: houhub_lib::app_state::default_chat_channel_manager(),
        workspace_transfer: Arc::new(
            houhub_lib::workspace_transfer::WorkspaceTransferManager::new_from_env(),
        ),
    ...
    if let Some(hub) = houhub_lib::logging::hub::log_hub() {
    ...
    houhub_lib::commands::delegation::apply_persisted_config(&state.db.conn, &delegation_broker)
        .await;
    // Same for the live-feedback enable flag, so the first companion launch
    // sees the operator's configured behavior.
    houhub_lib::commands::feedback::apply_persisted_feedback_config(
        &state.db.conn,
        &feedback_config,
    )
    .await;
    // Same for the ask-user-question enable flag.
    houhub_lib::commands::question::apply_persisted_question_config(
        &state.db.conn,
        &question_config,
    )
    .await;
    // Same for the get-session-info enable flag.
    houhub_lib::commands::session_info::apply_persisted_session_info_config(
        &state.db.conn,
        &session_info_config,
    )
    .await;
    ...
    // the lifetime of the process.
    {
        let listener = houhub_lib::acp::delegation::listener::DelegationListener::new(
            ...
            Arc::new(houhub_lib::acp::manager::ConnectionManagerFeedbackLookup {
                manager: Arc::new(state.connection_manager.clone_ref()),
            }),
            Arc::new(houhub_lib::acp::manager::ConnectionManagerQuestionLookup {
                manager: Arc::new(state.connection_manager.clone_ref()),
            }),
            Arc::new(houhub_lib::commands::session_info::DbSessionInfoLookup::new(
                Arc::new(houhub_lib::db::AppDatabase {
                    conn: state.db.conn.clone(),
                }),
            )),
            Arc::new(houhub_lib::work_task::EngineWorkTaskTools),
            Arc::new(houhub_lib::commands::chat_authoring::DbChatAuthoring::new(
                Arc::new(houhub_lib::db::AppDatabase {
                    conn: state.db.conn.clone(),
                }),
                state.emitter.clone(),
        ...
        let service = houhub_lib::acp::delegation::service::DelegationService::new(
            listener,
            delegation_socket_path.clone(),
        );
        houhub_lib::acp::delegation::service::install(service.clone());
        tokio::spawn(async move {
    ...
    // (`~/.houhub/skills/`). Runs in the background; failures are logged
    // but non-fatal.
    tokio::spawn(async move {
        let report = houhub_lib::commands::experts::ensure_central_experts_installed().await;
    ...
    tokio::spawn(async move {
        let report = houhub_lib::commands::science::ensure_central_science_installed().await;
        if !report.errors.is_empty() {
            tracing::error!(
    ...
    // `DelegationCompleted`.
    tokio::spawn(houhub_lib::lifecycle_subscriber_task(
        state.db.conn.clone(),
        state.connection_manager.clone_ref(),
        state.acp_event_bus.clone(),
    ...
    // JSON broadcaster.
    tokio::spawn(houhub_lib::pet_state_mapper::pet_state_subscriber_task(
        state.acp_event_bus.clone(),
        state.event_broadcaster.clone(),
        state.emitter.clone(),
        pet_state_handle,
    ...
    // Override the 60-second default via `HOUHUB_ACP_IDLE_TIMEOUT_SECS`
    // (set to `0` to disable).
    if let Some(idle_timeout) = houhub_lib::idle_timeout_from_env() {
        tokio::spawn(houhub_lib::idle_sweep_task(
            state.connection_manager.clone_ref(),
            idle_timeout,
    ...
    if let Some(idle_timeout) = houhub_lib::office_watch::idle_timeout_from_env() {
        tokio::spawn(houhub_lib::office_watch::office_watch_idle_sweep_task(
            idle_timeout,
            std::time::Duration::from_secs(houhub_lib::office_watch::SWEEP_INTERVAL_SECS),
        ));
    }
    ...
    if let Some(engine) = houhub_lib::automation::build_engine(
    ...
    // it changed. Does nothing at all until a WebDAV endpoint is configured.
    {
        ...
        tokio::spawn(async move {
            houhub_lib::commands::config_sync::auto_sync::run_auto_sync_loop(
                db_for_sync,
                emitter,
                houhub_lib::commands::config_sync::APP_VERSION.to_string(),
            )
            .await;
        });
    }
    ...
    houhub_lib::web::handlers::files::purge_upload_staging().await;

    // Build router
    let shutdown_signal = state.web_server_state.shutdown_signal();
    let router = houhub_lib::web::router::build_router(
        state.clone(),
        token.clone(),
        static_dir,
        shutdown_signal,
    ...
    if let Err(e) = houhub_lib::web::socket_inherit::mark_listener_non_inheritable(&listener) {
        tracing::warn!(
    ...
    // `HOUHUB_HOST` may be `localhost` or a bracketed IPv6 (`[::1]`); advertise
    ...
    houhub_lib::office_watch::stop_all_office_watches();
    ExitCode::SUCCESS
}
...
fn default_data_dir() -> PathBuf {
    dirs::data_dir()
        .map(|d| d.join("houhub"))
        .unwrap_or_else(|| PathBuf::from(".houhub-data"))
}
