//! Centralized resolution of houhub-owned filesystem paths.
//!
//! Mirrors the conventions already used by `preferences.rs` (`~/.houhub/`)
//! and `experts.rs` (`~/.houhub/skills/`). New features that need a
//! user-scoped persistent directory should call into this module instead of
//! re-deriving `dirs::home_dir().join(".houhub")` themselves.
...
///
/// Returns the relative `.houhub` path when no home directory is available;
/// callers must still handle creation failures gracefully.
pub fn houhub_home_dir() -> PathBuf {
    if let Some(custom) = std::env::var_os("HOUHUB_HOME").filter(|s| !s.is_empty()) {
        return PathBuf::from(custom);
    }
    dirs::home_dir()
        .map(|h| h.join(HOUHUB_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(HOUHUB_DIR_NAME))
}

/// Root directory for desktop-pet assets.
///
/// Resolution order:
/// 1. `$HOUHUB_HOME/pets` (explicit override, used in tests and custom installs)
/// 2. `$HOUHUB_DATA_DIR/pets` (server-mode data directory, populated by
///    `houhub-server` from the corresponding env var)
/// 3. `~/.houhub/pets` (default for the desktop app)
pub fn houhub_pets_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("HOUHUB_HOME").filter(|s| !s.is_empty()) {
...
/// 1. `$HOUHUB_HOME/uploads`
/// 2. `$HOUHUB_DATA_DIR/uploads` (server-mode data directory)
/// 3. `~/.houhub/uploads` (desktop default)
///
/// Files in this directory are not garbage-collected by houhub itself —
...
/// `HOUHUB_UPLOAD_MAX_TOTAL_BYTES` (see `web::handlers::files`): new
...
/// in-flight reservation counter to make `HOUHUB_UPLOAD_MAX_TOTAL_BYTES`
/// a hard ceiling within one `houhub-server` process. Multiple
/// `houhub-server` processes sharing the same uploads root (e.g.
...
pub fn houhub_uploads_root() -> PathBuf {
    ...
    dirs::home_dir()
        .map(|h| h.join(HOUHUB_DIR_NAME).join(UPLOADS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(HOUHUB_DIR_NAME).join(UPLOADS_DIR_NAME))
}
...
///
/// Resolution mirrors [`houhub_uploads_root`] exactly so logs land on the same
/// filesystem root as uploads/pets/the database:
/// 1. `$HOUHUB_HOME/logs` (explicit override)
/// 2. `$HOUHUB_DATA_DIR/logs` (server-mode data directory)
/// 3. `~/.houhub/logs` (default for the desktop app)
///
/// Pure env + `dirs::home_dir()`, so it is callable at the very start of a
/// process — before the database (or, in `houhub-server`, the tokio runtime)
/// exists — which is exactly when the subscriber must be installed.
pub fn houhub_logs_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("HOUHUB_HOME").filter(|s| !s.is_empty()) {
...
///
/// Resolution:
/// 1. If `HOUHUB_DATA_DIR` is set and non-empty, return its absolutized
...
/// authoritative and not re-read `HOUHUB_DATA_DIR` themselves; the
/// startup code in `lib.rs` / `bin/houhub_server.rs` writes the
...
/// consult `HOUHUB_DATA_DIR` — it returns the identifier-derived path
...
/// operator pre-set `HOUHUB_DATA_DIR` to a custom location.
pub fn resolve_effective_data_dir(tauri_fallback: &Path) -> PathBuf {
    if let Some(custom) = std::env::var_os("HOUHUB_DATA_DIR").filter(|s| !s.is_empty()) {
...
// The resolver functions above depend on global env vars (`HOUHUB_HOME`,
// `HOUHUB_DATA_DIR`), so unit tests would need cross-test serialization to
// avoid races. The behaviour is covered end-to-end by `pets::*` tests which
// set `HOUHUB_HOME` inside a serialized test mutex; we deliberately don't
...
mod tests {
    ...
    fn simplifies_a_verbatim_drive_path() {
        assert_eq!(
            simplify_verbatim_path(Path::new(r"\\?\C:\Users\song\.houhub\uploads\b\img.png")),
            PathBuf::from(r"C:\Users\song\.houhub\uploads\b\img.png")
        );
    }
    ...
    fn leaves_every_other_shape_untouched() {
        for path in [
            "/home/u/.houhub/uploads/b/img.png",
