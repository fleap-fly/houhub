//! houhub's own **ACP-native transcript** for custom agents.
//!
...
//! houhub**, so the connection layer witnesses the entire conversation. Here we
//! take it all the way — instead of recording a derived measurement, we record
//! the ACP wire itself:
//!
//! * each prompt houhub sends (`session/prompt`'s content blocks), and
...
//!
//! ## Why raw ACP and not houhub's own `AcpEvent`
//!
...
//! ## File layout
//!
//! `<paths::houhub_acp_transcripts_root()>/<registry-id>/<session-id>.jsonl`
//!
...
pub struct TranscriptHeader {
    ...
    /// them in memory only — so after a restart `session/load` fails and houhub
    /// starts a fresh agent session for the SAME conversation. The turns houhub
...
pub enum EntryKind {
    /// A prompt houhub sent to the agent. `payload` is the ACP content-block
    ...
    /// could only be inferred; houhub is the party that receives the response,
...
pub struct TranscriptEntry {
    /// Epoch milliseconds when houhub observed this.
    pub t: u64,
    pub k: EntryKind,
...
/// come from outside houhub (user input / the agent's own id generator), so
...
impl PendingRecord {
    ...
    /// second copy of history HouHub already recorded.
    ///
...
pub fn record_header(
    ...
    record_header_in(&crate::paths::houhub_acp_transcripts_root(), agent_dir, header)
}
...
pub fn record_entry(
    ...
    record_entry_in(
        &crate::paths::houhub_acp_transcripts_root(),
        agent_dir,
        session_id,
        kind,
        payload,
...
/// **The replay gate.** True when this conversation already has history HouHub
...
/// the replay is then appended to history HouHub already had, and because the
...
/// agent has forgotten a session, houhub opens a fresh one and links it back
...
mod tests {
    ...
    /// of history HouHub already has. It reads the file, so a prompt sitting in
