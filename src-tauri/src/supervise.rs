//! `houhub-server --supervise` — a minimal process supervisor that owns the
...
//! [`crate::update::runtime::EXIT_RESTART`]. We wait `HOUHUB_RESTART_DELAY_MS`
...
//! `HOUHUB_UPGRADE_TRIAL_SECS` (the new binary can't even boot), we restore
