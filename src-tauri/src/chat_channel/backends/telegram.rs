...
mod tests {
    ...
    fn chat_filter_matches_configured_username_case_insensitively() {
        let message = serde_json::json!({
            "chat": { "id": -100123, "username": "HouhubTopics" }
        });

        assert!(telegram_message_chat_matches(&message, "@houhubtopics"));
        assert!(telegram_message_chat_matches(&message, "HOUHUBTOPICS"));
    ...
    fn strip_bot_mention_removes_the_mention_in_either_case() {
        assert_eq!(
            strip_bot_mention("/task@houhub_bot build", "houhub_bot"),
            "/task build"
        );
        assert_eq!(
            strip_bot_mention("/task@HouHub_Bot build", "houhub_bot"),
        ...
        assert_eq!(strip_bot_mention("/task build", "houhub_bot"), "/task build");
        assert_eq!(strip_bot_mention("/task@houhub_bot", ""), "/task@houhub_bot");
    }
    ...
    fn strip_bot_mention_survives_text_whose_lowercase_is_a_different_length() {
        ...
        assert_eq!(
            strip_bot_mention("\u{130}@houhub_bot hi", "houhub_bot"),
            "\u{130} hi"
        );
        // Kelvin sign, 3 bytes -> 1
        assert_eq!(
            strip_bot_mention("\u{212A}@houhub_bot hi", "houhub_bot"),
            "\u{212A} hi"
        );
        // capital sharp s, 3 bytes -> 2
        assert_eq!(
            strip_bot_mention("\u{1E9E}@houhub_bot hi", "houhub_bot"),
            "\u{1E9E} hi"
        );
        // Enough drift to run the old offset off the end of the string.
        assert_eq!(
            strip_bot_mention("\u{130}\u{130}@houhub_bot", "houhub_bot"),
            "\u{130}\u{130}"
        );
        assert_eq!(
            strip_bot_mention("\u{212A}\u{212A}@houhub_bot x", "houhub_bot"),
            "\u{212A}\u{212A} x"
        );
        // A mention whose tail is a wide character is not a mention, and
        // deciding that must not slice through the character.
        assert_eq!(
            strip_bot_mention("@houhub_bo\u{65E5}", "houhub_bot"),
            "@houhub_bo\u{65E5}"
        );
    }
