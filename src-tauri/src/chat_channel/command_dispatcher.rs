...
struct CommandConfigCache {
    ...
    /// since boot, so starting houhub inside the first 31 seconds of a boot
    /// aborted the process here. houhub ships an autostart plugin, so launching
...
mod tests {
    ...
    async fn callback_data_dispatches_without_command_prefix() {
        ...
        let folder_id = seed_folder(&db, "/tmp/houhub-dispatch-callback").await;
        ...
        let response = dispatch_command(
            ...
            &EventEmitter::Noop,
            &bridge,
            std::path::Path::new("/tmp/houhub-dispatch-data"),
            channel_id,
            "sender-1",
            &target,
