...
/// `auth.type` into a literal `~` directory beside HouHub's working directory
...
/// HouHub's: `merge_agent_env` copies `HOME` into the child like any other
...
fn open_trajectory(db_path: &Path) -> Option<Connection> {
    ...
    // houhub never mutates the server's stores, so ask for a read-only handle;
...
fn try_open_trajectory(db_path: &Path, flags: OpenFlags) -> Option<Connection> {
    ...
    // The server may hold the trajectory open while houhub lists sessions; give
...
/// houhub's MCP-aware cards (delegation, ask-question, the workbench
...
mod tests {
    ...
    fn mcp_dispatch_unwrap_names_the_tool_and_peels_the_arguments() {
        let (name, input) = unwrap_mcp_dispatch(&dispatch(
            ...
            r#"{"Arguments":{"task_ids":["a"]},"ServerName":"houhub-mcp","ToolName":"get_delegation_status","toolAction":"Checking"}"#,
        ))
        .expect("an MCP envelope");
        assert_eq!(name, "houhub-mcp_get_delegation_status");
        assert_eq!(
            input,
