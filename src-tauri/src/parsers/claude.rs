...
impl ClaudeRecordAccumulator {
    ...
    ///
    /// ```text
    /// [[houhub-background-task]]{"task_id":…,"status":…,"summary":…,"result":…}
    /// ```
    ///
...
/// report reaches a client — but houhub's history path parses the CLI's own
...
mod tests {
    ...
    #[test]
    /// The user typed `/compact`; the divider is what it produced. Live, houhub
    ...
    fn parse_falls_back_to_user_message_when_ai_title_empty() {
        let path = std::env::temp_dir().join(format!(
            "houhub-claude-aititle-empty-{}.jsonl",
            uuid::Uuid::new_v4()
        ));
        let mut file = fs::File::create(&path).expect("create temp jsonl");
        writeln!(
    ...
    fn parse_both_titles(
        ...
        let path =
            std::env::temp_dir().join(format!("houhub-claude-{tag}-{}.jsonl", uuid::Uuid::new_v4()));
        let mut file = fs::File::create(&path).expect("create temp jsonl");
        for line in lines {
    ...
    fn custom_title_entry_is_not_rendered_as_a_turn() {
        ...
        let path = std::env::temp_dir().join(format!(
            "houhub-claude-customtitle-turn-{}.jsonl",
            uuid::Uuid::new_v4()
        ));
        let mut file = fs::File::create(&path).expect("create temp jsonl");
    ...
    fn parse_with_subagents(
        ...
        let stem = std::env::temp_dir().join(format!("houhub-claude-sub-{}", uuid::Uuid::new_v4()));
