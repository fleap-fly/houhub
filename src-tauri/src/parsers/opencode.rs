...
/// One OpenCode tool call rewritten into houhub's shared tool vocabulary.
...
/// keys every renderer in houhub dispatches on (`file_path`, `old_string`,
/// `new_string`, `pattern`, …).
///
...
/// same shape `render_ask_result` writes for houhub's own companion — routes it
