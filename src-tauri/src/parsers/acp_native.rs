//! History parser for agents that have **no houhub-side store parser**: custom
//! ACP agents.
//!
//! Every other parser in this module reverse-engineers one agent's private
//! session store. This one reads houhub's own ACP transcript
...
//! `session/prompt`. houhub records it as an explicit
...
impl AcpNativeParser {
    pub fn new(agent_type: AgentType) -> Self {
        Self {
            agent_type,
            root: crate::paths::houhub_acp_transcripts_root(),
        }
    }
...
impl AgentParser for AcpNativeParser {
    ...
    fn get_conversation(&self, conversation_id: &str) -> Result<ConversationDetail, ParseError> {
        ...
        // session and houhub started a fresh one, the earlier turns still live
...
impl AcpNativeParser {
    fn summarize(&self, session_id: &str, transcript: &Transcript) -> ConversationSummary {
        ...
        ConversationSummary {
            ...
            // honestly yield — ACP has no title channel, and houhub's DB-side
...
pub fn project_turns(entries: &[TranscriptEntry]) -> Vec<MessageTurn> {
    ...
    // span starts when houhub SENT the prompt, not when the first token arrived —
...
fn apply_update(
    ...
    match update {
        SessionUpdate::UserMessageChunk(chunk) => {
            // Live path: houhub already recorded the outgoing prompt, so the
...
mod tests {
    ...
    /// depending on whether houhub recorded the turn or the agent replayed it.
    #[test]
    ...
    /// together over the block shapes houhub's composer actually sends.
    #[test]
    ...
    /// A `delegate_to_agent` call from the houhub-mcp companion must replay
    ...
    fn a_delegation_call_replays_under_its_wire_name() {
        let entries = vec![
            ...
            update(
                ...
                serde_json::json!({
                    "sessionUpdate": "tool_call",
                    "toolCallId": "call-9",
                    "title": "mcp__houhub-mcp__delegate_to_agent",
                    "kind": "other",
                    "status": "pending",
        ...
        match &blocks[0] {
            ContentBlock::ToolUse {
                ...
                assert_eq!(tool_name, "mcp__houhub-mcp__delegate_to_agent");
    ...
    fn a_session_restarted_after_the_agent_forgot_it_reads_as_one_conversation() {
        ...
        // failed `session/load` and houhub opened `sess-new` for the same
