...
/// These paths are owned by the respective CLIs — houhub only reads them.
#[derive(Clone)]
...
/// happens to know about it. `houhub-mcp` is injected into every MCP-capable
...
pub fn build_agent_parser(agent_type: AgentType) -> Box<dyn AgentParser> {
    let inner: Box<dyn AgentParser> = match agent_type {
        ...
        // history is houhub's own ACP transcript.
        AgentType::Custom(_) => Box::new(acp_native::AcpNativeParser::new(agent_type)),
    };
    Box::new(RouteSanitized(inner))
}

/// Removes HouHub's internal `@agent` routing frame from whatever a parser read
...
mod route_sanitizer_tests {
    ...
    fn routing_frame(agent_wire: &str) -> String {
        let mut blocks = vec![PromptInputBlock::Text {
            text: format!("ask [@A](houhub://agent/{agent_wire}) to help"),
        }];
    ...
    fn frames_are_stripped_from_any_agents_history_not_just_codex() {
        ...
        let frame = routing_frame("antigravity");
        let visible = "ask [@A](houhub://agent/antigravity) to help";
    ...
    fn a_frame_in_its_own_block_leaves_no_empty_block_behind() {
        let visible = "ask [@A](houhub://agent/claude_code) to help";
    ...
    fn a_blank_line_before_the_frame_is_not_left_behind() {
        let visible = "ask [@A](houhub://agent/codex) to help";
    ...
    fn a_frame_an_agent_stored_without_separators_leaves_neither_prose_nor_title() {
        ...
        assert!(
            capped.contains("houhub_internal_agent_routes"),
    ...
    fn a_title_capped_halfway_through_the_marker_still_leaks_nothing() {
        let prose = format!("{} ask [@A](houhub://agent/antigravity) to help", "x".repeat(66));
        let frame = routing_frame("antigravity");
        let persisted = format!("{prose} {}", frame.replace('\u{001e}', " "));

        let capped = super::title_from_user_text(&persisted);
        let marker = "{\"kind\":\"houhub_internal_agent_routes\"";
        assert!(
            capped.contains("{\"kind\":\"houhub") && !capped.contains(marker),
    ...
    fn assistant_turns_and_look_alike_user_prose_are_left_alone() {
        ...
        let look_alike =
            "see <houhub_internal_agent_routes version=\"2\">note</houhub_internal_agent_routes>";
    ...
    fn only_a_title_is_cut_at_a_dangling_separator() {
        ...
        let dangling = "see \u{001e}<houhub_internal_agent_routes version=\"2\">no";
        let detail = sanitized(Fixture {
...
mod tests {
    ...
    fn fold_reference_links_reduces_links_to_labels() {
        ...
        // houhub:// links fold too; an agent mention keeps its `@`.
        assert_eq!(
            fold_reference_links("调用 [@Codex CLI](houhub://agent/codex) 执行"),
            "调用 @Codex CLI 执行"
        );
    ...
    fn path_matching_handles_separator_differences() {
        assert!(path_eq_for_matching(
            "/Users/demo/workspace/houhub",
            "/Users/demo/workspace/houhub/"
        ));
        assert!(path_eq_for_matching(
            "C:\\Users\\demo\\workspace\\houhub",
            "C:/Users/demo/workspace/houhub"
        ));
    }
}
