...
impl GeminiParser {
    ...
    fn parse_jsonl_chat_value(raw: &str) -> Option<Value> {
        ...
        if !has_session_id || !has_project_hash {
            ...
            // Upstream gives up here. houhub keeps a transcript that named its
            // own session: `projectHash` is metadata houhub never reads, and
    ...
    ///
    /// houhub's [`TurnUsage`] buckets are Anthropic-shaped and DISJOINT —
...
mod tests {
    ...
    fn parses_gemini_session_detail_from_chat_json() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-test-{nanos}"));
        let chats_dir = base.join("tmp").join("houhub").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("houhub").join(".project_root"),
  ...
  "messages": [
    ...
    fn gemini_prefers_update_topic_title_over_first_user_message() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-topic-{nanos}"));
        let chats_dir = base.join("tmp").join("houhub").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("houhub").join(".project_root"),
  ...
  "messages": [
    ...
    fn gemini_falls_back_to_real_prompt_skipping_session_context() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-ctx-{nanos}"));
        let chats_dir = base.join("tmp").join("houhub").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("houhub").join(".project_root"),
  ...
  "messages": [
    ...
    fn parses_gemini_session_detail_from_jsonl_chat_log() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-jsonl-test-{nanos}"));
        let chats_dir = base.join("tmp").join("houhub-jsonl").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("houhub-jsonl").join(".project_root"),
...
"#;
    ...
    fn parse_detail_completion_time_uses_assistant_timestamp_not_next_message_gap() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-completed-{nanos}"));
        let chats_dir = base.join("tmp").join("houhub").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
  ...
  "messages": [
    ...
    fn chat_fixture(tag: &str, alias: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-{tag}-{nanos}"));
    ...
    fn jsonl_rebuilds_history_from_set_messages() {
        let base = chat_fixture("setmsgs", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_set_messages_replaces_then_appends() {
        let base = chat_fixture("replace", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_rewind_truncates_from_target() {
        let base = chat_fixture("rewind", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_rewind_to_unknown_id_clears_everything() {
        let base = chat_fixture("rewind-miss", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_repeated_id_replaces_whole_record_in_place() {
        let base = chat_fixture("replace-id", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_skips_malformed_lines() {
        let base = chat_fixture("badline", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_falls_back_to_legacy_single_object() {
        let base = chat_fixture("legacy", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
  ...
  "messages": [
    ...
    fn jsonl_metadata_only_session_still_lists() {
        let base = chat_fixture("empty", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn jsonl_prefers_metadata_summary_for_title() {
        let base = chat_fixture("summary", "houhub");
        let file_path = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#;
    ...
    fn discovers_subagent_transcripts_as_children() {
        let base = chat_fixture("subagent", "houhub");
        let nested = base
            .join("tmp")
            .join("houhub")
            .join("chats")
...
"#,
    ...
    fn falls_back_to_metadata_directories_for_folder_path() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("houhub-gemini-dirs-{nanos}"));
