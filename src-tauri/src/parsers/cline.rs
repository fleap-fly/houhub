...
// houhub opening a live SQLite store owned by another process.
//
...
/// only the fields houhub surfaces are declared.
...
impl ClineParser {
    ...
    fn read_manifest(&self, session_id: &str) -> Option<SessionManifest> {
        ...
        // other path is built from, and the id houhub recorded as `external_id`.
...
mod tests {
    ...
    fn a_quoted_closing_tag_does_not_hang_the_session_cleanup() {
        ...
        // houhub cannot recover: the first closing tag ends the block, and the
