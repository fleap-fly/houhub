...
impl CodexParser {
    ...
    /// already visited (a malformed cycle) or one houhub cannot find, which is
    ...
    /// records the switch in its SQLite state; houhub reads only the rollouts, so
...
const COLLAB_OP_KEY: &str = "__houhubCollabOp";
...
pub const CODEX_SUBAGENT_LAUNCH_KEY: &str = "__houhubCodexSubagentLaunch";
...
pub const CODEX_SUBAGENT_STATE_KEY: &str = "__houhubCodexSubagentState";
...
impl CodexParser {
    fn parse_conversation_detail(
        ...
        // Capsule model mirrors the live frontend: spawn owns execution metadata,
        // wait owns the final result, and close only fills a never-waited fallback.
        // codex-acp 1.1.3+ also emits `_meta.codex.subagent` activity signals;
        // HouHub suppresses those redundant, transcript-free updates so live and
        // reconstructed capsules stay consistent.
...
///
/// Deliberately the same shape the LIVE path produces: HouHub seeds codex-acp's
...
///   corpus, but it evaporates the moment the user resumes the session in houhub:
...
mod tests {
    ...
    fn internal_agent_routes_never_surface_in_codex_history() {
        ...
        let visible = "Ask [@Antigravity](houhub://agent/antigravity) to review";
    ...
    fn route_only_event_does_not_suppress_visible_response_item_fallback() {
        ...
        let visible = "Ask [@Codex](houhub://agent/codex) to inspect this";
        let mut prompt = vec![PromptInputBlock::Text {
            text: visible.into(),
        }];
        append_agent_routes(&mut prompt, true);
    ...
    fn user_authored_agent_route_envelope_survives_summary_and_detail() {
        ...
        let visible = "Explain \u{001e}<houhub_internal_agent_routes version=\"2\">user text</houhub_internal_agent_routes>\u{001e}";
        let lines = [
            serde_json::json!({
                "timestamp": "2026-08-15T08:00:00Z",
                "type": "session_meta",
        ...
        assert!(summary
            .title
            .as_deref()
            .is_some_and(|title| title.contains("houhub_internal_agent_routes")));
    ...
    fn internal_routes_are_removed_from_image_response_item_text() {
        ...
        let visible = "Review this image [@Codex](houhub://agent/codex)";
        let mut prompt = vec![PromptInputBlock::Text {
            text: visible.into(),
        }];
        append_agent_routes(&mut prompt, true);
    ...
    fn parse_detail_sets_context_window_stats_from_token_count() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-ctx-{nanos}.jsonl"));
    ...
    fn parse_rollout(label: &str, content: &str, session_id: &str) -> crate::models::ConversationDetail {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-{label}-{nanos}.jsonl"));
        fs::write(&path, content).expect("write test jsonl");
    ...
    fn parse_detail_durations_partition_a_multi_message_turn() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-spans-{nanos}.jsonl"));
    ...
    fn a_mid_turn_turn_context_does_not_truncate_the_reply_after_it() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-midctx-{nanos}.jsonl"));
    ...
    fn parse_detail_completion_time_uses_agent_message_timestamp_not_added_turn_span() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-completed-{nanos}.jsonl"));
    ...
    fn parse_detail_keeps_single_user_turn_when_goal_text_persisted() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-goaltext-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_detail_goal_opener_and_later_same_text_user_are_both_kept() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-goaldup-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_detail_goal_opener_then_confirm_reply_keeps_prompt_and_titles_from_goal() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-goalconfirm-{nanos}.jsonl"));
        let content = concat!(
            "{\"timestamp\":\"2026-03-01T10:00:00Z\",\"type\":\"session_meta\",\"payload\":{\"id\":\"gc-1\",\"cwd\":\"/tmp/demo\"}}\n",
    ...
    fn parse_summary_goal_opener_then_confirm_reply_titles_from_goal_and_counts_opener() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-sumconfirm-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_summary_titles_pure_goal_session_from_objective() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-sumgoal-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_summary_prefers_native_thread_name_over_goal_objective() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-sumname-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_summary_goal_opener_then_image_user_still_synthesizes_opener() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-sumimg-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn an_image_prompt_written_to_both_channels_counts_once() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("houhub-codex-sumimg-bothchan-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn image_response_item_count_matches_detail_without_canonical_user_event() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("houhub-codex-sumimg-parity-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn parse_summary_goal_null_clear_adds_no_synthetic_count() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-sumnull-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn goal_with_text_only_response_item_titles_from_objective_in_both_paths() {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-gtxt-{nanos}.jsonl"));
        let content = concat!(
    ...
    fn summary_ignores_non_opening_goal_for_synthesis() {
        ...
        let path_a: PathBuf = env::temp_dir().join(format!("houhub-codex-term-{nanos}.jsonl"));
        fs::write(
            &path_a,
            concat!(
        ...
        let path_b: PathBuf = env::temp_dir().join(format!("houhub-codex-termact-{nanos}.jsonl"));
        fs::write(
            &path_b,
            concat!(
    ...
    fn image_generation_end_and_call_dedupe_by_id() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-img-dedupe-{nanos}.jsonl"));
    ...
    fn image_generation_end_honors_explicit_mime_type() {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path: PathBuf = env::temp_dir().join(format!("houhub-codex-img-mime-{nanos}.jsonl"));
    ...
    fn image_generation_end_omits_revised_prompt_when_missing() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("houhub-codex-img-noprompt-{nanos}.jsonl"));
    ...
    fn image_generation_inside_subagent_is_suppressed_in_parent() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path: PathBuf =
            env::temp_dir().join(format!("houhub-codex-img-subagent-{nanos}.jsonl"));
    ...
    fn write_temp_rollout(tag: &str, lines: &[String]) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let path = env::temp_dir().join(format!("houhub-codex-{tag}-{nanos}.jsonl"));
    ...
    fn temp_session_dir(tag: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("system time ok")
            .as_nanos();
        let dir = env::temp_dir().join(format!("houhub-codex-{tag}-{nanos}"));
    ...
    fn subagent_waits_emit_independent_collab_capsules_and_keep_narration() {
        ...
        assert!(a_cap.contains("__houhubCollabOp"));
    ...
    fn completed_mcp_items_split_a_two_call_one_chunk_script() {
        let script = concat!(
            ...
            "tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",working_dir:wd,task:taskA}),",
            "tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",working_dir:wd,task:taskB})",
        ...
        assert_eq!(
            uses.iter()
                ...
                .collect::<Vec<_>>(),
            vec![
                ("exec-b", "mcp__houhub_mcp__delegate_to_agent"),
                ("exec-a", "mcp__houhub_mcp__delegate_to_agent"),
            ],
    ...
    fn mixed_native_collaboration_and_semantic_delegation_keep_their_identities() {
        ...
        let initial_script =
            "const r = await tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",working_dir:\"/tmp/mcp-worker\",task:\"semantic initial\"});text(JSON.stringify(r));";
        let continuation_script =
            "const r = await tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",working_dir:\"/tmp/mcp-worker\",task:\"semantic followup\",continue_from_task_id:\"task-semantic-initial\"});text(JSON.stringify(r));";
        ...
        assert_eq!(
            ...
            vec![
                (
                    ...
                    "mcp__houhub_mcp__delegate_to_agent",
                    Some(
    ...
    fn a_deferred_scripts_late_mcp_item_cannot_bind_to_the_next_script() {
        let script = "const r=await tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",task:\"A\"});text(JSON.stringify(r));";
        let mut lines = code_mode_rollout(
            script,
        ...
        lines.push(rollout_line(
            ...
            serde_json::json!({
                "type":"item_completed",
                "item": {
                    "type":"McpToolCall", "id":"exec-from-a", "server":"houhub-mcp",
    ...
    /// written beside it. Shape taken verbatim from a real rollout: houhub-mcp
    /// refuses a `delegate_to_agent` past the delegation depth limit.
    #[test]
    fn a_failed_semantic_mcp_item_settles_its_card_as_failed() {
        let script = concat!(
            "const dir=\"/tmp/w\";",
            "const res=await tools.mcp__houhub_mcp__delegate_to_agent({agent_type:\"codex\",working_dir:dir,task:t});",
        ...
        let lines_with_item = |item: bool| {
            let mut lines = code_mode_rollout(
                concat!(
                    ...
                    "  tools.mcp__houhub_mcp__get_delegation_status({task_ids:[\"t1\"],wait_ms:30000}),\n",
    ...
    fn a_thrown_script_keeps_its_own_card_over_a_matching_semantic_item() {
        let script = concat!(
            "const r=await tools.mcp__houhub_mcp__task_progress({message:m});",
    ...
    fn polling_a_background_session_appends_to_the_card_of_its_command() {
        ...
        assert_eq!(output, "   Compiling houhub\ntest result: ok. 1 passed");
    }
    ...
    fn deferred_script_rollout(wait_output: serde_json::Value) -> Vec<String> {
        let mut lines = code_mode_rollout(
            "const r = await tools.mcp__houhub_mcp__get_delegation_status({task_ids:[\"t1\"],wait_ms:60000});\ntext(JSON.stringify(r));\n",
    ...
    fn a_deferred_script_completed_mcp_item_replaces_its_wrapper_card() {
        let script = concat!(
            "const task=\"t1\";",
            "const r=await tools.mcp__houhub_mcp__get_delegation_status({task_ids:[task],wait_ms:60000});",
            "text(JSON.stringify(r));"
        );
        let mut lines = code_mode_rollout(
            script,
        ...
        assert_eq!(
            ...
            vec![ (
                ...
                "mcp__houhub_mcp__get_delegation_status".into(),
    ...
    fn a_deferred_scripts_result_lands_on_its_own_card() {
        ...
        assert_eq!(
            ...
            vec![(
                "call_1".to_string(),
                "mcp__houhub_mcp__get_delegation_status".to_string()
            )]
        );
        assert_eq!(
            tool_results(&detail),
    ...
    fn a_script_cell_is_never_mistaken_for_a_shell_session() {
        ...
        let mut lines = code_mode_rollout(
            "const r = await tools.mcp__houhub_mcp__ask_user_question({questions: []});\ntext(JSON.stringify(r));\n",
            serde_json::json!([
        ...
        assert_eq!(
            uses.iter()
                .map(|(id, name, _)| (id.as_str(), name.as_str()))
                .collect::<Vec<_>>(),
            vec![("call_1", "mcp__houhub_mcp__ask_user_question")]
        );
    ...
    fn a_resumed_mixed_rollout_keeps_both_halves_exactly_once() {
        ...
        // in houhub — which appends NATIVE `event_msg` turns to the SAME file.
