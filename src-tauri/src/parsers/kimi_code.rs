...
/// own `logs/kimi-code.log` (the wire only stores the houhub-managed model alias).
...
struct WireParse {
    ...
    /// The houhub-managed model alias from a `config.update` record (fallback only;
...
/// (`... llm config ... model=kimi-k2.7-code modelAlias=houhub-managed ...`). The wire
/// only stores the (houhub-managed) alias, so the log is the sole place the actual
...
mod tests {
    ...
    fn unique_root(tag: &str) -> PathBuf {
        std::env::temp_dir()
            .join(format!("houhub-kimi-{tag}-{}", uuid::Uuid::new_v4()))
            .join(".kimi-code")
            .join("sessions")
    }
    ...
    fn sample_wire() -> Vec<Value> {
        vec![
            ...
            json!({"type":"config.update","modelAlias":"houhub-managed","thinkingLevel":"high","time":1782276644194i64}),
            ...
            json!({"type":"usage.record","model":"houhub-managed","usage":{"inputOther":7962,"output":37,"inputCacheRead":9472,"inputCacheCreation":0},"usageScope":"turn","time":1782276660974i64}),
            ...
            json!({"type":"usage.record","model":"houhub-managed","usage":{"inputOther":265,"output":36,"inputCacheRead":17408,"inputCacheCreation":0},"usageScope":"turn","time":1782276664500i64}),
        ]
    }
    ...
    fn content_free_session_is_not_listed() {
        ...
        write_session(
            ...
            &[
                ...
                json!({"type":"config.update","modelAlias":"houhub-managed","time":1782276644194i64}),
            ],
        );
    ...
    fn agent_delegation_loads_subagent_tool_calls_into_agent_stats() {
        ...
        write_subagent_wire(
            ...
            &[
                ...
                json!({"type":"config.update","profileName":"coder","modelAlias":"houhub-managed","time":1782276651000i64}),
