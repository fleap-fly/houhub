...
/// puts houhub in a different directory than the CLI it just launched —
...
///
/// Not mirrored, deliberately: the `--config-dir` FLAG, which houhub never
...
mod tests {
    ...
    fn a_null_active_leaf_clears_the_conversation() {
        ...
        // deletion: an already-imported session keeps its row in houhub's DB, and
    ...
    // The ACP entrypoint — the one houhub itself drives — writes the human
