...
/// them as user turns was a houhub-only artifact: it split one exchange into two
...
mod tests {
    ...
    fn parses_item_format_text_session() {
        let root = std::env::temp_dir().join(format!("houhub-cb-text-{}", uuid::Uuid::new_v4()));
        let sid = "sess-text";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn assert_title(tag: &str, records: &[Value], expected: Option<&str>) {
        let root = std::env::temp_dir().join(format!("houhub-cb-{tag}-{}", uuid::Uuid::new_v4()));
        let sid = "sess-title";
    ...
    fn parses_tool_calls_with_error_detection() {
        let root = std::env::temp_dir().join(format!("houhub-cb-tool-{}", uuid::Uuid::new_v4()));
        let sid = "sess-tool";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn empty_session_file_is_handled() {
        let root = std::env::temp_dir().join(format!("houhub-cb-empty-{}", uuid::Uuid::new_v4()));
    ...
    fn metadata_only_session_is_not_listed() {
        let root = std::env::temp_dir().join(format!("houhub-cb-meta-{}", uuid::Uuid::new_v4()));
        let sid = "sess-meta";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn model_falls_back_to_model_id_when_request_model_name_blank() {
        let root = std::env::temp_dir().join(format!("houhub-cb-model-{}", uuid::Uuid::new_v4()));
        let sid = "sess-model";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn read_tool_output_is_structurized() {
        let root = std::env::temp_dir().join(format!("houhub-cb-read-{}", uuid::Uuid::new_v4()));
        let sid = "sess-read";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn subagent_task_is_rewritten_to_agent() {
        let root = std::env::temp_dir().join(format!("houhub-cb-agent-{}", uuid::Uuid::new_v4()));
        let sid = "sess-agent";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn subagent_tool_calls_loaded_into_agent_stats() {
        let root =
            std::env::temp_dir().join(format!("houhub-cb-substats-{}", uuid::Uuid::new_v4()));
    ...
    /// `[[houhub-background-task]]{…}` payload of the tool result with this id.
    ...
    fn injected_meta_user_record_is_not_a_prompt() {
        ...
        // skips these records; houhub folds the payload onto the launch card
        // instead.
        let root = std::env::temp_dir().join(format!("houhub-cb-meta-user-{}", uuid::Uuid::new_v4()));
        let sid = "sess-bg";
    ...
    fn unsettled_async_launch_marker_reports_no_status() {
        ...
        let root = std::env::temp_dir().join(format!("houhub-cb-unsettled-{}", uuid::Uuid::new_v4()));
        let sid = "sess-bg";
    ...
    fn a_failed_worker_notification_keeps_its_status() {
        let root = std::env::temp_dir().join(format!("houhub-cb-bgfail-{}", uuid::Uuid::new_v4()));
        let sid = "sess-bg";
        let mut records = async_launch_records();
    ...
    fn async_launch_loads_the_detached_workers_transcript() {
        ...
        let root = std::env::temp_dir().join(format!("houhub-cb-asyncsub-{}", uuid::Uuid::new_v4()));
        let sid = "sess-bg";
        write_session(&root, "Users-demo-app", sid, &async_launch_records());
        write_subagent(
            &root,
    ...
    fn a_non_agent_result_is_never_touched_by_the_background_fold() {
        ...
        let root = std::env::temp_dir().join(format!("houhub-cb-bgiso-{}", uuid::Uuid::new_v4()));
        let sid = "sess-bg";
        let mut records = async_launch_records();
        records.push(
    ...
    fn deferred_mcp_tool_is_unwrapped() {
        let root = std::env::temp_dir().join(format!("houhub-cb-defer-{}", uuid::Uuid::new_v4()));
        let sid = "sess-defer";
        write_session(
            &root,
            "Users-demo-app",
            ...
            &[
                ...
                json!({"type":"function_call","timestamp":1782193811200i64,"cwd":"/Users/demo/app","sessionId":sid,
                       ...
                       "arguments":"{\"params\":{\"agent_type\":\"codex\",\"task\":\"build\",\"working_dir\":\"/Users/demo/app\"},\"toolName\":\"mcp__houhub-mcp__delegate_to_agent\"}"}),
                ...
                json!({"type":"function_call","timestamp":1782193811600i64,"cwd":"/Users/demo/app","sessionId":sid,
                       ...
                       "arguments":"{\"params\":{\"task_ids\":[\"e5c9\"],\"wait_ms\":60000},\"toolName\":\"mcp__houhub-mcp__get_delegation_status\"}"}),
                json!({"type":"function_call","timestamp":1782193811700i64,"cwd":"/Users/demo/app","sessionId":sid,
                       "name":"DeferExecuteTool","callId":"d3",
                       "arguments":"{\"params\":{\"task_id\":\"e5c9\"},\"toolName\":\"mcp__houhub-mcp__cancel_delegation\"}"}),
        ...
        assert_eq!(name_of("d1"), "mcp__houhub-mcp__delegate_to_agent");
        assert_eq!(name_of("d2"), "mcp__houhub-mcp__get_delegation_status");
        assert_eq!(name_of("d3"), "mcp__houhub-mcp__cancel_delegation");
    ...
    fn subagent_transcript_is_excluded_from_conversation_list() {
        ...
        let root = std::env::temp_dir().join(format!("houhub-cb-sublist-{}", uuid::Uuid::new_v4()));
        let sid = "sess-list";
        write_session(
            &root,
            "Users-demo-app",
    ...
    fn top_level_session_under_subagents_named_project_dir_is_listed() {
        ...
        let root = std::env::temp_dir().join(format!("houhub-cb-subdir-{}", uuid::Uuid::new_v4()));
