...
/// script body so a quirky data dir (`HOUHUB_DATA_DIR=$HOME/x`, paths with
...
pub fn create_credential_helper_script(
    ...
    #[cfg(unix)]
    {
        let script_path = app_data_dir.join("git-credential-houhub.sh");
        let content = format!(
            r#"#!/bin/sh
# Houhub credential helper — calls the app binary to look up credentials.
...
"#,
    ...
    #[cfg(windows)]
    {
        let script_path = app_data_dir.join("git-credential-houhub.bat");
        let content = format!(
            r#"@echo off
...
pub fn run_credential_helper() {
    ...
    // recent houhub versions embed this so server deployments and custom
    // HOUHUB_DATA_DIR setups land on the right database.
    let explicit_data_dir = parse_data_dir_arg(std::env::args());

    // Pin HOUHUB_DATA_DIR for downstream lookups (notably the file-based
    ...
    if let Some(dir) = &explicit_data_dir {
        std::env::set_var("HOUHUB_DATA_DIR", dir);
    }
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            tracing::error!("[houhub credential-helper] tokio runtime build failed: {e}");
            return;
        }
    };
    ...
    match rt.block_on(lookup_credential(&app_data_dir, &host)) {
        ...
        Err(e) => tracing::error!("[houhub credential-helper] lookup failed: {e}"),
    }
}
...
/// Look up a (username, token) pair for the given host using the houhub
...
fn resolve_app_data_dir() -> Option<std::path::PathBuf> {
    // On macOS: ~/Library/Application Support/app.houhub
    // On Linux: ~/.local/share/app.houhub
    // On Windows: %APPDATA%/app.houhub
    #[cfg(target_os = "macos")]
    {
        dirs::data_dir().map(|d| d.join("app.houhub"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::data_dir().map(|d| d.join("app.houhub"))
...
case "$1" in
*[Uu]sername*) echo "$HOUHUB_GIT_USERNAME" ;;
*[Pp]assword*) echo "$HOUHUB_GIT_PASSWORD" ;;
esac
"#;
            std::fs::write(&script_path, content)?;
            // Make executable
...
if %errorlevel% equ 0 (
    echo %HOUHUB_GIT_USERNAME%
    exit /b
)
echo %1 | findstr /i "password" >nul
if %errorlevel% equ 0 (
    echo %HOUHUB_GIT_PASSWORD%
    exit /b
)
"#;
            std::fs::write(&script_path, content)?;
...
pub fn inject_credentials(
    ...
    cmd.env("GIT_CONFIG_COUNT", "1")
        ...
        .env("HOUHUB_GIT_USERNAME", username)
        .env("HOUHUB_GIT_PASSWORD", token)
...
mod tests {
    ...
    fn test_parse_data_dir_arg_space_separated() {
        let args = vec![
            ...
            "--data-dir".to_string(),
            "/tmp/houhub-data".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
            Some(std::path::PathBuf::from("/tmp/houhub-data"))
        );
    }
    ...
    fn test_parse_data_dir_arg_equals_form() {
        let args = vec![
            "houhub".to_string(),
            "--data-dir=/tmp/houhub-data".to_string(),
            "--credential-helper".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
    ...
    fn test_parse_data_dir_arg_absent() {
        let args = vec!["houhub".to_string(), "--credential-helper".to_string()];
    ...
    fn test_parse_data_dir_arg_missing_value_treated_as_absent() {
        ...
        let args = vec!["houhub".to_string(), "--data-dir".to_string()];
        assert_eq!(parse_data_dir_arg(args), None);
    }
    ...
    fn test_helper_script_embeds_data_dir() {
        let tmp = std::env::temp_dir().join(format!("houhub-cred-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).expect("create tmp dir");
        let binary = std::path::PathBuf::from("/usr/local/bin/houhub-server");
        ...
        // deployments don't fall back to the hardcoded `app.houhub` path.
        // Paths are sh-single-quoted so spaces / `$` / backticks survive.
        assert!(content.contains("/usr/local/bin/houhub-server"));
    ...
    fn test_helper_script_absolutizes_relative_data_dir() {
        // Regression: a relative `HOUHUB_DATA_DIR=data` previously produced
        ...
        let _guard = STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let unique = format!("houhub-rel-test-{}", uuid::Uuid::new_v4());
        ...
        let binary = std::path::PathBuf::from("./houhub-server");
        ...
        assert!(
            !content.contains("'./houhub-server'"),
    ...
    fn test_bat_double_quote_escapes_percent() {
        ...
        assert_eq!(
            bat_double_quote(r"C:\Users\%USERNAME%\houhub"),
            "\"C:\\Users\\%%USERNAME%%\\houhub\""
        );
    }
    ...
    fn test_helper_script_escapes_apostrophe_in_path() {
        ...
        let tmp = std::env::temp_dir().join(format!("houhub-apos-test-{}", uuid::Uuid::new_v4()));
    ...
    fn test_credential_helper_e2e_server_mode() {
        ...
        let data_dir =
            std::env::temp_dir().join(format!("houhub-helper-e2e-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&data_dir).expect("create data dir");

        // Save & override HOUHUB_DATA_DIR for the duration of this test —
        // server-mode `keyring_store` resolves `tokens.json` from this var.
        let saved_env = std::env::var("HOUHUB_DATA_DIR").ok();
        std::env::set_var("HOUHUB_DATA_DIR", &data_dir);
        ...
        rt.block_on(async {
            ...
            // Seed the file token store. Uses HOUHUB_DATA_DIR via
        ...
        match saved_env {
            Some(v) => std::env::set_var("HOUHUB_DATA_DIR", v),
            None => std::env::remove_var("HOUHUB_DATA_DIR"),
        }
