...
pub struct ToolCallState {
    ...
    ///
    /// Convention used by houhub's multi-agent delegation (the `delegate_to_agent`
    /// MCP tool) — `DelegationBroker` writes the following object under
    /// `meta["houhub.delegation"]` on the parent's active tool call:
    ///
    /// ```jsonc
    /// {
...
/// rebuilds `meta["houhub.delegation"]` (status + child id) from the child's
...
pub struct SessionState {
    ...
    ///
    /// Cline forced this. houhub pins the provider with `CLINE_PROVIDER` — the
    ...
    ///
    /// houhub is what disabled the control, so houhub is what withholds it: these
    ...
    /// `TokenRegistry` when `houhub-mcp` is injected at init.
...
impl SessionState {
    ...
    /// houhub ships does (`upsert_tool_call` inserts at `Pending` and the
        ...
        self.active_tool_calls
            ...
            .map(|tc| {
                ...
                ToolCallState {
                    ...
                    // binding here (`meta["houhub.delegation"]`), which is what
...
mod tests {
    ...
    fn run_tool_call(s: &mut SessionState, id: &str, output_bytes: usize, settled: bool) {
        s.apply_event(&AcpEvent::ToolCall {
            ...
            meta: Some(serde_json::json!({ "houhub.delegation": { "status": "completed" } })),
            images: None,
        });
    ...
    ///
    /// Every agent houhub ships reports a terminal status, which is what makes
