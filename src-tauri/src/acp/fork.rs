...
///   to 0.74.0 it ignored the fingerprint entirely, so houhub sent the id alone;
...
pub fn resolve_fork_point(
    ...
    match agent_type {
        ...
        // houhub SYNTHESIZED with no text of its own (a `/goal` marker, a bare
        ...
        AgentType::ClaudeCode | AgentType::DeepSeek => {
            ...
            Some(ForkPoint {
                ...
                // the field is required, and houhub's own turn id (`turn-<n>`,
...
mod tests {
    ...
    /// The one case that must stay a tail fork: a turn houhub synthesized with
