...
pub(crate) const FS_POLICY_ENV: &str = "HOUHUB_ACP_FS_POLICY";
...
pub(crate) const FS_EXTRA_ROOTS_ENV: &str = "HOUHUB_ACP_FS_EXTRA_ROOTS";
...
/// AS LONG AS houhub also advertises `terminal(true)`: an agent refused a read
/// simply `cat`s the file through the shell houhub runs for it (empirically what
...
/// read gate can never work". Under `HOUHUB_ACP_HOST_TOOLS=agent` houhub hosts
...
/// agent's own sandboxed process. The escape hatch is houhub's terminal, not
...
impl FsAccessPolicy {
    ...
    /// session working directory. Reachable via `HOUHUB_ACP_FS_POLICY=strict`.
    ...
    /// No path gate at all in either direction. `HOUHUB_ACP_FS_POLICY=unrestricted`.
    ...
    /// the temp dirs (attachments), and any `HOUHUB_ACP_FS_EXTRA_ROOTS` entries.
    pub fn permissive(
        workspace_root: &Path,
    ...
    /// Resolve the policy for a connection from `HOUHUB_ACP_FS_POLICY`, checking
    /// the agent's `runtime_env` first (so it can be set per agent through the
    /// existing agent-settings `env_json`) then houhub's own process env.
    pub fn from_env(
        workspace_root: &Path,
        agent_type: AgentType,
        runtime_env: &BTreeMap<String, String>,
...
/// Read a knob VERBATIM from the agent's `runtime_env` first, then houhub's
...
/// Read a knob from the agent's `runtime_env` first, then houhub's process env,
/// trimmed — for values compared as keywords, never as paths.
///
/// Mirrors `HOUHUB_ACP_HOST_TOOLS` (`acp::host_tools_policy`): a houhub-only key
...
/// exactly this precedence — one notion of "a houhub launch knob" across both.
...
/// Extra writable roots from `HOUHUB_ACP_FS_EXTRA_ROOTS`. Relative entries are
/// dropped for the same reason as relative agent homes: they would be resolved
/// against houhub's cwd, so `.` or `..` silently widens the policy to an
...
/// `external_transcript_sources` uses) so houhub has exactly ONE notion of where
/// each agent lives. Where the agent's home is relocatable, the agent's
/// `runtime_env` wins over houhub's process env — a per-agent `GROK_HOME` in
...
fn resolve_root_slot(slot: &RootSlot, runtime_env: &BTreeMap<String, String>) -> Option<PathBuf> {
    for (key, suffix, expands) in slot.candidates {
        ...
        //
        // Absolutizing it would resolve against HOUHUB's cwd while the child runs
        // in the workspace cwd, and that mismatch WIDENS rather than merely
        // mismatching: `CODEX_HOME="."` with houhub launched from `/` yields the
        ...
        // credentials as a side effect. The agent still runs; only HouHub's own
        // `fs/*` channel declines to authorize the whole home, and
        // `HOUHUB_ACP_FS_EXTRA_ROOTS` can still name a narrower directory inside
        // it.
    ...
    // default — resolved against the CHILD's home, not houhub's (see
    // `child_home_dir`).
...
/// HouHub's is the one holding every other agent's credentials; authorizing
...
/// the home-relative defaults must follow the child's home. Using houhub's would
...
pub(crate) fn child_home_dir(runtime_env: &BTreeMap<String, String>) -> Option<PathBuf> {
    ...
    // real bug: `dirs::home_dir()` consults HOUHUB's `$HOME` before falling back to
    // the passwd entry, so it answers for houhub, not for a child whose `HOME` was
    // removed.
    let home = match runtime_env.get(HOME_KEY) {
        ...
        // passwd lookup — and houhub's `$HOME` is NOT that answer whenever the two
        // differ. Refuse rather than guess; the user can still name the directory
        // via `HOUHUB_ACP_FS_EXTRA_ROOTS`.
        ...
        // Absent: the child inherits houhub's environment, so houhub's answer is
        // exact — including its own passwd fallback when houhub has no `HOME`.
        None => {
            ...
            // relocating `USERPROFILE`, houhub's answer may not be the child's, so
    ...
    // absolutized against houhub's cwd — the same widening as a relative agent
...
fn agent_root_slots(agent_type: AgentType) -> &'static [RootSlot] {
    match agent_type {
        ...
        // A custom ACP agent has no houhub-known private directory layout —
        // houhub never reads its store (history comes from houhub's own ACP
...
mod tests {
    ...
    fn temp_workspace() -> PathBuf {
        let path = std::env::temp_dir().join(format!("houhub-fs-test-{}", uuid::Uuid::new_v4()));
    ...
    /// it is what `HOUHUB_ACP_FS_POLICY=strict` promises to restore.
    #[tokio::test(flavor = "current_thread")]
    ...
    fn agent_data_roots_honor_runtime_env_relocation() {
        let relocated = absolute_path("tmp/houhub-relocated-agent-home");
        let cases = [
    ...
    fn relocation_suffixes_match_the_agents_own_resolvers() {
        let base = absolute_path("tmp/houhub-relocation-base");
        let cases = [
    ...
    /// A `~` follows the CHILD's home, not HouHub's.
    ///
    ...
    #[test]
    fn tilde_expands_against_the_childs_home_not_houhubs() {
        let houhub_home = dirs::home_dir().expect("home dir");
        ...
        assert!(
            !roots.contains(&houhub_home.join("profile")),
            "HouHub's home must not: {roots:?}"
        );
    }
    ...
    /// — while HouHub, separately, created a literal `~` folder next to wherever
    ...
    /// re-inherit houhub's process env — mirroring the launch contract.
    #[test]
    ...
    fn relocation_excludes_the_inactive_default_root() {
        let relocated = absolute_path("tmp/houhub-isolated-profile");
        let cases = [
            (AgentType::Codex, "CODEX_HOME"),
            (AgentType::Grok, "GROK_HOME"),
            (AgentType::ClaudeCode, "CLAUDE_CONFIG_DIR"),
    ...
    /// through to houhub's process env would aim the root at a profile the child
    ...
    fn blank_runtime_value_falls_through_to_the_next_candidate() {
        let xdg = absolute_path("tmp/houhub-xdg");
    ...
    fn child_env_value_models_the_spawn_layers_env_remove() {
        let key = "HOUHUB_TEST_FS_CHILD_ENV";
        ...
        temp_env::with_var(key, Some("/parent/value"), || {
            ...
            // Non-empty replaces houhub's value, verbatim when not trimming.
            assert_eq!(
    ...
    /// against houhub's cwd while the child runs in the workspace cwd, and that
    /// mismatch widens: `"."` with houhub launched from `/` yields `/`, which every
    ...
    /// defaults must therefore follow the CHILD's home: anchoring them on houhub's
    /// would grant the inactive profile and refuse the active one.
    #[cfg(not(windows))]
    #[test]
    fn home_relative_defaults_follow_the_childs_home() {
        let houhub_home = dirs::home_dir().expect("home dir");
        let isolated = PathBuf::from("/tmp/houhub-isolated-home");
        let runtime_env = BTreeMap::from([
            ("HOME".to_string(), isolated.to_string_lossy().to_string()),
            // Explicitly remove the relocatable agent override so a developer
            // shell's CODEX_HOME/GROK_HOME/etc. cannot change this test's child.
        ...
        for (agent_type, rel) in [
            ...
            assert!(
                !roots.contains(&houhub_home.join(rel)),
                "{agent_type:?}: inactive {} must not stay writable, got {roots:?}",
                houhub_home.join(rel).display()
            );
        }
    }

    /// A relative child `HOME` would be absolutized against houhub's cwd — the same
    ...
    /// account database — which houhub cannot read without duplicating a passwd
    /// lookup, and `dirs::home_dir()` does NOT answer because it consults houhub's
    /// own `$HOME` first. So the slot is refused rather than guessed at.
    ///
    /// Asserting "no roots" is stronger than asserting "not houhub's root", and it
    ...
    fn removed_child_home_refuses_the_slot() {
        ...
        for agent_type in ALL_AGENT_TYPES {
            ...
            assert!(
                roots.is_empty(),
                "{agent_type:?}: a removed HOME must yield no root (never houhub's \
                 {}), got {roots:?}",
                houhub_home.display()
            );
        }
    ...
    fn relative_extra_roots_are_dropped() {
        let absolute = absolute_path("tmp/houhub-abs-extra");
        let runtime_env = BTreeMap::from([(
            FS_EXTRA_ROOTS_ENV.to_string(),
    ...
    fn cursor_relocation_respects_resolver_precedence() {
        let config_dir = absolute_path("tmp/houhub-cursor");
        let runtime_env = BTreeMap::from([
            (
            ...
            (
                "XDG_CONFIG_HOME".to_string(),
                absolute_path("tmp/houhub-xdg").to_string_lossy().to_string(),
            ),
        ]);
    ...
    fn pi_session_dir_relocation_is_honored() {
        let sessions = absolute_path("tmp/houhub-pi-sessions-elsewhere");
        let runtime_env = BTreeMap::from([(
