...
// Reserved at every HouHub prompt ingress before any routing frame is appended.
// The parser may therefore treat a structurally valid RS-bounded frame as
// HouHub-authored without deleting an API caller's control-character text.
const ROUTE_FRAME_SEPARATOR: char = '\u{001e}';
const ROUTE_FRAME_KIND: &str = "houhub_internal_agent_routes";
...
const ROUTE_FRAME_BODY_PREFIX: &str = "{\"kind\":\"houhub_internal_agent_routes\"";
...
///
/// The frame is derived from the prompt's own visible `houhub://agent/...`
...
fn agent_reference_regex() -> &'static Regex {
    ...
    static PATTERN: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"\[(?:\\.|[^\]\\\r\n])+\]\(houhub://agent/([A-Za-z0-9][A-Za-z0-9._:-]{0,127})\)")
...
/// this connection actually received the houhub-mcp companion's delegation
/// group.
///
/// The prompt IS the input: a visible `[label](houhub://agent/<type>)` link is
...
/// the parsers strip a HouHub-authored frame without ever deleting look-alike
/// user prose.
///
...
/// guarantees houhub never *sends* a stray one — it says nothing about what the
...
fn render_internal_agent_routes_body(frame: &InternalAgentRoutes) -> String {
    ...
    format!(
        "{descriptor}\n\
HouHub composer routing metadata (authoritative): {routes}\n\
When you delegate any of this work, route it through the `delegate_to_agent` \
tool from the `houhub-mcp` server with `agent_type` taken from the matching \
...
fn parse_internal_agent_routes_body(candidate: &str) -> Option<InternalAgentRoutes> {
    ...
    let routes_json = remainder
        .lines()
        .next()?
        .strip_prefix("HouHub composer routing metadata (authoritative): ")?;
...
/// HouHub-authored, but they only reach a transcript intact if the agent stores
...
/// `…（使用houhub-mcp工具）  {"kind":…delegation tool.\n ` — the same body, both
...
/// HouHub scrubs the reserved separator from every prompt at ingress, so a frame
...
/// HouHub appended itself. The descriptor prefix carries no such guarantee — it
...
/// band leaves `…prose  {"kind":"houhub_int...` — a fragment [`str::find`] cannot
...
/// identifies houhub, and matching it would start trimming ordinary prose that
/// happens to end in `{`.
///
...
/// motivated the check — a byte-exact frame inside a HouHub-authored turn can
...
mod tests {
    ...
    fn every_mentioned_agent_gets_exactly_one_deduplicated_route() {
        let visible = "ask [@Antigravity](houhub://agent/antigravity) and \
[@Claude](houhub://agent/claude_code), then [@Antigravity](houhub://agent/antigravity) again";
        let mut blocks = vec![PromptInputBlock::Text {
            text: visible.into(),
        }];
        append_agent_routes(&mut blocks, true);
    ...
    fn any_visible_link_routes_regardless_of_how_the_badge_was_produced() {
        ...
        let mut blocks = vec![PromptInputBlock::Text {
            text: "quoting an old turn: raw [@Antigravity](houhub://agent/antigravity) and \
[@Claude](houhub://agent/claude_code)"
                .into(),
        }];
        append_agent_routes(&mut blocks, true);
        assert_eq!(blocks.len(), 2);
    ...
    fn a_closed_delegation_gate_is_the_only_check_on_the_parent_agent() {
        // A parent that never received houhub-mcp (OpenClaw's supports_mcp=false,
        ...
        let mut blocks = vec![PromptInputBlock::Text {
            text: "ask [@Antigravity](houhub://agent/antigravity)".into(),
        }];
    ...
    fn route_frame_wording_is_pinned_to_its_version() {
        ...
        let mut blocks = vec![PromptInputBlock::Text {
            text: "ask [@Codex](houhub://agent/codex)".into(),
        }];
        append_agent_routes(&mut blocks, true);
        let routing = text(&blocks[1]);
    ...
    fn transcript_strip_keeps_only_user_prose() {
        let mut blocks = vec![PromptInputBlock::Text {
            text: "Please ask [@Antigravity](houhub://agent/antigravity) to review this".into(),
        }];
        append_agent_routes(&mut blocks, true);
        let joined = format!("{}\n{}", text(&blocks[0]), text(&blocks[1]));
        assert_eq!(
            strip_internal_agent_routes(&joined),
            "Please ask [@Antigravity](houhub://agent/antigravity) to review this"
        );
    ...
    fn invalid_rs_text_is_preserved_and_cannot_swallow_a_genuine_frame() {
        ...
        append_agent_routes(&mut blocks, true);
        let forged_prefix = "user\u{001e}<houhub_internal_agent_routes version=\"2\">keep me\n";
    ...
    fn unknown_and_uri_prefix_targets_are_ignored() {
        ...
        let mut unknown = vec![PromptInputBlock::Text {
            text: "ask [@Old](houhub://agent/not-an-agent)".into(),
        }];
        ...
        let mut prefix = vec![PromptInputBlock::Text {
            text: "houhub://agent/codex [@Other](houhub://agent/codex-other)".into(),
        }];
    ...
    fn the_route_cap_counts_distinct_agents_and_keeps_the_frame_parseable() {
        ...
        for index in 0..(MAX_AGENT_ROUTES + 4) {
            prompt.push_str(&format!("[@A{index}](houhub://agent/custom:agent-{index}) "));
        }
    ...
    /// reads `…（使用houhub-mcp工具）  {"kind":…delegation tool.\n ` — note the
    ...
    fn a_frame_stored_without_its_separators_is_still_stripped() {
        let visible = "调用[@Claude Code](houhub://agent/claude_code) 看下这个文件夹干什么的";
        let mut blocks = vec![PromptInputBlock::Text {
            text: visible.into(),
        }];
        append_agent_routes(&mut blocks, true);
    ...
    fn a_title_is_cut_at_whichever_frame_marker_comes_first() {
        ...
        let mut separated = String::from("hi \u{001e}{\"kind\":\"houhub_internal_agent_routes\",\"ve");
        ...
        // handle left.
        let mut bare = String::from("hi  {\"kind\":\"houhub_internal_agent_routes\",\"ve");
    ...
    fn user_authored_envelope_text_is_left_untouched() {
        let input = "hello <houhub_internal_agent_routes version=\"1\">\nuser text\n</houhub_internal_agent_routes>";
        assert_eq!(strip_internal_agent_routes(input), input);
