...
//!
//! houhub advertises `fs.readTextFile` / `fs.writeTextFile` / `terminal` on
...
//! houhub then services both **in houhub's own process**
...
//! agent's process tree. houhub's process is not in it, so a kernel deny on
...
//!
//! houhub cannot fix that by *containing* the agent. Refusing the capabilities
...
pub(crate) const HOST_TOOLS_ENV: &str = "HOUHUB_ACP_HOST_TOOLS";
...
pub enum HostToolsPolicy {
    /// houhub advertises and services `fs/*` + `terminal/*`. Historical
    /// behaviour and still the default: most agents ship no sandbox, so there
    /// is nothing to restore, and houhub's hosting buys a consistent cwd and a
    /// single place to apply
    /// [`FsAccessPolicy`](crate::acp::file_system_runtime::FsAccessPolicy)'s
    /// write roots.
    Default,
    /// houhub advertises NEITHER channel, and refuses both if called anyway.
    ...
    ///
    /// It also withholds houhub-mcp's `delegation` tool group (see
    /// `inject_houhub_mcp`). `delegate_to_agent` is a third door into the same
    /// room: it has houhub spawn a SECOND agent, in houhub's process tree under
    ...
    /// reporting) surface houhub's own state rather than executing anything on
    /// the user's machine, so they stay.
    ///
    /// The cost, and why this is opt-in rather than the default: houhub's fs
...
impl HostToolsPolicy {
    ...
    /// houhub's own process env — the same precedence, and the same helper, as
    ...
    /// Whether houhub hosts the `fs/*` and `terminal/*` channels this launch.
    ...
    pub fn describe(self) -> &'static str {
        match self {
            Self::Default => "houhub (fs + terminal advertised)",
...
mod tests {
    ...
    fn value_is_trimmed_and_case_sensitive() {
        temp_env::with_var_unset(HOST_TOOLS_ENV, || {
            ...
            // But it is not case-folded, matching HOUHUB_ACP_FS_POLICY. The UI
    ...
    fn per_agent_env_json_wins_over_process_env() {
        ...
        // channels back without changing how houhub hosts every other agent.
    ...
    fn the_reported_agent_mode_covers_the_process_env_layer() {
        ...
        // the knob for houhub itself sets it for every agent whose own env_json
