...
//! `houhub-mcp`. Unlike live-feedback ([`crate::acp::feedback`]), which is a
...
pub struct QuestionSpec {
    ...
    /// houhub-mcp ask tool still requires [`MIN_OPTIONS`] at its own parse
    /// layer).
...
pub fn validate_specs(specs: &[QuestionSpec]) -> Result<(), String> {
    ...
    for (qi, q) in specs.iter().enumerate() {
        ...
        // Unlike `parse_questions` (the houhub-mcp tool contract, which keeps
...
/// Some native asks carry NO `header` (the short category chip HouHub renders):
...
/// in the SAME interactive card as the houhub-mcp ask tool.
///
...
/// looking exactly like they did before HouHub advertised `elicitation.form`
...
/// `agentInfo.version` (`SessionState::codex_user_input_shape`) — houhub's pin is
...
/// assumptions about keys they do not own, and this parser is shared — houhub
...
///      since the pin is what houhub launches.
...
///
/// houhub's card already offers a free-text "Other" input on every question and
...
///     fallback. This includes approvals for houhub-mcp's OWN tools in
...
/// adapter's shape.
///
/// Counts are clamped to HouHub's bounds because
/// [`crate::acp::manager::ConnectionManager::register_question`] re-runs
...
pub fn classify_elicitation(
    ...
    let ElicitationMode::Form(form) = &req.mode else {
        // HouHub only advertises `elicitation.form` — URL mode goes down
...
/// which falls back to the main field. If houhub ever advertises
...
/// HouHub synthesizes for a native ask that resolves out-of-band rather than as a
...
/// toggle independently — `houhub-mcp` is injected when ANY is enabled, and each
...
mod tests {
    ...
    /// were enough to unlock the codex fixups, a third party could make houhub
    ...
    /// the JSON Schema reading houhub has always used — `description` is the
