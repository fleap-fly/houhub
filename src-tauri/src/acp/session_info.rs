...
//! agent's prompt as `[title](houhub://session/<conversation_id>)`. The agent
...
pub struct SessionInfo {
    pub found: bool,
    /// The id that was queried (houhub's internal conversation PK). Echoed so the
...
/// Listener-facing access to resolve a session by its houhub conversation id. The
...
pub trait SessionInfoAccess: Send + Sync {
    /// Resolve `session_id` (houhub's internal conversation PK) into a
...
/// independently — `houhub-mcp` is injected when ANY feature is enabled, and each
