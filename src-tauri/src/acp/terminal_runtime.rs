...
/// drop, and HouHub never sets `kill_on_drop`).
...
pub struct TerminalRuntime {
    ...
    /// the agent's per-request `env` is applied. This is where the HouHub
    ...
    /// default to the folder the conversation runs in instead of HouHub's own
...
impl TerminalRuntime {
    ...
    fn configure_command(
        ...
        // CodeBuddy omit it, which would otherwise inherit HouHub's own process
        ...
        // HouHub credential helper), then layer the agent's request env on top
    ...
    pub async fn create_terminal(
        ...
        // `which` guess that runs in HouHub's own cwd/env — means we never
...
mod tests {
    ...
    /// layer with the HouHub credential helper's `GIT_CONFIG_*` keys —
    ...
    async fn base_env_propagates_and_request_env_overrides() {
        ...
        base_env.insert("HOUHUB_TEST_BASE_VAR".to_string(), "from_base".to_string());
        base_env.insert("HOUHUB_TEST_OVERRIDE".to_string(), "loses".to_string());
        ...
        request.args = vec![
            ...
            "printf '%s\\n' \"$HOUHUB_TEST_BASE_VAR\" \"$HOUHUB_TEST_OVERRIDE\"".into(),
        ];
        request.env = vec![EnvVariable::new("HOUHUB_TEST_OVERRIDE", "request_wins")];
    ...
    /// configured working directory rather than HouHub's own process cwd.
    #[tokio::test]
    ...
    async fn explicit_missing_cwd_surfaces_as_spawn_failure() {
        ...
        request.cwd = Some(PathBuf::from("/houhub-nonexistent-cwd/does/not/exist"));
    ...
    /// check, run in HouHub's own cwd, would shell-wrap and split the path.
    #[tokio::test]
