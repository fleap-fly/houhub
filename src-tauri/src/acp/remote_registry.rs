...
///
/// This is the catalog houhub does NOT ship built-ins for: the registry lists
/// far more agents than houhub has hand-written support for, and every one of
...
pub struct RegistryCatalogAgent {
    ...
    /// Channels this entry publishes, in houhub's preference order.
    pub distribution_kinds: Vec<String>,
    /// houhub already ships a hand-written integration for this id.
    pub builtin: bool,
...
/// Fetch the full ACP registry, annotated with what houhub can do with each
...
/// Registry ids that name an agent houhub already ships, under a different id
...
mod tests {
    ...
    fn builtin_ids_are_recognized_without_the_custom_registry() {
        ...
        // Published under a different id than houhub's own (`kimi-code`), so
