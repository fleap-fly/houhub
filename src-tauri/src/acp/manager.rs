...
/// Read the spawn-handshake timeout from `HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS`,
...
fn spawn_handshake_timeout_from_env() -> Duration {
    let secs = std::env::var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
pub struct ConnectionManager {
    ...
    /// `houhub-mcp` appear in the agent's MCP server list during ACP
...
impl ConnectionManager {
    ...
    pub async fn spawn_agent(
        ...
        // latencies and tune `HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS`.
    ...
    pub async fn send_prompt_linked_with_message_id(
        ...
        crate::acp::prompt_hydration::hydrate_prompt_blocks(
            &mut blocks,
            &crate::paths::houhub_uploads_root(),
        )
        .await?;
            ...
            let handle = tokio::spawn(async move {
                ...
                let outcome: Result<(), AcpError> = async {
                    ...
                    let preserved = if let Some(eid) = eid_opt {
                        // THE houhub#500 write. Branch A above adopts a row the
    ...
    /// (and `turn_in_flight` with it) tracks turns HOUHUB started, and a goal
    ...
    /// what `houhub-mcp` will pass through to `session/set_config_option`
...
/// fail any git command that depends on the houhub-injected helper.
#[derive(Clone)]
...
mod tests {
    ...
    #[tokio::test]
    async fn a_codex_goal_clear_interrupts_even_when_houhub_thinks_it_is_idle() {
        // HouHub's `Prompting` only covers turns IT started. A goal loop's
    ...
    fn spawn_handshake_timeout_from_env_uses_default_when_unset() {
        ...
        let prev = std::env::var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS").ok();
        std::env::remove_var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS");
        let default = spawn_handshake_timeout_from_env();
        assert_eq!(default, Duration::from_secs(SPAWN_HANDSHAKE_TIMEOUT_SECS));

        std::env::set_var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", "5");
        assert_eq!(spawn_handshake_timeout_from_env(), Duration::from_secs(5));

        std::env::set_var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", "garbage");
        assert_eq!(
            spawn_handshake_timeout_from_env(),
        ...
        // Restore.
        match prev {
            Some(v) => std::env::set_var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS", v),
            None => std::env::remove_var("HOUHUB_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS"),
        }
    }
    ...
    async fn fork_session_links_unbound_row_from_caller_ids() {
        ...
        // so fork_session must adopt the caller-supplied row and then fork.
        use crate::acp::connection::ConnectionCommand;
        use crate::db::test_helpers;

        let db = test_helpers::fresh_in_memory_db().await;
        let folder_id = test_helpers::seed_folder(&db, "/tmp/houhub-fork-relink").await;
        let pre = conversation_service::create(
            &db.conn,
            folder_id,
            AgentType::ClaudeCode,
