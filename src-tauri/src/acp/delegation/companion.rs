//! Companion-side MCP protocol — the bits that live inside the `houhub-mcp`
...
/// won't notice the bound being hit. Misses are absorbed by the HouHub
...
/// Static MCP tool schema. Lives next to this module so houhub-mcp ships
...
/// Which tool groups this companion exposes. One `houhub-mcp` process can carry
...
pub struct CompanionFeatures {
    ...
    /// has open, and nothing else houhub hands an agent is a window onto what
...
async fn build_tools_call_spawn(
    ...
    match name.as_str() {
        ...
        "get_session_info" => {
            // `session_id` is the HouHub conversation id the agent read out of a
            // `houhub://session/<id>` reference. Accept a JSON number or a numeric
                ...
                None => {
                    return LineAction::Respond(err(
                        ...
                        "get_session_info requires an integer `session_id` \
                         (the number in the houhub://session/<id> reference)",
                    ));
                }
            };
        ...
        "browser_snapshot" => {
            ...
            // on the houhub side — which is what leaves the line on the tab's
        ...
        | "browser_select_option" => {
            ...
            // audit line are the same for all of them on the houhub side.
            ...
            // line it leaves on the strip is written on the houhub side.
            let round_trip =
        ...
        "browser_console_messages" => {
            ...
            // A registry read on the houhub side; the grant check and the
        ...
        "browser_screenshot" => {
            ...
            // the houhub side, which is what leaves the line on the strip.
            let round_trip =
        ...
        "browser_eval" => {
            ...
            // would take the question away mid-read while the houhub side went
...
/// can't pin shutdown — the HouHub main side's `cancel_by_parent` cascade
...
///
/// The length check is here as well as on the houhub side, so an oversized
...
/// — the houhub-side one is the gate, this one is the message.
...
mod tests {
    ...
    fn ctx_with(features: CompanionFeatures) -> CompanionContext {
        CompanionContext {
            ...
            socket_path: "/tmp/houhub-mcp-companion-test-nope.sock".into(),
            token: "tok".into(),
            features,
    ...
    async fn initialize_returns_protocol_version() {
        ...
        assert_eq!(result["serverInfo"]["name"], "houhub-mcp");
    }
    ...
    fn render_session_result_not_found_is_soft_with_note_text() {
        let outcome = json!({
            ...
            "note": "No session matches id 9. It may have been deleted, or never imported into HouHub."
        });
    ...
    async fn feedback_spawn_commits_after_delivery() {
        ...
        // live in, and houhub exports a 72-byte per-session `TMPDIR` to the
    ...
    /// houhub side wrote — neither is an `isError`, because both are things to
