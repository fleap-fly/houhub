//! Lifecycle handle for the houhub-mcp broker socket — the half of the
//! companion round-trip that lives inside houhub's own process.
//!
...
mod tests {
    ...
    /// `/.tmpXXXXXX/houhub-delegation-*.sock` on top — about 38 bytes. That is
    /// fine against the ~20-byte default and fatal against houhub's own
    /// per-session `TMPDIR`, which is 72 bytes (`/var/folders/…/T/houhub-acp/
    /// <pid>-<hex>`): the composed path reaches 110 and `sun_path` caps at 104
    /// on macOS. The whole suite went red inside a houhub session and green
    ...
    fn over_long_socket_path(holder: &Path) -> PathBuf {
        const NAME: &str = "houhub-delegation-unreachable.sock";
    ...
    async fn start_binds_probeable_socket_and_survives_a_restart() {
        let dir = socket_dir();
        let socket = dir.path().join("houhub-delegation-test.sock");
    ...
    async fn ensure_running_is_a_noop_while_the_socket_answers() {
        let dir = socket_dir();
        let service = make_service(dir.path().join("houhub-delegation-noop.sock"));
    ...
    async fn a_failed_rebind_leaves_the_working_acceptor_alone() {
        ...
        let socket = nest.join("houhub-delegation-keep.sock");
        let service = make_service(socket.clone());
    ...
    async fn rebinding_replaces_the_socket_without_ever_unlinking_the_path() {
        ...
        let dir = socket_dir();
        let socket = dir.path().join("houhub-delegation-swap.sock");
        let service = make_service(socket.clone());
    ...
    async fn concurrent_ensure_running_calls_settle_on_one_acceptor() {
        let dir = socket_dir();
        let service = make_service(dir.path().join("houhub-delegation-race.sock"));
