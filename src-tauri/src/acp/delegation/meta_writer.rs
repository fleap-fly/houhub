...
//! tool-call. The shape written under `meta["houhub.delegation"]`
...
pub const DELEGATION_META_KEY: &str = "houhub.delegation";

/// Capability the broker uses to patch `meta["houhub.delegation"]` on
...
pub trait DelegationMetaWriter: Send + Sync {
    ...
    /// round-trip reveals which houhub-mcp tool the call actually is and with
...
impl DelegationMetaWriter for ConnectionManagerMetaWriter {
    async fn write_meta(
        ...
        // `inferLiveToolName` reads "has houhub.delegation meta" as
...
/// Helper to construct the canonical `meta["houhub.delegation"]` value.
