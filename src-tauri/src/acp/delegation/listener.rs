//! Main-process side of the `houhub-mcp` round-trip: accept UDS / named-pipe
...
impl TokenRegistry {
    ...
    /// launch, so `companions` counts injected `houhub-mcp` processes; the two
...
impl DelegationListener {
    ...
    /// is — `.stg-<pid>-<8 hex>` is 8 bytes under `houhub-delegation-<pid>.sock`
    ...
    /// safe: the staging directory is `$TMPDIR`, shared with every other houhub
    ...
    /// (`houhub-delegation-<pid>.sock`): `sun_path` caps a unix socket address
        ...
        C: AsyncReadExt + AsyncWriteExt + Unpin,
    {
        ...
        let resp = match msg {
            ...
            BrokerMessage::BrowserCapture(req) => {
                ...
                // houhub side whether or not the caller waits.
            ...
            BrokerMessage::BrowserTabOp(req) => {
                // Bounded by the settle timeout on the houhub side. No
    ...
    ///
    /// SCOPE (deliberate, user-confirmed): the lookup is by houhub conversation id
    ...
    /// resolves. This is sound in houhub's single-tenant trust model: there is no
    /// per-user isolation anywhere (desktop is one local user; server mode shares
    /// one `HOUHUB_TOKEN` + one data dir across an operator's devices), the user can
...
/// hands a child — this is houhub's own end of the identical budget.
#[cfg(unix)]
...
/// unscoped name would be owned by whichever user ran houhub first and
...
/// `$TMPDIR/houhub-delegation-<pid>.sock` has always had exactly the same
...
/// houhub instances on the same machine don't collide.
///
...
/// housekeeping: houhub exports a ~72-byte per-session `TMPDIR` to the agents it
/// launches, so a houhub started from inside one is already within a few bytes
...
///
/// Windows: a named pipe address `\\.\pipe\houhub-delegation-<pid>`. Windows
...
pub fn default_socket_path(temp_dir: &Path) -> PathBuf {
    let name = format!("houhub-delegation-{}.sock", std::process::id());
    ...
    // `/tmp/houhub-` + at most 10 digits of euid + `/houhub-delegation-` + at
...
mod tests {
    ...
    fn an_over_long_temp_dir_falls_back_to_the_short_socket_dir() {
        let name = format!("houhub-delegation-{}.sock", std::process::id());
        // A macOS `/var/folders/…/T` with houhub's own per-session nesting under
        ...
        let suffix = "/T/houhub-acp/12345-deadbeef";
    ...
    ///
    /// `.stg-<pid>-<8 hex>` is shorter than `houhub-delegation-<pid>.sock`, which
    ...
    async fn the_chosen_socket_path_can_actually_be_bound_and_dialed() {
        let ambient = PathBuf::from(format!(
            "/var/folders/hl/{}/T/houhub-acp/12345-deadbeef",
            "z".repeat(60)
        ));
        let path = default_socket_path(&ambient);
