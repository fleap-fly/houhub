...
//!
//! Result durability: child output is NOT stored in houhub's DB, so the broker
...
/// child output isn't stored in houhub's DB; the full result lives in the
...
fn attach_blocked(report: &mut DelegationTaskReport, blocked: BlockedOn) {
    ...
    report.message = Some(format!(
        "Running.\n{BLOCKED_LINE_PREFIX} {what}\nThe sub-agent is parked until \
         a human answers in HouHub — it is not making progress. Tell the user, \
         then wait again."
    ));
...
struct PendingToolCall {
    ...
    /// *some* houhub-mcp call whose identity only the companion round-trip can
...
pub struct DelegationBroker {
    ...
    /// Writer for `meta["houhub.delegation"]` on the parent's active
...
impl DelegationBroker {
    ...
    /// houhub-mcp tool (`get_delegation_status` / `cancel_delegation`) with
    ...
    /// right now. Surfaced by the houhub-mcp service-status indicator so the
...
/// task's terminal status (NOT its text — child output isn't in houhub's DB)
...
mod tests {
    ...
    async fn emitter_records_after_meta_write_on_complete_call() {
        // Frontend's snapshot-recovery path reads `meta["houhub.delegation"]`
