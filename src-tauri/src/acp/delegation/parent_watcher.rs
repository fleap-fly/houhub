//! Self-cleanup watchdog for `houhub-mcp`.
//!
...
//! (agent CLI that hangs, parent houhub crash that orphans the agent) can
//! leave `houhub-mcp` running forever, holding open the binary file and a
//! companion connection that no one will ever read from.
//!
//! When the parent houhub / houhub-server passes `--parent-pid <pid>` on
//! the command line, `houhub-mcp` spawns this watchdog. It polls the OS
...
//! invisible next to the other work `houhub-mcp` does.
//!
...
/// Default polling cadence. Fast enough that a stale `houhub-mcp.exe`
...
/// permission glitch can't cause the watchdog to kill `houhub-mcp` while
