//! Wire format for `houhub-mcp` companion ↔ main process round-trip over UDS
...
//! ### Version coupling
//!
//! The companion (`houhub-mcp`) and the listener (inside the houhub main
...
//! `houhub-mcp` uses an absolute path that is replaced atomically by the
...
pub struct BrokerRequest {
    ...
    /// anything else.
    pub token: String,
    /// houhub-internal ACP connection UUID for the parent session.
...
/// Resolve a session the user referenced (`houhub://session/<id>`) into its
...
/// lookup is by houhub's internal conversation id (the number in the reference),
...
pub struct BrokerSessionRequest {
    pub token: String,
    /// houhub's internal conversation PK (the number in `houhub://session/<id>`).
    pub session_id: i32,
...
pub enum BrokerMessage {
    ...
    /// companion — it comes from houhub's own service-status check
...
/// `{"ok": true}` answer back. Used by the houhub-mcp service-status indicator
...
mod tests {
    ...
    async fn uds_round_trip() {
        ...
        let path = dir.path().join("houhub-mcp.sock");
