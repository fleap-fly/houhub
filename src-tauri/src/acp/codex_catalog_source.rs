...
//!
//! Because `model_catalog_json` is a whole-table replace, Houhub must reproduce
//! codex's own catalog inside the generated file — so it has to match the codex
//! Houhub **actually launches**. That codex is the one **nested** under the
...
//!
//! The cache lives under [`crate::paths::houhub_home_dir`] (not the app data dir)
...
/// Candidate codex-acp package dirs: the global npm prefix and Houhub's user
/// prefix (`~/.Houhub/npm-global`, used when a global install hit EACCES).
