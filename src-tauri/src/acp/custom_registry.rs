...
pub struct CustomAgentDef {
    ...
    /// already follow. houhub cannot detect where an arbitrary ACP agent loads
    ...
    /// built-in houhub-mcp companion (delegation, live feedback, task tools).
    ///
    /// Not every ACP agent does: some reject any entry in that field and fail
    /// session creation outright, so a custom agent that houhub cannot connect
...
/// (`binary_cache::binary_dir`), which houhub also *deletes* when reinstalling —
...
pub fn build_meta(def: &CustomAgentDef) -> Result<AcpAgentMeta, CustomAgentError> {
    ...
    Ok(AcpAgentMeta {
        ...
        // for a custom agent, the houhub-mcp companion is the whole of it. On
...
fn build_binary_distribution(
    ...
    let dir_entry = if nested {
        ...
        Some(BinaryDirEntry {
            ...
            // The ACP registry has no field for this, and houhub cannot know
...
mod tests {
    ...
    fn the_mcp_declaration_reaches_the_launch_metadata() {
        ...
        // `connection.rs` reads to decide whether houhub-mcp goes out on
        // `session/new`.
