...
/// Root for houhub-managed agent binaries.
///
/// Resolution order mirrors `paths.rs`:
/// 1. `$HOUHUB_HOME/acp-binaries`
/// 2. `$HOUHUB_DATA_DIR/acp-binaries` (server mode)
/// 3. `<data-local>/app.houhub/acp-binaries`
///
...
pub(crate) fn cache_dir() -> Result<PathBuf, AcpError> {
    ...
    Ok(base.join("app.houhub").join("acp-binaries"))
}
...
pub(crate) fn legacy_cache_dir() -> Option<PathBuf> {
    let legacy = dirs::cache_dir()?.join("app.houhub").join("acp-binaries");
    let current = cache_dir().ok()?;
    (legacy != current && legacy.is_dir()).then_some(legacy)
}

/// Directory where houhub caches a managed `uv` toolchain (`uv` + `uvx`),
...
/// Locate a houhub-managed uv tool binary (`uv` or `uvx`) if it has already
...
/// Pinned `uv` toolchain version houhub downloads on demand when the user has no
...
/// Download + cache the `uv` toolchain (`uv` + `uvx`) into houhub's cache when no
...
fn migrate_root(legacy: &Path, current: &Path) {
    ...
    for entry in entries {
        ...
        // deletes the intact source over. And a second houhub running the same
...
/// peer houhub sweeping while this one is mid-copy would delete the staging tree
...
/// Drop staging directories left behind by a houhub that is no longer running.
///
...
fn copy_tree(from: &Path, to: &Path) -> std::io::Result<()> {
    ...
    if meta.file_type().is_symlink() {
        ...
        #[cfg(windows)]
        {
            ...
            // two roots are the same path unless `HOUHUB_HOME`/`HOUHUB_DATA_DIR`
...
fn install_extracted_tree(
    ...
    if let Some(parent) = entry_path.parent() {
        ...
        // check from ever rejecting a tree houhub itself installed.
...
mod tests {
    ...
    /// houhub, a copy this one is still assembling.
    #[test]
    ...
    /// one whose owner is still alive is left alone, because a peer houhub may
    ...
    // Custom agents download from URLs houhub did not vet, so a published
