...
async fn build_agent(
    ...
    let agent = match meta.distribution {
        AgentDistribution::Npx { cmd, args, env, .. } => {
            ...
            // codex-acp honors APP_SERVER_LOGS as a directory for its
            // adapter-side logs. Keep normal launches unchanged and enable it
            // only with HouHub's explicit diagnostic flag; a cache-directory
            // failure must never prevent an ACP connection from starting.
    ...
    Ok(match working_dir {
        Some(dir) => agent.with_current_dir(dir),
        None => agent,
    })
}
...
/// `_meta.codex.subagent = {threadId, path, activity}`. HouHub already renders
...
/// `parsers/codex.rs`), so this new live signal is redundant with what HouHub
