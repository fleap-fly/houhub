...
/// Everything the UI needs to explain that houhub's entry for this agent is an
...
pub struct AdapterInfo {
    /// npm spec houhub installs, e.g. "@agentclientprotocol/claude-agent-acp@0.81.0".
    ...
    /// Where the user's own vendor CLI was found, if at all. houhub never
...
async fn check_uv_environment(
    ...
    // launchable as-is, but installing uv unlocks houhub's managed install /
...
async fn check_binary_environment(
    ...
    if platform_supported {
        let cache_check = match binary_cache::find_best_cached_binary_for_agent(agent_type, cmd) {
            ...
            // installed. `dir_entry` describes the shape of the archive houhub
