...
//!
//! houhub creates, loads and resumes sessions with UNTYPED requests (so it can
...
//! prompt helpers houhub never used (it sends `session/prompt` itself and reads
...
/// runtime's own router errors on it instead, which is why houhub claims those
