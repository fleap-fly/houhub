...
//!
//! houhub sends `session/new`, `session/load` and `session/resume` itself —
...
//! session id, feeding an unbounded channel — minus the prompt helpers houhub
//! never used. houhub sends `session/prompt` itself and reads the stop reason off
...
/// runtime's own router errors on it instead, which is why houhub claims the
...
mod tests {
    ...
    /// houhub creates the session UNTYPED (as `send_new_session_capturing_models`
