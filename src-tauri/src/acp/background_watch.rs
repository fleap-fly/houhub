...
impl PromptLedger {
    ...
    /// conversation open on the same folder. houhub is a multi-agent
    ...
    pub(crate) fn record_prompt_blocks(&self, blocks: &[crate::acp::types::PromptInputBlock]) {
        ...
        // new transcript file. This is the only notice houhub gets.
...
mod tests {
    ...
    fn steered_message_classifies_foreground_like_a_prompt() {
        ...
        assert!(
            ...
            "the HouHub-sent prompt classifies foreground"
        );
    ...
    #[test]
    fn houhub_sent_prompt_is_foreground_and_not_surfaced() {
        let dir = tempfile::tempdir().unwrap();
        let path = temp_session(&dir);
        write_lines(&path, &[]);
        let ledger = PromptLedger::shared();
    ...
    /// A page the built-in browser handed over, as houhub sends it with nothing
    ...
    fn a_page_sent_with_nothing_typed_stays_a_foreground_turn() {
        ...
        assert!(
            tick_now(&mut ws, &ledger).is_none(),
            "the prompt houhub sent must not come back as an overlay turn"
        );
