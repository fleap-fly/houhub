...
/// houhub's own earlier splice, which named the local from a table instead of
/// from the bundle. Matched so the wrong-local installs it left behind can be
/// repaired in place; see [`repair_range`].
const LEGACY_HOUHUB_POLICY: &str = concat!(
...
pub enum CompatPatchStatus {
    ...
    AlreadyFixed,
    Applied,
    /// An earlier houhub splice was found and rewritten against the local this
...
fn legacy_houhub_policy() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(LEGACY_HOUHUB_POLICY).expect("legacy policy is a valid regex"))
}
...
struct BundleSplice {
    ...
    action_local: String,
    /// True when `range` covers an earlier houhub splice being rewritten.
    repair: bool,
}
...
/// The range of an earlier houhub splice, when the bundle carries one.
///
...
fn repair_range(content: &str) -> Option<std::ops::Range<usize>> {
    let mut matches = legacy_houhub_policy().captures_iter(content);
    let first = matches.next()?;
    if matches.next().is_some() {
        return None;
    }
...
fn plan_splice(content: &str) -> Option<BundleSplice> {
    ...
    // Repair first: an install carrying houhub's earlier splice no longer has
...
pub fn maybe_apply(platform_dir: &Path, version: &str) -> CompatPatchStatus {
    ...
    let Some(plan) = plan_splice(&content) else {
        ...
        let status = if legacy_houhub_policy().is_match(&content) {
            tracing::error!(
                "Cursor ACP retry compatibility patch: bundle carries houhub's earlier \
...
mod tests {
    ...
    // "already fixed", so the repair has to recognise houhub's own handiwork and
