...
//! 107.70 GB, which nothing in houhub would otherwise ever come back for —
//! those directories are in the SYSTEM temp dir, not in houhub's own subtree.
//!
//! # Why this is user-triggered and not a startup sweep
//!
//! `%TEMP%\_MEI*` is the namespace of every PyInstaller application on the
//! machine, not houhub's. houhub cannot tell its own leaked directory from one
...
/// Whether an entry is one houhub is willing to delete.
///
