...
//! temp directory on every launch and only clean up on a GRACEFUL exit. houhub
...
//! then `USERPROFILE`. Pointing those at a directory houhub owns turns an
//! unbounded leak into a directory houhub can delete.
//!
...
/// but nothing except houhub writes here.
const SCRATCH_NAMESPACE: &str = "houhub-acp";
...
/// to keep for houhub's OWN broker socket. One definition rather than two
...
/// is not spawned at all when `HOUHUB_ACP_IDLE_TIMEOUT_SECS=0`, and disabling
...
/// disabled with `HOUHUB_ACP_TMP_ISOLATION=0`.
...
/// INSIDE it. Isolation costs `/houhub-acp/<pid>-<hex>` — about 25 bytes — and
...
/// account to run houhub would own `houhub-acp` and every other account would be
...
/// is what the sweeps actually need — [`sweep_roots`] visits every root houhub
...
/// exactly as it did before. A launch must never fail because houhub could not
...
/// Refuse a scratch root houhub did not create.
///
/// Guards the two ways `/tmp/houhub-acp-<euid>` can be waiting for us: a
...
fn verify_root_is_ours(root: &Path) -> std::io::Result<()> {
    ...
    // Ours, but reachable by others — a root left behind by a houhub that
    ...
    // pointed `HOUHUB_ACP_TMP_ROOT` at may not carry Unix modes at all, and
...
/// it points while houhub deleted an empty scratch directory and reported
...
/// Reclaim scratch directories left by OTHER houhub processes that have exited.
///
...
/// access-denied cannot happen — and wrong here, where one houhub instance is
...
/// `<owning houhub pid>-<8 hex>`.
///
/// The pid is HOUHUB'S, not the agent's: the directory is created before the
/// spawn, so the agent has no pid yet. It is what lets a startup sweep tell
/// "some other houhub owns this" from "its owner is gone".
...
mod tests {
    ...
    /// also absorb `/houhub-acp/<pid>-<hex>` — which is the whole of #754.
    #[cfg(unix)]
    ...
    /// `<ambient>/houhub-acp/<pid>-<hex>/znr-<uuid>.sock` — 119 bytes — and
    ...
    /// houhub that predates the `0700`. Refusing that would turn every upgrade
    ...
    fn a_loose_root_we_own_is_tightened_rather_than_refused() {
        ...
        let holder = tempfile::tempdir().expect("tempdir");
        let root = holder.path().join("houhub-acp");
