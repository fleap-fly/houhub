...
/// Run git against a throwaway index, leaving the worktree's real index
/// untouched. Split indexes are disabled so git cannot leave sharedindex files
/// behind next to the scratch index.
...
/// Add a task worktree from an exact base revision. The general folder command
/// intentionally keeps its legacy three-argument API; task runs need the
/// pinned revision so a branch cannot accidentally start from a concurrently
/// advanced HEAD.
pub async fn worktree_add_at(
    path: &str,
    branch: &str,
    worktree_path: &str,
    start_point: &str,
    ...
    if out.status.success() {
        ...
        Err(git_command_error("worktree add", &out.stderr))
    }
}
...
impl ScratchIndex {
    ...
    async fn build(wt_path: &str, anchor: &str) -> Result<Self, AppCommandError> {
        ...
        let path = git_dir.join(format!("houhub-diff-index-{}-{seq}", std::process::id()));
...
/// Resolve the tip of an exact local branch ref. Tags and similarly named
/// refs are deliberately excluded from the lookup.
...
/// Re-create a checkout of an existing local branch after its worktree
/// directory disappeared, and verify that git attached the branch rather than
/// silently falling back to a detached tag checkout.
...
/// Whether a local task branch still contains commits/tree changes not present
/// on the recorded base. Errors conservatively keep the branch.
pub async fn branch_holds_unlanded_work(
    ...
    if let Some(base_branch) = base_branch {
        match local_branch_tip(repo_path, base_branch).await {
            Ok(Some(value)) => base_ref = Some(value),
            Ok(None) => {}
            Err(_) => return true,
        }
    }
    ...
    match is_ancestor(repo_path, &tip, &base_ref).await {
        Ok(true) => false,
        Ok(false) => match trees_equal(repo_path, &base_ref, &tip).await {
            Ok(equal) => !equal,
            Err(_) => true,
        },
        Err(_) => true,
...
mod tests {
    ...
    async fn the_acceptance_measure_sees_work_that_was_never_committed() {
        ...
        let leftovers: Vec<_> = std::fs::read_dir(dir.path().join(".git"))
            ...
            .filter(|n| n.starts_with("houhub-diff-index"))
            .collect();
