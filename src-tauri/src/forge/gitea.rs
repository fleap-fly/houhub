...
/// the one houhub opened for it last time — and it is the one place this client
...
/// `open` / `closed` / `merged`, the three words the rest of houhub understands.
...
mod tests {
        ...
        let app = axum::Router::new()
            ...
            .route("/api/v1/repos/acme/app/pulls", {
                ...
                .post(move |Json(body): Json<serde_json::Value>| {
                    ...
                    async { Json(pull_json(8, "open", false, "houhub/task-1")) }
                })
            })
            .route(
    ...
    async fn a_draft_pull_request_is_a_prefixed_title() {
        ...
        assert_eq!(seen.wrote("pull")[0]["head"], "houhub/task-1");
