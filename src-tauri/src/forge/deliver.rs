...
pub struct DeliveryCtx<'a> {
    ...
    /// houhub data dir — where the GIT_ASKPASS helper script lives.
    pub data_dir: &'a Path,
...
/// What IS refused is a fork houhub cannot even name — a deleted or invisible
...
pub fn pull_is_workable(
    ...
    {
        return Err(format!(
            "{noun} #{} comes from a fork whose repository houhub cannot see (it may be private \
...
/// How a houhub task id is written into text that lands on a forge.
///
/// The number is houhub-local: it names a row in this workspace's task list,
...
/// Body of the pull request houhub opens. `Closes #N` is what links it to the
...
pub fn pull_request_body(issue_url: &str, issue_number: i64, task_id: i32) -> String {
    ...
    format!(
        "Closes #{issue_number}\n\n\
         Prepared by [houhub](https://github.com/xggz/houhub) work task {task} \
         from {issue_url}.\n\n\
...
/// The comment houhub posts when a task finishes.
///
...
pub fn writeback_comment_body(
    ...
    match outcome {
        TaskOutcome::Merged { commit, base_branch } => {
            ...
            format!(
                "houhub work task {task} is done — merged locally into `{base_branch}` as \
                 `{short}`{numbers}."
            )
        }
        TaskOutcome::Delivered { pr_url } => {
            format!("houhub work task {task} is done — {pr_url}{numbers}.")
        }
        TaskOutcome::Accepted { nothing_to_land: true } => {
            format!("houhub work task {task} is done — accepted with nothing to land.")
        }
        TaskOutcome::Accepted { nothing_to_land: false } => {
            format!("houhub work task {task} is done — accepted without merging{numbers}.")
        }
    }
}
...
mod tests {
    ...
    fn the_write_back_body_is_numbers_and_links_only() {
        ...
        // A code-spanned id, never `#12`: the number is houhub's, and both
