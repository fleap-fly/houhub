...
pub enum ForgeError {
    ...
    /// The host is not one houhub can read: nothing is configured for it and its
    ...
    /// The host answered in a way only the OTHER forge answers — houhub had it
...
/// but houhub lets the user change the proxy at runtime — so the client is
