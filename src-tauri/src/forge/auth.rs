    ...
    let account = match account_id {
        ...
        None => on_host
            ...
            .ok_or_else(|| {
                ...
                // about an instance houhub positively identified. What such a
...
/// What houhub knows about a host, from the accounts configured for it.
...
pub struct HostProfile {
    ...
    /// Whether `provider` is something houhub actually KNOWS about this host —
...
/// is as close as houhub gets to "this is not a forge we speak".
///
...
const NONEXISTENT_PATH: &str = "__houhub_forge_probe";
...
/// would pin the host to the hostname guess until houhub restarts, which is the
...
mod tests {
    ...
    /// host houhub can read. Saying which is which is the whole point of
    ...
    /// — and this function then answers that request with "not a forge houhub
    ...
    async fn resolution_prefers_pinned_id_then_default_and_redacts_debug() {
        ...
        temp_env::async_with_vars([("HOUHUB_DATA_DIR", Some(dir.as_str()))], async move {
            let db = db_conn;
    ...
    async fn resolution_happy_path(conn: &sea_orm::DatabaseConnection) {
        ...
        // is not a missing account, it is a forge houhub does not speak — and
    ...
    /// the host to the hostname guess until houhub restarts, which is the exact
