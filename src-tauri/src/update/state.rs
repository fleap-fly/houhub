...
mod tests {
    ...
    fn a_command_error_publishes_its_explanation_alongside_the_message() {
        let h = new_handle();
        let e = EventEmitter::Noop;
        let err = AppCommandError::permission_denied("Update target is not writable: /opt/houhub")
            .with_detail("Permission denied (os error 13)")
            .with_i18n(
                "SystemSettings.updateErrors.permissionDenied",
                std::collections::BTreeMap::from([("path".to_string(), "/opt/houhub".to_string())]),
            );
        ...
        assert_eq!(wire["error"], "Update target is not writable: /opt/houhub");
        assert_eq!(wire["errorInfo"]["code"], "permission_denied");
        assert_eq!(
            wire["errorInfo"]["i18n_key"],
            "SystemSettings.updateErrors.permissionDenied"
        );
        assert_eq!(wire["errorInfo"]["i18n_params"]["path"], "/opt/houhub");
