...
pub fn asset_basename() -> Option<&'static str> {
    ...
    Some(match (OS, ARCH) {
        ("linux", "x86_64") => "houhub-server-linux-x64",
        ("linux", "aarch64") => "houhub-server-linux-arm64",
        ("macos", "x86_64") => "houhub-server-darwin-x64",
        ("macos", "aarch64") => "houhub-server-darwin-arm64",
        ("windows", "x86_64") => "houhub-server-windows-x64",
        _ => return None,
    })
}
...
fn resolve_targets() -> Result<Targets, AppCommandError> {
    ...
    // real one.
    let web_dir = match std::env::var("HOUHUB_STATIC_DIR")
        .ok()
...
fn upgrade_marker_path() -> Option<PathBuf> {
    crate::update::runtime::self_exe()
        .parent()
        .map(|d| d.join(".houhub-upgrade-staged"))
}
...
fn check_writable(dir: &Path) -> Result<(), AppCommandError> {
    let probe = dir.join(format!(".houhub-write-probe-{}", std::process::id()));
...
pub async fn perform_update(
    ...
    let staging = data_dir.join(format!(".houhub-update-{}", std::process::id()));
    ...
    if !new_server.is_file() || !new_mcp.is_file() || !new_web.is_dir() {
        return Err(AppCommandError::new(
            crate::app_error::AppErrorCode::TaskExecutionFailed,
            "Downloaded update is incomplete (expected houhub-server, houhub-mcp and a web/ directory)",
        ));
    }
...
fn extract_tar_gz(bytes: &[u8], dest: &Path, max: u64) -> Result<(), AppCommandError> {
    ...
    for entry in entries {
        ...
        if etype.is_dir() {
            ...
            extracted += written;
            // Preserve unix mode so +x on houhub-server / houhub-mcp survives.
            #[cfg(unix)]
...
mod tests {
    ...
    fn sanitize_keeps_normal_paths() {
        let p = sanitize_entry_path(Path::new("houhub-server-linux-x64/web/index.html")).unwrap();
        assert_eq!(p, PathBuf::from("houhub-server-linux-x64/web/index.html"));
    }
    ...
    fn replace_file_keeps_backup_and_swaps() {
        ...
        let target = dir.path().join("houhub-server");
    ...
    /// `/usr/local/share/houhub` in the deployments this guards).
    fn upgraded_install(dir: &Path) -> Targets {
        ...
        let server_bin = bindir.join("houhub-server");
