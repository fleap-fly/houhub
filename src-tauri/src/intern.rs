...
mod tests {
    ...
    fn intern_returns_pointer_identical_slices() {
        let a = intern("houhub-intern-test-alpha");
        let b = intern(&String::from("houhub-intern-test-alpha"));
        assert_eq!(a, b);
        assert!(
    ...
    fn intern_keeps_distinct_values_distinct() {
        let a = intern("houhub-intern-test-one");
        let b = intern("houhub-intern-test-two");
        assert_ne!(a, b);
    }
