...
mod tests {
    ...
    fn register_is_idempotent_and_unregister_removes() {
        let root = Path::new("/tmp/houhub-registry-a/root");
        let target = Path::new("/tmp/houhub-registry-a/target");
    ...
    fn unknown_paths_are_denied() {
        ...
        assert!(!is_allowed(
            Path::new("/tmp/houhub-registry-b/root"),
            Path::new("/tmp/houhub-registry-b/elsewhere/file.txt")
        ));
    }
}
