...
/// `HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS`; `0` disables the sweep.
...
/// so (a) the master `HOUHUB_TOKEN` never enters the preview iframe, and (b) a
...
/// Read the idle timeout from `HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS`. `0`
/// disables the sweep; unparseable falls back to the default.
pub fn idle_timeout_from_env() -> Option<Duration> {
    let secs = match std::env::var("HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS") {
...
mod tests {
    ...
    async fn ref_count_sharing_and_teardown() {
        let dir = std::env::temp_dir().join(format!("houhub-ow-ref-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn idle_timeout_env_parsing() {
        std::env::set_var("HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS", "0");
        assert!(idle_timeout_from_env().is_none());
        std::env::set_var("HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS", "nope");
        assert_eq!(
            idle_timeout_from_env().unwrap().as_secs(),
            DEFAULT_IDLE_TIMEOUT_SECS
        );
        std::env::set_var("HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS", "120");
        assert_eq!(idle_timeout_from_env().unwrap().as_secs(), 120);
        std::env::remove_var("HOUHUB_OFFICE_WATCH_IDLE_TIMEOUT_SECS");
        assert_eq!(
            idle_timeout_from_env().unwrap().as_secs(),
            DEFAULT_IDLE_TIMEOUT_SECS
        );
