...
/// effect at exec: a `fork()` on another thread — HouHub forks constantly, for
...
/// Return the user-local npm prefix directory (`~/.houhub/npm-global/`).
///
...
pub(crate) fn user_npm_prefix() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".houhub").join("npm-global"))
}
