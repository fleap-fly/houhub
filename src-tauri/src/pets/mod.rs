...
//! `paths::houhub_pets_root()` and is **decoupled from Tauri** so the same
...
use crate::paths::houhub_pets_root;
...
mod tests {
    ...
    // Filesystem-touching tests rely on `HOUHUB_HOME`, which is shared global
