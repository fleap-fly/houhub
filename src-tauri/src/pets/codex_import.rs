//! One-way import: Codex pets → houhub pets.
//!
//! Codex stores pets at `${CODEX_HOME:-~/.codex}/pets/<id>/` with the same
//! `pet.json` + `spritesheet.webp` layout houhub uses. We copy the directory
...
/// Copy selected Codex pets into `~/.houhub/pets/`. Each entry copies the full
        ...
        let target_id = if pet.already_imported {
            if !request.overwrite_with_suffix {
                skipped.push(ImportSkipped {
                    ...
                    reason: "Pet with this id already exists in houhub.".to_string(),
                });
                continue;
            }
