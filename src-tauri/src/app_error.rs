...
/// `uploads_root/` directory past `HOUHUB_UPLOAD_MAX_TOTAL_BYTES`. The
...
/// The file is not an houhub backup, or its `format_version` is
...
/// that wrote it runs a newer houhub. Params: `snapshotVersion`, `appVersion`.
...
/// The file is not a houhub config snapshot, or its JSON is malformed.
