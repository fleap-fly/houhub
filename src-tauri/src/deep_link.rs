//! OS-level `houhub://` URL scheme.
//!
//! In-app markdown already uses `houhub://session/<id>` and `houhub://agent/<type>`
//! as reference links. Those stay in-process badges. This module is the
//! **system** handler: a click in another app, `open houhub://…`, or a browser
//! custom-scheme navigation should bring the desktop workspace forward and
//! focus the named conversation.
//!
//! Supported forms:
//!
//! - `houhub://session/<id>` — HouHub's numeric conversation PK, or an agent's
//!   `external_id` (Grok UUID, Codex thread id, …)
//! - `houhub://workspace?conversationId=<id>` — same lookup; `folderId` and
//!   `agent` are optional and filled from the row when omitted
//! - `houhub://open` / `houhub://` — just show the main window
//!
//! `houhub://agent/…`, `houhub://commit/…`, and `houhub://embedded/…` are
...
/// What a parsed `houhub://` URL wants the desktop app to do.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DeepLink {
    /// Bring the workspace forward without targeting a conversation.
    Open,
    /// Focus a conversation looked up by `houhub://session/<ref>`.
...
/// A `houhub://` link resolved to a conversation the workspace can open.
///
...
/// The one channel a resolved `houhub://` target travels on.
///
...
/// Only the desktop URL handler parks targets; `houhub-server` compiles the
...
/// Collect `houhub:` URLs from a process argv (Windows / Linux second launch).
...
mod tests {
    ...
    fn parses_session_numeric_id() {
        assert_eq!(
            parse_deep_link("houhub://session/214"),
            Some(DeepLink::Session {
    ...
    fn parses_session_external_id() {
        assert_eq!(
            parse_deep_link("houhub://session/0193c0de-aaaa-7bbb-8ccc-ddddeeeeffff"),
            Some(DeepLink::Session {
                session_ref: "0193c0de-aaaa-7bbb-8ccc-ddddeeeeffff".into()
            })
        );
        assert_eq!(
            parse_deep_link("houhub://session/codex_abc%20123"),
            Some(DeepLink::Session {
    ...
    fn rejects_path_traversal_and_in_app_badges() {
        assert_eq!(parse_deep_link("houhub://session/../etc/passwd"), None);
        assert_eq!(parse_deep_link("houhub://session/"), None);
        assert_eq!(parse_deep_link("houhub://agent/grok"), None);
        assert_eq!(parse_deep_link("houhub://commit/abc"), None);
        assert_eq!(parse_deep_link("https://example.com/session/1"), None);
    }

    #[test]
    fn parses_open_and_workspace_query() {
        assert_eq!(parse_deep_link("houhub://"), Some(DeepLink::Open));
        assert_eq!(parse_deep_link("houhub://open"), Some(DeepLink::Open));
        assert_eq!(
            parse_deep_link("houhub://workspace?conversationId=9&folderId=3&agent=grok"),
            Some(DeepLink::Workspace {
                folder_id: Some(3),
                conversation_id: 9,
        ...
        assert_eq!(
            parse_deep_link("houhub://workspace?conversationId=9"),
            Some(DeepLink::Workspace {
                folder_id: None,
                conversation_id: 9,
                agent: None,
        ...
        assert_eq!(parse_deep_link("houhub://workspace"), None);
    }

    #[test]
    fn argv_keeps_only_houhub_urls() {
        let argv = [
            "/Applications/houhub.app/Contents/MacOS/houhub",
            "houhub://session/12",
            "--flag",
        ];
        assert_eq!(urls_from_argv(&argv), vec!["houhub://session/12".to_string()]);
    }

    #[tokio::test]
    async fn resolves_numeric_and_external_ids() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-deep-link").await;
    ...
    async fn numeric_ref_falls_back_to_external_id() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-deep-link-numeric").await;
    ...
    async fn startup_path_resolves_first_session_url_else_plain_workspace() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-deep-link-startup").await;
        let id = seed_conversation(&db, folder, AgentType::Grok).await;

        assert_eq!(startup_workspace_path(&db, &[]).await, "workspace");
        assert_eq!(
            startup_workspace_path(&db, &["houhub://open".into()]).await,
            "workspace"
        );
        assert_eq!(
            startup_workspace_path(&db, &["houhub://session/999999".into()]).await,
            "workspace"
        );
        assert_eq!(
    ...
    async fn workspace_query_fills_agent_and_rejects_mismatch() {
        let db = fresh_in_memory_db().await;
        let folder = seed_folder(&db, "/tmp/houhub-deep-link-ws").await;
        let id = seed_conversation(&db, folder, AgentType::Grok).await;
