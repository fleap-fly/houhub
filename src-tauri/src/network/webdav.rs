...
impl WebdavClient {
    ...
    fn url_for(&self, rel: &str) -> Result<Url, WebdavError> {
        let mut url = self.base.clone();
        {
            ...
            // pushing onto it without dropping that would yield `/dav//houhub`.
...
mod tests {
    ...
    fn base_url_without_trailing_slash_still_appends() {
        let url = client().url_for("houhub/v1/default/config.json").expect("url");
        assert_eq!(
            url.as_str(),
            "https://dav.example.com/dav/houhub/v1/default/config.json"
        );
    }
    ...
    fn trailing_slash_in_the_configured_url_changes_nothing() {
        let with_slash = WebdavClient::new("https://dav.example.com/dav/", "u", "p")
            .expect("client")
            .url_for("houhub/v1/default/config.json")
            .expect("url");
        assert_eq!(
            with_slash.as_str(),
            "https://dav.example.com/dav/houhub/v1/default/config.json"
    ...
    fn segments_are_percent_encoded() {
        let url = client().url_for("houhub/my drive/config.json").expect("url");
    ...
    fn path_segments_cannot_escape_the_configured_directory() {
        assert_eq!(sanitize_path_segment(" houhub "), Some("houhub".to_string()));
