...
/// from houhub (`inherited`), and whatever the launch itself already sets.
///
/// Merged rather than left to inheritance because a per-agent `NO_PROXY`
/// would replace only its own spelling: the child would still inherit houhub's
...
/// the settings, or an exported `HTTP_PROXY` houhub passes through — no agent
...
///
/// houhub's own settings can no longer produce one (they are normalized before
...
/// `HTTP_PROXY`; houhub's setting writes all of them with one value, so the
...
mod tests {
    ...
    /// houhub's list, and curl, Bun, Python and Node read that one first.
    #[test]
