...
/// flags and the `eval "$HOUHUB_CMD"` wrapping for shells we know speak that
        ...
        match detect_windows_shell_flavor(shell) {
            WindowsShellFlavor::Cmd => {
                if let Some(command) = initial_command {
                    cmd.env("HOUHUB_CMD", command);
                    // Set UTF-8 code page before running the actual command
                    cmd.args(["/D", "/S", "/C", "chcp 65001 >nul & %HOUHUB_CMD%"]);
                } else {
            ...
            WindowsShellFlavor::PowerShell => {
                if let Some(command) = initial_command {
                    ...
                    cmd.args([
                        ...
                        "-NoProfile",
                        "-Command",
                        "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $ErrorActionPreference = 'Stop'; Invoke-Expression $env:HOUHUB_CMD",
                    ]);
                } else {
                    ...
                    cmd.args([
                        ...
                        "-NoExit",
                        "-Command",
                        "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $host.UI.RawUI.WindowTitle = 'houhub'",
                    ]);
                }
            }
            WindowsShellFlavor::Posix => {
                ...
                cmd.env("TERM_PROGRAM", "houhub");
                cmd.env("LANG", "C.UTF-8");
                if let Some(command) = initial_command {
                    cmd.env("HOUHUB_CMD", command);
                    cmd.args(["-l", "-i", "-c", "eval \"$HOUHUB_CMD\""]);
                } else {
                    cmd.args(["-l", "-i"]);
                }
            }
