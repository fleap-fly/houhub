...
/// houhub process already holds the lock (e.g. a desktop app and a server pointed
/// at the same `HOUHUB_DATA_DIR`), or — rarely — when the lock can't be
...
pub fn build_engine(
    ...
    let engine_lock = match acquire_engine_ownership(&data_dir) {
        ...
        Ownership::Taken => {
            tracing::info!(
                "[automation] another houhub process owns the automation engine for {}; \
...
/// isolated `houhub-dev.db` never blocks a release `houhub.db`, and vice versa.
...
/// `bin/houhub_server.rs` via `tokio::spawn`).
