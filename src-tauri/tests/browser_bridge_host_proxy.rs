...
//! (`HOUHUB_BRIDGE_PUBLIC_HOST` set), where `X-Forwarded-Host` is believed.
//!
...
/// The hostname the proxy in front publishes houhub at.
const PUBLIC: &str = "houhub.test";
/// What the proxy puts in `Host` when it forwards: its own upstream address,
/// which names no bridge target.
const INTERNAL: &str = "houhub-internal";
...
async fn the_proxys_forwarded_host_routes_and_a_pages_cannot_escape() {
    ...
    // the bridge's and never becomes houhub's pages on the target's origin.
    let response = get(target, Some(PUBLIC.to_string())).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "hello from upstream");

    // And houhub's own public name is still houhub's, from either header.
