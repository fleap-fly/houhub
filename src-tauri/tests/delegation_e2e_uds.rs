...
/// `/.tmpXXXXXX/houhub-e2e-*.sock` on top. That is fine against the ~20-byte
/// default and fatal against houhub's own per-session `TMPDIR`, which is 72
/// bytes: `houhub-e2e-batch.sock` composed to exactly 104 there, and `sun_path`
/// caps at 104 on macOS. Three of these tests went red inside a houhub session
...
impl houhub_lib::acp::feedback::SessionFeedbackAccess for NoFeedback {
    ...
    ) -> Vec<houhub_lib::acp::feedback::PendingFeedback> {
        Vec::new()
    }
...
impl houhub_lib::acp::session_info::SessionInfoAccess for NoSessionInfo {
    async fn resolve(
        ...
        session_id: i32,
        _max_messages: u32,
    ) -> houhub_lib::acp::session_info::SessionInfo {
        houhub_lib::acp::session_info::SessionInfo::not_found(session_id)
    }
}
...
async fn end_to_end_uds_happy_path() {
    ...
    let dir = socket_dir();
    let socket = dir.path().join("houhub-e2e.sock");
...
async fn end_to_end_uds_batch_status() {
    ...
    let dir = socket_dir();
    let socket = dir.path().join("houhub-e2e-batch.sock");
    let socket_for_listener = socket.clone();
    let listener_task = tokio::spawn(async move {
        let _ = listener.run(socket_for_listener).await;
    });
...
async fn end_to_end_uds_invalid_token_rejected() {
    ...
    let dir = socket_dir();
    let socket = dir.path().join("houhub-e2e-reject.sock");
    let socket_for_listener = socket.clone();
    let listener_task = tokio::spawn(async move {
        let _ = listener.run(socket_for_listener).await;
    });
...
async fn end_to_end_uds_ask_question_round_trip() {
    ...
    let dir = socket_dir();
    let socket = dir.path().join("houhub-e2e-ask.sock");
    let socket_for_listener = socket.clone();
    let listener_task = tokio::spawn(async move {
        let _ = listener.run(socket_for_listener).await;
    });
...
async fn end_to_end_uds_ask_revoked_after_register_declines() {
    ...
    let dir = socket_dir();
    let socket = dir.path().join("houhub-e2e-ask-revoked.sock");
    let socket_for_listener = socket.clone();
    let listener_task = tokio::spawn(async move {
        let _ = listener.run(socket_for_listener).await;
    });
