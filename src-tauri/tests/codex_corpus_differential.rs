...
//!
//! ```sh
//! HOUHUB_CODEX_CORPUS_DIR=~/.codex/sessions \
//! HOUHUB_CODEX_CORPUS_OUT=/tmp/codex-digest-branch.txt \
...
use houhub_lib::parsers::codex::CodexParser;
use houhub_lib::parsers::AgentParser;
...
fn codex_corpus_digest() {
    let Ok(root) = std::env::var("HOUHUB_CODEX_CORPUS_DIR") else {
        eprintln!("HOUHUB_CODEX_CORPUS_DIR unset — skipping");
        return;
    };
    let root = PathBuf::from(shellexpand_home(&root));
    let out = std::env::var("HOUHUB_CODEX_CORPUS_OUT")
