...
use houhub_lib::db::test_helpers::fresh_disk_db;
...
async fn build_server() -> (TestServer, tempfile::TempDir, tempfile::TempDir) {
    ...
    houhub_lib::db::service::folder_service::add_folder(&db.conn, "/tmp/proj")
        .await
        .expect("seed folder");
...
async fn export_download_upload_inspect_stage_roundtrip() {
    ...
    let form = MultipartForm::new().add_part(
        "file",
        Part::bytes(bytes).file_name("houhub-backup.houhub.zip"),
    );
    let up = server
    ...
    assert!(data_dir
        .path()
        .join(".houhub-restore-pending.json")
        .is_file());
}
