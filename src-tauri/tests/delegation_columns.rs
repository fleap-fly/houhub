...
use houhub_lib::db::entities::conversation;
use houhub_lib::db::test_helpers::{fresh_in_memory_db, seed_folder};
use houhub_lib::models::agent::AgentType;
use sea_orm::{ActiveModelTrait, EntityTrait, NotSet, Set};

#[tokio::test]
async fn delegation_columns_round_trip() {
    let db = fresh_in_memory_db().await;
    let folder_id = seed_folder(&db, "/tmp/houhub-delegation-test").await;
...
async fn delegation_columns_default_to_null_on_existing_create() {
    let db = fresh_in_memory_db().await;
    let folder_id = seed_folder(&db, "/tmp/houhub-delegation-null").await;
    // The existing create helper does not set the new columns; verify they default to None.
    let conv_id =
        houhub_lib::db::test_helpers::seed_conversation(&db, folder_id, AgentType::ClaudeCode).await;
