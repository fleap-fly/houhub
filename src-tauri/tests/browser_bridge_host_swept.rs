...
use axum::Router;
use houhub_lib::web::browser_bridge::{self, BridgeConfig, BridgeGrant, HostPattern};

const WORKBENCH: &str = "houhub.test";
/// HouHub's own port here; no target can ever be opened on it, so a name
...
fn cookie_for(grant: &BridgeGrant) -> String {
    let cap = grant
        ...
        .unwrap();
    format!("houhub-bridge-{}={cap}", grant.target_port)
}
...
/// left a service worker behind, and houhub's own pages served under that
/// name would be served through it.
#[tokio::test]
async fn a_name_that_was_handed_out_never_becomes_houhubs() {
    configure_once();
    ...
    let grant = browser_bridge::open(upstream, "tab-swept", Some(WORKBENCH))
        .await
        .unwrap();
    let host = format!("{}:{houhub}", grant.bridge_host.as_deref().unwrap());
    ...
    // No target holds the name now — and it is still not houhub's.
    let response = get(host.clone(), true).await.unwrap();
    ...
    // houhub: under `auto` the shape alone is nobody's claim, or a workbench
    ...
    for config in [
        ...
        // And a name never handed out still reaches houhub, whichever way
        // the bridge is pointed now.
        let response = get(format!("{RESERVED_PORT}.{WORKBENCH}:{houhub}"), false)
            .await
            .unwrap();
        assert_eq!(response.text().await.unwrap(), "houhub's own page", "off: {off}");
    }
}
