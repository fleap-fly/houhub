...
//! it were wrong.
//!
//! So the fixture in `tests/fixtures/antigravity/` is NOT written by the houhub
...
//! harness writes. What this test proves is that houhub's subset agrees with the
...
use houhub_lib::models::{ContentBlock, TurnRole};
use houhub_lib::parsers::{antigravity::AntigravityParser, AgentParser};
...
fn decodes_a_trajectory_encoded_by_googles_own_descriptors() {
    ...
    // and none of houhub's MCP-aware cards can match it. The server does not
    ...
    // ── a step arm houhub does not model ────────────────────────────────────
    ...
    // in `attachments` (tag 155) — real fields houhub deliberately does not
