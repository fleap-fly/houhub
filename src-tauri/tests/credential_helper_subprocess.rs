...
//! the actual `houhub-server` binary with `--credential-helper`, pipes the
...
use houhub_lib::db::service::app_metadata_service;
...
async fn helper_subprocess_emits_username_and_password_for_seeded_host() {
    let data_dir =
        std::env::temp_dir().join(format!("houhub-helper-subproc-{}", uuid::Uuid::new_v4()));
    ...
    // contention.
    {
        let db = houhub_lib::db::init_database(&data_dir, "test")
            .await
            .expect("init db");
    ...
    // process. The helper subprocess gets `HOUHUB_DATA_DIR` via its
    ...
    let binary = env!("CARGO_BIN_EXE_houhub-server");
    let mut child = Command::new(binary)
        .arg("--credential-helper")
        .arg("--data-dir")
        .arg(&data_dir)
        // Keep the env minimal so the subprocess uses --data-dir, not
        // whatever HOUHUB_DATA_DIR happens to be set to in the test runner.
        .env_clear()
        .env("PATH", std::env::var("PATH").unwrap_or_default())
        .env("HOUHUB_DATA_DIR", &data_dir)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn houhub-server --credential-helper");
...
async fn helper_subprocess_outputs_nothing_for_unconfigured_host() {
    ...
    {
        let _db = houhub_lib::db::init_database(&data_dir, "test")
            .await
            .expect("init db");
    }
