...
use axum::Router;
use houhub_lib::web::browser_bridge::{self, BridgeConfig, BridgeError, BridgeGrant};
...
/// houhub's own does. What a `#[tokio::test]` spawns stops when its test
...
/// `browser_bridge::open` from that runtime, as houhub's API handler calls it
...
fn cookie_for(grant: &BridgeGrant) -> String {
    format!("houhub-bridge-{}={}", port_of(grant), cap_of(grant))
}
...
async fn requests_need_this_listeners_cookie() {
    ...
    let response = client()
        .get(&url)
        .header(header::COOKIE, format!("houhub-bridge-{}=wrong", port_of(&grant)))
        .send()
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::FORBIDDEN);
    let response = client()
        ...
        .header(
            header::COOKIE,
            format!("houhub-bridge-{}={}", port_of(&grant) + 1, cap_of(&grant)),
        )
        .send()
        .await
        .unwrap();
    ...
    assert!(forbidden.contains("Reopen the page from houhub"));
...
async fn only_the_pages_own_requests_pass() {
    ...
    assert_eq!(
        send(None, Some(format!("http://houhub.example:{}", port_of(&grant)))).await.unwrap().status(),
        StatusCode::FORBIDDEN
    );
...
async fn redirects_and_bodies_pass_through() {
    ...
    let response = client()
        .get(format!("{}/__houhub_bridge/other", base(&grant)))
        .header(header::COOKIE, cookie_for(&grant))
        .header("sec-fetch-site", "same-origin")
        .send()
        .await
...
async fn tabs_share_a_listener_per_target_port() {
    ...
    let response = client()
        ...
        .header(
            header::COOKIE,
            format!("houhub-bridge-{}={}", port_of(&other), cap_of(&first)),
        )
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn houhubs_own_port_is_refused() {
    configure_once();
