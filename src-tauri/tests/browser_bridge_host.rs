...
//! (`HOUHUB_BRIDGE_HOST_PATTERN`): one real listener standing in for houhub's
...
use axum::Router;
use houhub_lib::web::browser_bridge::{self, BridgeConfig, BridgeError, BridgeGrant, HostPattern};
...
const WORKBENCH: &str = "houhub.test";
...
/// HouHub's own listener, with the bridge's host routing in front of it
/// exactly where `web::router::build_router` puts it: outermost, so a
/// bridged request never reaches anything below.
async fn spawn_houhub() -> u16 {
    let app = Router::new()
        .route("/api/whoami", get(|| async { "houhub's own api" }))
        .fallback(|| async { "houhub's own page" })
...
fn cookie_for(grant: &BridgeGrant) -> String {
    format!("houhub-bridge-{}={}", grant.target_port, cap_of(grant))
}

/// The authority the browser would use: the grant's hostname on the port it
/// already talks to, which is houhub's own.
fn authority(grant: &BridgeGrant, houhub_port: u16) -> String {
    format!(
        "{}:{houhub_port}",
...
async fn a_target_is_named_by_hostname_and_bound_to_nothing() {
    ...
    // HouHub's own port is refused here as it is anywhere.
...
#[tokio::test]
async fn the_page_comes_through_houhubs_own_listener() {
    configure_once();
    let upstream = spawn_upstream().await;
    let houhub = spawn_houhub().await;
    let grant = browser_bridge::open(upstream, "tab-page", Some(WORKBENCH))
        .await
        .unwrap();
    let host = authority(&grant, houhub);
    ...
    let response = client()
        .get(format!("http://127.0.0.1:{houhub}{}?to=%2Fhello", grant.entry_path))
    ...
    let response = client()
        .get(format!("http://127.0.0.1:{houhub}/hello"))
        .header(header::HOST, &host)
    ...
    let response = client()
        .get(format!("http://127.0.0.1:{houhub}{}", browser_bridge::PING_PATH))
        .header(header::HOST, &host)
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn houhub_and_the_dev_servers_never_answer_for_each_other() {
    configure_once();
    let upstream = spawn_upstream().await;
    let houhub = spawn_houhub().await;
    ...
    let get = |path: &str, host: String, cookie: bool| {
        let mut request = client()
            .get(format!("http://127.0.0.1:{houhub}{path}"))
            .header(header::HOST, host)
            .header("sec-fetch-site", "same-origin");
        if cookie {
    ...
    // HouHub's own name still reaches houhub, and a target's name never does —
    // not its pages and not its API, whatever the request carries.
    let response = get("/", format!("{WORKBENCH}:{houhub}"), false).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    let response = get("/api/whoami", format!("{WORKBENCH}:{houhub}"), false).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own api");
    let response = get("/api/whoami", host.clone(), true).await.unwrap();
    assert_ne!(response.text().await.unwrap(), "houhub's own api");
    let response = get("/hello", host.clone(), true).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "hello from upstream");

    // A name shaped like the bridge's that no target holds is nobody's
    // claim under `auto`, which describes `<port>.<anything>` — houhub
    ...
    let response = get("/", format!("{RESERVED_PORT}.{WORKBENCH}:{houhub}"), false)
        .await
        .unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    // A hostname that is not the bridge's passes straight through.
    let response = get("/", format!("app.{WORKBENCH}:{houhub}"), false).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    // Including the one the browser would send with no name at all.
    let response = get("/", format!("127.0.0.1:{houhub}"), false).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    // The bridge's own prefix is never forwarded upstream.
    let response = get("/__houhub_bridge/anything", host, true).await.unwrap();
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
}

/// The bridge answers only for names it handed out. `auto` describes a
/// shape — `<port>.<anything>` — and a shape would let it claim names houhub
/// was never asked to take, a workbench on a numeric-leading hostname among
/// them.
#[tokio::test]
async fn only_a_name_houhub_handed_out_is_the_bridges() {
    configure_once();
    let upstream = spawn_upstream().await;
    let houhub = spawn_houhub().await;
    ...
    // from is not this target's — houhub answers, as it would for any other
    // name it was reached at.
    let response = get(format!("{upstream}.elsewhere.test:{houhub}")).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    ...
    let response = get(format!("{upstream}.houhub:{houhub}")).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
    // The name that was handed out still works, and a second one is minted
    // for a workbench reached at a second hostname.
    let response = get(format!("{upstream}.{WORKBENCH}:{houhub}")).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "hello from upstream");
    ...
    let response = get(format!("{upstream}.other.test:{houhub}")).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "hello from upstream");
}
...
async fn a_forwarding_header_decides_nothing_without_a_proxy_in_front() {
    ...
    // forwarded from.
    let response = get(host.clone(), format!("{WORKBENCH}:{houhub}")).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "hello from upstream");
    // And the other way: from houhub's own name, a forwarding header naming
    // a target reaches nothing of the bridge's.
    let response = get(format!("{WORKBENCH}:{houhub}"), host).await.unwrap();
    assert_eq!(response.text().await.unwrap(), "houhub's own page");
}
...
async fn only_this_targets_own_page_may_ask() {
    ...
    for cookie in [
        None,
        Some(format!("houhub-bridge-{}=wrong", grant.target_port)),
        // Another target's cookie, which a browser would not even send to
        // this host-only name.
        Some(format!("houhub-bridge-{}={}", upstream + 1, cap_of(&grant))),
    ] {
    ...
    assert_eq!(
        send(
            Some(ok.clone()),
            None,
            Some(format!("http://{}.{WORKBENCH}:{houhub}", upstream + 1))
        )
        .await
        .unwrap()
        .status(),
...
async fn websockets_are_bridged_through_the_shared_listener() {
    ...
    let connect = |cookie: Option<String>| {
        let mut request = format!("ws://127.0.0.1:{houhub}/ws")
            .into_client_request()
            .unwrap();
        request
            .headers_mut()
