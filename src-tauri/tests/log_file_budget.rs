...
//! one-shot, and the env vars this sets (`HOUHUB_HOME`, `HOUHUB_LOG_MAX_BYTES`) are
...
fn file_log_stops_growing_at_the_daily_ceiling() {
    ...
    // Both are read inside `init_desktop()` below: HOUHUB_HOME relocates
    // `houhub_logs_root()`, HOUHUB_LOG_MAX_BYTES sets the ceiling. Set them before
    // the install rather than via `temp_env`, because the guard returned by the
    // install must outlive the closure.
    std::env::set_var("HOUHUB_HOME", home.path());
    std::env::set_var("HOUHUB_LOG_MAX_BYTES", CEILING.to_string());
    // A stray RUST_LOG/HOUHUB_LOG in the developer's shell would otherwise pick
    // the level (and could filter our test events out entirely).
    std::env::remove_var("RUST_LOG");
    std::env::remove_var("HOUHUB_LOG");

    let logs_dir = houhub_lib::paths::houhub_logs_root();
    assert!(
    ...
    let guard = houhub_lib::logging::init::init_desktop();
