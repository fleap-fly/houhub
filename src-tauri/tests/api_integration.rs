...
use houhub_lib::db::test_helpers::fresh_in_memory_db;
use houhub_lib::web::router::build_router;
use houhub_lib::web::shutdown::ShutdownSignal;
...
async fn open_folder_then_list_open_folders_shows_it() {
    ...
    let open_resp = server
        ...
        .add_header("authorization", format!("Bearer {TEST_TOKEN}"))
        .json(&json!({"path": "/tmp/houhub-test-folder"}))
        .await;
    assert_eq!(
...
async fn json_api_is_gzip_compressed_when_client_accepts() {
    ...
    houhub_lib::db::service::folder_service::add_folder(
        &state.db.conn,
        "/tmp/houhub-compression-test-folder-with-a-reasonably-long-path",
    )
    .await
    .expect("seed folder");
...
async fn get_folder_conversation_accepts_turn_window_params() {
    let (server, state, _data, _static) = build_test_server_with_state().await;
    let folder_id = houhub_lib::db::service::folder_service::add_folder(
        &state.db.conn,
        "/tmp/houhub-window-param-test",
    )
    .await
    .expect("seed folder")
    .id;
    let conv_id = houhub_lib::commands::conversations::create_conversation_core(
        &state.db.conn,
        folder_id,
        houhub_lib::models::AgentType::ClaudeCode,
        None,
    )
    .await
...
#[tokio::test]
async fn houhub_mcp_service_status_reports_a_socketless_runtime_as_stopped() {
    let (server, _data, _static) = build_test_server().await;
    let resp = server
        .post("/api/get_houhub_mcp_service_status")
        .add_header("authorization", format!("Bearer {TEST_TOKEN}"))
        .json(&json!({}))
        .await;
    assert_eq!(resp.status_code(), 200);
...
#[tokio::test]
async fn starting_houhub_mcp_service_without_a_handle_is_rejected() {
    let (server, _data, _static) = build_test_server().await;
    let resp = server
        .post("/api/start_houhub_mcp_service")
        .add_header("authorization", format!("Bearer {TEST_TOKEN}"))
        .json(&json!({}))
        .await;
...
#[tokio::test]
async fn houhub_mcp_service_status_requires_a_token() {
    let (server, _data, _static) = build_test_server().await;
    let resp = server
        .post("/api/get_houhub_mcp_service_status")
        .json(&json!({}))
