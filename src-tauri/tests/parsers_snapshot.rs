...
fn gemini_minimal_session_snapshot() {
    ...
    let project_dir = base.join("tmp").join("houhub");
...
/// SKILL.md inside `<skill_content>`. This pins the rewrite onto houhub's shared
...
fn kimi_code_minimal_session_snapshot() {
    ...
    write(
        ...
        "2026-03-01T10:00:00.000Z INFO  llm config  provider=kimi model=kimi-k2.7-code modelAlias=houhub-managed\n",
    );
    ...
    let wire = [
        ...
        json!({"type":"config.update","modelAlias":"houhub-managed","thinkingLevel":"high","time":1772359200000i64}),
        ...
        json!({"type":"usage.record","model":"houhub-managed","usage":{"inputOther":1200,"output":40,"inputCacheRead":800,"inputCacheCreation":0},"usageScope":"turn","time":1772359204500i64}),
        ...
        json!({"type":"usage.record","model":"houhub-managed","usage":{"inputOther":60,"output":80,"inputCacheRead":2000,"inputCacheCreation":0},"usageScope":"turn","time":1772359205500i64}),
    ];
    let wire_text = wire
        .iter()
        .map(|v| v.to_string())
