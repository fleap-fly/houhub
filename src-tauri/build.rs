...
/// to an existing file at build.rs time. The real `houhub-mcp` sidecar is
...
///   * release.yml         → explicit "Stage houhub-mcp sidecar" step
...
fn ensure_sidecar_placeholder() {
    ...
    let path = dir.join(format!("houhub-mcp-{triple}{ext}"));
    ...
    if needs_placeholder {
        ...
        println!(
            "cargo:warning=houhub-mcp sidecar missing at {}; wrote 0-byte placeholder. \
