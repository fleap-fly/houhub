#
# Houhub Server installer for Windows
# Usage:
#   irm https://raw.githubusercontent.com/fleap-fly/houhub/main/install.ps1 | iex
#   .\install.ps1 -Version v0.5.0
#

param(
    [string]$Version = "",
    [string]$InstallDir = "$env:LOCALAPPDATA\houhub",
    [switch]$NoCleanup
)

$ErrorActionPreference = "Stop"
$Repo = "fleap-fly/houhub"
$Artifact = "houhub-server-windows-x64"
...
$ManagedBins = @("houhub-server", "houhub-mcp")
...
# exe-sibling lookup. Pass -NoCleanup (or set HOUHUB_NO_CLEANUP=1) to disable.
$Cleanup = -not $NoCleanup
if ($env:HOUHUB_NO_CLEANUP -eq "1") {
    $Cleanup = $false
}
...
function Read-BinVersion([string]$BinPath) {
    ...
    $stdout = Join-Path $env:TEMP ("houhub-ver-" + [Guid]::NewGuid().ToString() + ".txt")
    $stderr = Join-Path $env:TEMP ("houhub-vererr-" + [Guid]::NewGuid().ToString() + ".txt")
    try {
...
# directory: that's the binary `Get-Command houhub-server` returns after install.
...
# houhub-server in PATH ends up before the destination after install and must be
...
$DestBin = Join-Path $InstallDir "houhub-server.exe"
...
# Scan PATH for both managed binaries — a stale `houhub-mcp.exe` in an earlier
# PATH entry would be picked by the runtime's `which` fallback once
# `houhub-server.exe` was upgraded out from under it, breaking delegation in
...
# What does `houhub-server` actually resolve to in the current PATH?
$ActiveBin = ""
$resolved = Get-Command houhub-server -ErrorAction SilentlyContinue
...
if ($CurrentVersion -and ($CurrentVersion -eq $TargetVer) `
    ...
    Write-Host "houhub-server is already at version $TargetVer, nothing to do."
    exit 0
}

if ($CurrentVersion) {
    Write-Host "Upgrading houhub-server: $CurrentVersion -> $TargetVer..."
} else {
    Write-Host "Installing houhub-server $Version (windows/x64)..."
}
...
if ($PathConflicts.Count -gt 0) {
    Write-Host ""
    Write-Host "Found other houhub-server binaries in PATH that may shadow ${DestBin}:"
    ...
    if ($Cleanup) {
        ...
        Write-Host "typing 'houhub-server' runs the new install at $DestBin."
    }
    Write-Host ""
}
...
#
# houhub-mcp.exe is the stdio MCP companion spawned per ACP session.
...
$ServerProcesses = Get-Process -Name "houhub-server" -ErrorAction SilentlyContinue
if ($ServerProcesses) {
    Write-Host "Stopping running houhub-server process(es)..."
    $WasRunning = $true
    $ServerProcesses | Stop-Process -Force
    Start-Sleep -Seconds 2
    # Verify stopped
    $StillRunning = Get-Process -Name "houhub-server" -ErrorAction SilentlyContinue
    if ($StillRunning) {
        $StillRunning | Stop-Process -Force
        Start-Sleep -Seconds 1
    }
    Write-Host "houhub-server stopped."
}

$McpProcesses = Get-Process -Name "houhub-mcp" -ErrorAction SilentlyContinue
if ($McpProcesses) {
    Write-Host "Stopping running houhub-mcp companion process(es)..."
    $McpProcesses | Stop-Process -Force
    Start-Sleep -Seconds 1
    $StillRunning = Get-Process -Name "houhub-mcp" -ErrorAction SilentlyContinue
    if ($StillRunning) {
        $StillRunning | Stop-Process -Force
        Start-Sleep -Seconds 1
    }
...
$TmpDir = Join-Path $env:TEMP "houhub-install-$(Get-Random)"
...
# below can resolve `houhub-server`. Without this, the first-time install would
...
if ($Cleanup -and $PathConflicts.Count -gt 0) {
    Write-Host ""
    Write-Host "Removing stale houhub-server binaries..."
    foreach ($c in $PathConflicts) {
        try {
            Remove-Item -LiteralPath $c -Force -ErrorAction Stop
            Write-Host "  removed $c"
        } catch {
            Write-Host "  failed to remove $c (remove it manually so 'houhub-server' resolves to the new install) — $($_.Exception.Message)"
            $ExitStatus = 1
        }
    }
}
...
if ($WasRunning) {
    Write-Host ""
    Write-Host "Note: houhub-server was stopped for the upgrade."
    Write-Host "Please restart it manually to ensure your environment variables (HOUHUB_PORT, HOUHUB_TOKEN, etc.) are preserved:"
    Write-Host "  `$env:HOUHUB_STATIC_DIR=`"$WebDir`"; houhub-server"
}
...
$InstalledVer = ""
try {
    $InstalledVer = (& (Join-Path $InstallDir "houhub-server.exe") --version 2>$null).Trim()
} catch {}
if (-not $InstalledVer) { $InstalledVer = $TargetVer }

Write-Host ""
Write-Host "houhub-server installed to $InstallDir\houhub-server.exe"
Write-Host "houhub-mcp    installed to $InstallDir\houhub-mcp.exe"
Write-Host "Version: $InstalledVer"

# Final smoke: houhub-mcp.exe must exist next to houhub-server.exe so the
# runtime's `locate_houhub_mcp_binary()` exe-sibling lookup hits. A failure
...
$McpPath = Join-Path $InstallDir "houhub-mcp.exe"
...
$ActiveBinAfter = ""
$resolvedAfter = Get-Command houhub-server -ErrorAction SilentlyContinue
...
if (-not $ActiveBinAfter) {
    ...
    Write-Host ""
    Write-Host "Warning: typing 'houhub-server' still runs $ActiveBinAfter, not $DestBin."
...
Write-Host "  `$env:HOUHUB_PORT=`"3080`"; `$env:HOUHUB_TOKEN=`"your-secret`"; `$env:HOUHUB_STATIC_DIR=`"$WebDir`"; houhub-server"
