...
`locate` on `globalThis.__houhubAgent` for Rust to call through world-scoped
...
object — the same isolation that keeps `__houhubAgent` out of the page's reach.
...
The world is separate from the page: `__houhubAgent` is not reachable from page
