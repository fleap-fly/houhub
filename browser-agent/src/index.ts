 ...
 * other direction: Rust evaluates `__houhubAgent.<fn>(...)` in the same world
 * and reads the JSON it returns. Nothing here is reachable from the page — the
 * world is separate, and the page never sees `__houhubAgent`.
 *
...
export type SnapshotOptions = {
   ...
   * keeps `__houhubAgent` out of the page's reach keeps the page's navigations
...
globalThis.__houhubAgent = { snapshot, elementForRef, act, locate, rectOf }
