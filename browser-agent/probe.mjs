...
const evalCall = (code) =>
  `(function(){\n${EVAL_RENDER}\ntry {\nvar __houhubEvalValue = (function () {\n${code}\n})();\nreturn __houhubEvalRender(__houhubEvalValue);\n} catch (e) {\nreturn __houhubEvalError(e);\n}\n})()`
...
const dir = mkdtempSync(join(tmpdir(), "houhub-agent-probe-"))
...
try {
  ...
  const snap = JSON.parse(
    await run("JSON.stringify(__houhubAgent.snapshot({}))")
  )
  console.log("\n=== tree ===\n" + snap.tree + "\n")
  ...
  const mainWorld = await send("Runtime.evaluate", {
    expression: "typeof globalThis.__houhubAgent",
    returnByValue: true,
  })
  check(
    "the page cannot see __houhubAgent",
    mainWorld.result.result.value,
    "undefined"
  )

  const cut = JSON.parse(
    await run("JSON.stringify(__houhubAgent.snapshot({maxChars: 40}))")
  )
  ...
  check(
    "a live ref resolves",
    await run(`!!__houhubAgent.elementForRef(${g}, ${kept})`),
    true
  )
  check(
    "a ref from another document does not",
    await run(`__houhubAgent.elementForRef("other", ${kept})`),
    null
  )
  check(
    "a ref for a removed element does not",
    await run(
      `(() => { __houhubAgent.elementForRef(${g}, ${spent}).remove();
                return __houhubAgent.elementForRef(${g}, ${spent}) })()`
    ),
    null
  )
  ...
  check(
    ...
    await run(
      `(() => { const el = __houhubAgent.elementForRef(${g}, ${kept});
                history.pushState({}, "", "?routed");
                return [el.isConnected, __houhubAgent.elementForRef(${g}, ${kept})] })()`
    ),
    [true, null]
  )
  check(
    ...
    await run(
      `(() => { const s = __houhubAgent.snapshot({});
                const m = s.tree.match(/button "Export" \\[ref=(e\\d+)\\]/);
                return !!__houhubAgent.elementForRef(s.generation, m[1]) })()`
    ),
    true
  )
  ...
  check(
    ...
    await new Promise(async (resolve) => {
      ...
      await sleep(150)
      await run(
        `globalThis.__s = __houhubAgent.snapshot({});
         globalThis.__m = __s.tree.match(/button "Export" \\[ref=(e\\d+)\\]/)[1];
         history.back(); history.forward();`
      )
      await sleep(250)
      resolve(await run(`__houhubAgent.elementForRef(__s.generation, __m)`))
    }),
    null
  )
  ...
  const actJson = async (gen, ref, request) =>
    JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.act(${gen}, ${JSON.stringify(ref)}, ${JSON.stringify(request)}))`
      )
    )
  ...
  }
  {
    ...
    const located = JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.locate(${gen}, ${JSON.stringify(under)}))`
      )
    )
    ...
    const point = JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.locate(${gen}, ${JSON.stringify(count)}))`
      )
    )
    check(
  ...
  }
  {
    ...
    const located = JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.locate(${gen}, ${JSON.stringify(clipped)}))`
      )
    )
    check(
  ...
  }
  {
    ...
    await run(
      `__houhubAgent.elementForRef(${gen}, ${JSON.stringify(spent)}).remove()`
    )
  ...
  }
  {
    ...
    check(
      ...
      [
        ...
        await run(
          `__houhubAgent.elementForRef(${JSON.stringify(a.generation)}, ${JSON.stringify(m)})`
        ),
        !!(await run(
          `__houhubAgent.elementForRef(${JSON.stringify(b.generation)}, ${JSON.stringify(m)})`
        )),
      ],
      [true, null, true]
    )
    ...
    const cut = JSON.parse(
      await run("JSON.stringify(__houhubAgent.snapshot({maxChars: 60}))")
    )
    ...
    check(
      ...
      [
        ...
        shown.length > 0,
        await run(
          `__houhubAgent.elementForRef(${JSON.stringify(cut.generation)}, ${JSON.stringify(hidden)})`
        ),
      ],
      [true, true, null]
    )
    ...
    await run(
      `__houhubAgent.elementForRef(${JSON.stringify(live.generation)}, ${JSON.stringify(doomed)}).remove()`
    )
    ...
    const snap = async (maxChars) =>
      JSON.parse(
        await run(
          `JSON.stringify(__houhubAgent.snapshot(${JSON.stringify(maxChars ? { maxChars } : {})}))`
        )
      )
      ...
      check(
        ...
        await run(
          `(() => { const here = location.href;
                    ...
                    return __houhubAgent.elementForRef(s.generation, m) })()`
        ),
        null
      )
  ...
  check(
    ...
    await run(
      `__houhubAgent.snapshot({epoch: "nav-7"}).generation.endsWith(".nav-7")`
    ),
    true
  )
  check(
    ...
    await run(
      `(() => { const s = __houhubAgent.snapshot({epoch: "nav-7"});
                const m = s.tree.match(/button "Export" \\[ref=(e\\d+)\\]/);
                __houhubAgent.snapshot({epoch: "nav-8"});
                return __houhubAgent.elementForRef(s.generation, m[1]) })()`
    ),
    null
  )
  ...
  // ── capturing ──────────────────────────────────────────────────────────
  {
    ...
    const rect = JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.rectOf(${gen}, ${JSON.stringify(count)}))`
      )
    )
    check(
    ...
    const stale = JSON.parse(
      await run(
        `JSON.stringify(__houhubAgent.rectOf("other", ${JSON.stringify(count)}))`
      )
    )
  ...
  // host puts it and the listener in this one where the helper has it.
  {
    await run(
      `globalThis.__lines = [];
       document.addEventListener("houhub:console", (e) => {
    ...
    const pageView = await send("Runtime.evaluate", {
      expression:
        "JSON.stringify([console.log.name, typeof globalThis.__lines, typeof __houhubAgent])",
      returnByValue: true,
    })
    check(
  ...
  // page, and the highlight must stay unreadable from it.
  {
    await run(
      `globalThis.__picks = []; globalThis.__sent = [];
       globalThis.__houhubSend = (m) => { __sent.push(m); __picks.push(JSON.parse(m)) }; true`
    )
    await run(PICKER)
    const seenByPage = await send("Runtime.evaluate", {
      expression:
        "JSON.stringify([typeof globalThis.__houhubPicker, typeof globalThis.__houhubSend])",
      returnByValue: true,
    })
    check(
    ...
    await run('__houhubPicker.start("p1")')
    const overlay = await send("Runtime.evaluate", {
      expression: `(() => { const el = document.querySelector("[data-houhub-picker]");
    ...
    const stillUp = await run(
      '!!document.querySelector("[data-houhub-picker]")'
    )
    await sleep(800)
    check(
      "the overlay stays for the drain once something is picked, then goes",
      [stillUp, await run('!!document.querySelector("[data-houhub-picker]")')],
      [true, false]
    )
    ...
    await run('__houhubPicker.start("forged")')
    await send("Runtime.evaluate", {
    ...
    check(
      ...
      [
        ...
        await run('!!document.querySelector("[data-houhub-picker]")'),
      ],
      [0, true]
    )
    await run("__houhubPicker.stop()")
    ...
    await run('__houhubPicker.start("frame")')
    ...
    await run('__houhubPicker.start("deep")')
    ...
    await run('__houhubPicker.start("nested")')
    ...
    await run('__houhubPicker.start("wheel")')
    ...
    await run('__houhubPicker.start("moved")')
    await sleep(120)
    const covered = JSON.parse(
      await run(`JSON.stringify((() => {
        const h = document.querySelector("[data-houhub-picker]");
    ...
    await run('__houhubPicker.start("captured")')
    ...
    await run("window.scrollTo(0, 0)")
    await run('__houhubPicker.start("cover")')
    await send("Runtime.evaluate", {
      expression: `(() => {
    ...
    await run('__houhubPicker.start("twice")')
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
      x: twiceAt.x,
      y: twiceAt.y,
    ...
    await run('__houhubPicker.start("keys")')
    ...
    // last thing hovered.
    await run('__houhubPicker.start("meddled")')
    ...
    await send("Runtime.evaluate", {
      ...
      expression: `(() => { const h = document.querySelector("[data-houhub-picker]");
    ...
    await run('__houhubPicker.start("emoji")')
    ...
    await run('__houhubPicker.start("fat")')
    ...
    await run('__houhubPicker.start("p2")')
    await send("Input.dispatchKeyEvent", {
      type: "keyDown",
      key: "Escape",
      code: "Escape",
    ...
    await run('__houhubPicker.start("moved")')
    await run(
      'document.getElementById("trap").appendChild(document.querySelector("[data-houhub-picker]"))'
    )
    ...
    check(
      ...
      [
        ...
        await run(
          '(document.querySelector("[data-houhub-picker]") || {}).parentElement?.tagName ?? "none"'
        ),
      ],
    ...
    await run('__houhubPicker.start("inert")')
    await run(
      'document.querySelector("[data-houhub-picker]").setAttribute("inert", "")'
    )
    await sleep(500)
    for (const [type, button, buttons] of [
      ["mouseMoved", "none", 0],
    ...
    await run('__houhubPicker.start("inertfight")')
    await send("Runtime.evaluate", {
      expression: `(() => {
        const el = document.querySelector("[data-houhub-picker]")
    ...
    await run('__houhubPicker.start("twiceframe")')
    await send("Input.dispatchMouseEvent", {
      type: "mouseMoved",
      x: movedBox.x,
      y: movedBox.y,
    ...
    await run('__houhubPicker.start("framekeys")')
    await sleep(80)
    for (const type of ["keyDown", "char", "keyUp"]) {
      await send("Input.dispatchKeyEvent", {
        type,
    ...
    await run('__houhubPicker.start("popover")')
    ...
    await run(
      'document.querySelector("[data-houhub-picker]").removeAttribute("popover")'
    )
    ...
    await run('__houhubPicker.start("smallpop")')
    ...
    await run('__houhubPicker.start("fakehide")')
    await send("Runtime.evaluate", {
    ...
    await run('__houhubPicker.start("modal")')
    ...
    await run('__houhubPicker.start("deep33")')
  ...
  // checks ran the whole thing in houhub's world and still looked green until
  ...
  check(
    "a snippet cannot see houhub's world",
    (await evalIn("return typeof globalThis.__houhubAgent")).value,
    "undefined"
  )
  // And leaves nothing behind in the page that would tell it houhub had run.
  check(
    "an evaluation leaves no names on the page",
    await inPage(
      'typeof globalThis.__houhubEvalRender + "," + typeof globalThis.__houhubEvalClip'
    ),
    "undefined,undefined"
  )
